...
;
	...
	public static function fileGetContents($file) {
		...
		if ((strlen ( $file ) > 1) and ($file [0] == '/') and ($file [1] != '/') and ! empty ( $_SERVER ['DOCUMENT_ROOT'] ) and ($_SERVER ['DOCUMENT_ROOT'] != '/')) {
		...
		foreach ( $alt as $f ) {
			...
			if (($ret === FALSE) and ! ini_get ( 'allow_url_fopen' ) and function_exists ( 'curl_init' ) and preg_match ( '%^(https?|ftp)://%', $f )) {
