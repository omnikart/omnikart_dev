...
class TCPDF {
	 ...
	 *        @protected
	 */
	protected $header_text_color = array (
			0,
			0,
			0 
	);
	 ...
	 *        @protected
	 */
	protected $header_line_color = array (
			0,
			0,
			0 
	);
	 ...
	 *        @protected
	 */
	protected $footer_text_color = array (
			0,
			0,
			0 
	);
	 ...
	 *        @protected
	 */
	protected $footer_line_color = array (
			0,
			0,
			0 
	);
	 ...
	 */
	protected $signature_appearance = array (
			'page' => 1,
			'rect' => '0 0 0 0' 
	);
	 ...
	 *        	page orientation. Possible values are (case insensitive):<ul><li>P or Portrait (default)</li><li>L or Landscape</li><li>'' (empty string) for automatic orientation</li></ul>
	 * @param $unit (string)
	 ...
	 * 
	 * @param $format (mixed)
	 *        	The format used for pages. It can be either: one of the string values specified at getPageSizeFromFormat() documentation or an array of two numbers (width, height) or an array containing the following measures and options:<ul>
	 ...
	 *        	</ul>
	 * @param $orientation (string)
	 *        	page orientation. Possible values are (case insensitive):<ul>
	 ...
	 * 
	 * @param $brd (mixed)
	 *        	Indicates if borders must be drawn around the cell. The value can be a number:<ul><li>0: no border (default)</li><li>1: frame</li></ul> or a string containing some or all of the following characters (in any order):<ul><li>L: left</li><li>T: top</li><li>R: right</li><li>B: bottom</li></ul> or an array of line styles for each border group - for example: array('LTRB' => array('width' => 2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)))
	 ...
	 *        	Font style. Possible values are (case insensitive):<ul><li>empty string: regular</li><li>B: bold</li><li>I: italic</li><li>U: underline</li><li>D: line-through</li><li>O: overline</li></ul> or any combination. The default value is regular.
	 * @param $fontsize (float)
	 *        	Font size in points. The default value is the current size.
	 * @param $getarray (boolean)
	...
	public function Annotation($x, $y, $w, $h, $text, $opt = array('Subtype'=>'Text'), $spaces = 0) {
		...
		if (! $this->pdfa_mode) {
			if ((($opt ['Subtype'] == 'FileAttachment') or ($opt ['Subtype'] == 'Sound')) and (! TCPDF_STATIC::empty_string ( $opt ['FS'] )) and (@file_exists ( $opt ['FS'] ) or TCPDF_STATIC::isValidURL ( $opt ['FS'] )) and (! isset ( $this->embeddedfiles [basename ( $opt ['FS'] )] ))) {
	 ...
	 * @param $ignore_min_height (boolean)
	 ...
	 *        	cell vertical alignment relative to the specified Y value. Possible values are:<ul><li>T : cell top</li><li>C : center</li><li>B : cell bottom</li><li>A : font top</li><li>L : font baseline</li><li>D : font bottom</li></ul>
	 * @param $valign (string)
	 *        	text vertical alignment inside the cell. Possible values are:<ul><li>T : top</li><li>M : middle</li><li>B : bottom</li></ul>
	 ...
	 * @param $valign (string)
	 *        	Vertical alignment of text (requires $maxh = $h > 0). Possible values are:<ul><li>T: TOP</li><li>M: middle</li><li>B: bottom</li></ul>. This feature works only when $ishtml=false and the cell must fit in a single page.
	 * @param $fitcell (boolean)
	...
	public function getNumLines($txt, $w = 0, $reseth = false, $autopadding = true, $cellpadding = '', $border = 0) {
		...
		for($i = 0; $i < $length; ++ $i) {
			...
			if (($c != 160) and (($c == 173) or preg_match ( $this->re_spaces, TCPDF_FONTS::unichr ( $c, $this->isunicode ) ) or (($c == 45) and ($i > 0) and ($i < ($length - 1)) and @preg_match ( '/[\p{L}]/' . $this->re_space ['m'], TCPDF_FONTS::unichr ( $chars [($i - 1)], $this->isunicode ) ) and @preg_match ( '/[\p{L}]/' . $this->re_space ['m'], TCPDF_FONTS::unichr ( $chars [($i + 1)], $this->isunicode ) )))) {
				$lastSeparator = $i;
			}
	...
	public function Write($h, $txt, $link = '', $fill = false, $align = '', $ln = false, $stretch = 0, $firstline = false, $firstblock = false, $maxh = 0, $wadj = 0, $margin = '') {
		...
		while ( $i < $nb ) {
			...
			if ($c == 10) { // 10 = "\n" = new line
				...
				if (($c != 160) and (($c == 173) or preg_match ( $this->re_spaces, TCPDF_FONTS::unichr ( $c, $this->isunicode ) ) or (($c == 45) and ($i < ($nb - 1)) and @preg_match ( '/[\p{L}]/' . $this->re_space ['m'], TCPDF_FONTS::unichr ( $pc, $this->isunicode ) ) and @preg_match ( '/[\p{L}]/' . $this->re_space ['m'], TCPDF_FONTS::unichr ( $chars [($i + 1)], $this->isunicode ) )))) {
				...
				if (($l > $wmax) or (($c == 173) and (($l + $tmp_shy_replacement_width) >= $wmax))) {
					...
					if ($sep == - 1) {
						...
						if (($this->rtl and ($this->x <= ($this->w - $this->rMargin - $this->cell_padding ['R'] - $margin ['R'] - $chrwidth))) or ((! $this->rtl) and ($this->x >= ($this->lMargin + $this->cell_padding ['L'] + $margin ['L'] + $chrwidth)))) {
	...
	public function Image($file, $x = '', $y = '', $w = 0, $h = 0, $type = '', $link = '', $align = '', $resize = false, $dpi = 300, $palign = '', $ismask = false, $imgmask = false, $border = 0, $fitbox = false, $hidden = false, $fitonpage = false, $alt = false, $altimgs = array()) {
		...
		if ($newimage) {
			...
			if ((method_exists ( 'TCPDF_IMAGES', $mtd )) and (! ($resize and (function_exists ( $gdfunction ) or extension_loaded ( 'imagick' ))))) {
				...
				if (($ismask === false) and ($imgmask === false) and (strpos ( $file, '__tcpdf_' . $this->file_id . '_imgmask_' ) === FALSE) and (($info === 'pngalpha') or (isset ( $info ['trns'] ) and ! empty ( $info ['trns'] )))) {
	 ...
	 * @param $preserve_objcopy (boolean)
	...
	protected function _putcatalog() {
		...
		// AcroForm
		if (! empty ( $this->form_obj_id ) or ($this->sign and isset ( $this->signature_data ['cert_type'] )) or ! empty ( $this->empty_signature_appearance )) {
			$out .= ' /AcroForm <<';
			$objrefs = '';
			...
			$out .= ' >>';
			// signatures
			if ($this->sign and isset ( $this->signature_data ['cert_type'] ) and (empty ( $this->signature_data ['approval'] ) or ($this->signature_data ['approval'] != 'A'))) {
				if ($this->signature_data ['cert_type'] > 0) {
	 ...
	 * The curve shall extend from the current point to the point (x3, y3), using (x1, y1) and (x2, y2) as the Bezier control points.
	...
	/**
	 * Append a cubic Bezier curve to the current path.
	 * The curve shall extend from the current point to the point (x3, y3), using the current point and (x2, y2) as the Bezier control points.
	 * The new current point shall be (x3, y3).
	 * 
	 * @param $x2 (float)
	 *        	Abscissa of control point 2.
	...
	/**
	 * Append a cubic Bezier curve to the current path.
	 * The curve shall extend from the current point to the point (x3, y3), using (x1, y1) and (x3, y3) as the Bezier control points.
	 * The new current point shall be (x3, y3).
	 * 
	 * @param $x1 (float)
	 *        	Abscissa of control point 1.
	 ...
	 * (Check the "Entries in a Graphics State Parameter Dictionary" on PDF 32000-1:2008).
	 * 
	 * @param $stroking (boolean)
	 *        	If true apply overprint for stroking operations.
	 * @param $nonstroking (boolean)
		...
		while ( $elkey < $maxel ) {
			...
			if (preg_match ( $tagpattern, $element )) {
				...
				if ($element [0] == '/') {
					...
					if ((($dom [$key] ['value'] == 'ul') or ($dom [$key] ['value'] == 'ol') or ($dom [$key] ['value'] == 'dl')) and (! isset ( $dom [$key] ['align'] ) or TCPDF_STATIC::empty_string ( $dom [$key] ['align'] ) or ($dom [$key] ['align'] != 'J'))) {
						if ($this->rtl) {
							$dom [$key] ['align'] = 'R';
						} else {
							$dom [$key] ['align'] = 'L';
			...
			if ($dom [$key] ['tag'] or ($key == 0)) {
				...
				if ((! $this->newline) and ($dom [$key] ['value'] == 'img') and (isset ( $dom [$key] ['height'] )) and ($dom [$key] ['height'] > 0)) {
					...
					if (($fontname != $curfontname) or ($fontstyle != $curfontstyle) or ($fontsize != $curfontsize) or ($this->cell_height_ratio != $dom [$key] ['line-height']) or ($dom [$key] ['tag'] and $dom [$key] ['opening'] and ($dom [$key] ['value'] == 'li'))) {
						if (($key < ($maxel - 1)) and (($dom [$key] ['tag'] and $dom [$key] ['opening'] and ($dom [$key] ['value'] == 'li')) or ($this->cell_height_ratio != $dom [$key] ['line-height']) or (! $this->newline and is_numeric ( $fontsize ) and is_numeric ( $curfontsize ) and ($fontsize >= 0) and ($curfontsize >= 0) and (($fontsize != $curfontsize) or ($fontstyle != $curfontstyle) or ($fontname != $curfontname))))) {
							if ($this->page > $startlinepage) {
			...
			case 'tr' :
				{
					...
					if ($this->num_columns > 1) {
						if (($prev_page < $this->page) and ((($this->current_column == 0) and ($dom [($dom [$key] ['parent'])] ['endcolumn'] == ($this->num_columns - 1))) or ($this->current_column == $dom [($dom [$key] ['parent'])] ['endcolumn']))) {
							// page jump
							$this->selectColumn ( 0 );
	 ...
	 * @param $cell (boolean)
	 *        	if true add the default left (or right if RTL) padding to each new line (default false).
	 * @param $firsttag (boolean)
	 *        	set to true when the tag is the first.
	 * @param $lasttag (boolean)
	 ...
	 * 
	 * @param $htmlval (string)
	 *        	String containing values and unit.
	 * @param $refsize (string)
	 ...
	 * @param $align (string)
	 *        	Indicates the alignment of the pointer next to template insertion relative to template height. The value can be:<ul><li>T: top-right for LTR or top-left for RTL</li><li>M: middle-right for LTR or middle-left for RTL</li><li>B: bottom-right for LTR or bottom-left for RTL</li><li>N: next line</li></ul>
	 * @param $palign (string)
	 *        	Allows to center or align the template on the current line. Possible values are:<ul><li>L : left align</li><li>C : center</li><li>R : right align</li><li>'' : empty string : left for LTR or right for RTL</li></ul>
	 * @param $fitonpage (boolean)
	 ...
	 *        	URL or identifier returned by AddLink().
	 * @param $align (string)
	 *        	Indicates the alignment of the pointer next to image insertion relative to image height. The value can be:<ul><li>T: top-right for LTR or top-left for RTL</li><li>M: middle-right for LTR or middle-left for RTL</li><li>B: bottom-right for LTR or bottom-left for RTL</li><li>N: next line</li></ul> If the alignment is an empty string, then the pointer will be restored on the starting SVG position.
	 * @param $palign (string)
	 *        	Allows to center or align the image on the current line. Possible values are:<ul><li>L : left align</li><li>C : center</li><li>R : right align</li><li>'' : empty string : left for LTR or right for RTL</li></ul>
	 * @param $border (mixed)
	 *        	Indicates if borders must be drawn around the cell. The value can be a number:<ul><li>0: no border (default)</li><li>1: frame</li></ul> or a string containing some or all of the following characters (in any order):<ul><li>L: left</li><li>T: top</li><li>R: right</li><li>B: bottom</li></ul> or an array of line styles for each border group - for example: array('LTRB' => array('width' => 2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)))
	 ...
	 * 
	 * @param $svgstyle (array)
	 *        	array of SVG styles to apply
	 * @param $prevsvgstyle (array)
	 ...
	 * @param $clip_function (string)
	 *        	clip function
	 * @param $clip_params (array)
