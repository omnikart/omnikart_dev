...
class OpenStack extends Base {
	...
	/**
	 * This holds the HTTP User-Agent: used for all requests to the services.
	 * It
	 ...
	 * service (and to select it from the Service Catalog).
	 * Use the
	...
	public function __construct($url, array $secret, array $options = array()) {
		...
		$version = phpversion ();
		if ($version < '5.3.1') {
			throw new Exceptions\UnsupportedVersionError ( sprintf ( Lang::translate ( 'PHP version [%s] is not supported' ), $version ) );
		}
	...
	/**
	 * Sets default values (an array) for a service.
	 * Each array must contain a
	...
	/**
	 * Get a specific default value for a service.
	 ...
	 * @api
	 * 
	 * @param integer $t_http
	 *        	the HTTP timeout value (the max period that
	 *        	the OpenStack object will wait for any HTTP request to complete).
	 *        	Value is in seconds.
	 * @param integer $t_conn
	 *        	the Connect timeout value (the max period
	 *        	that the OpenStack object will wait to establish an HTTP
	 *        	connection). Value is in seconds.
	 * @param integer $t_overlimit
	 *        	the overlimit timeout value (the max period
	...
	public function authenticate() {
		// try to auth
		$response = $this->request ( $this->url (), 'POST', array (
		...
		if ($response->HttpStatus () >= 400) {
			throw new Exceptions\AuthenticationError ( sprintf ( Lang::translate ( 'Authentication failure, status [%d], response [%s]' ), $response->httpStatus (), $json ) );
		}
		 ...
		 * we set it.
		 * This occurs with pure Keystone, but not
	 ...
	 * @param
	 *        	array headers - an associative array of headers
	 * @param
	 *        	string data - either a string or a resource (file pointer) to
	...
	public function request($url, $method = 'GET', $headers = array(), $data = null) {
		...
		if (is_resource ( $data )) {
			...
			switch ($method) {
				...
				case 'GET' :
					...
					$http->setOption ( CURLOPT_WRITEFUNCTION, array (
							$this,
							'_write_cb' 
					) );
					break;
				...
				case 'POST' :
					...
					if (! isset ( $headers ['Content-Length'] )) {
						throw new Exceptions\HttpError ( Lang::translate ( 'The Content-Length: header must be specified for file uploads' ) );
					}
					...
					$http->setOption ( CURLOPT_READFUNCTION, array (
							$this,
							'_read_cb' 
					) );
					break;
			...
			throw new Exceptions\HttpError ( Lang::translate ( 'Unrecognized data type for PUT/POST body, must be string or resource' ) );
		}
		...
		if ($response->httpStatus () == 413) {
			...
			if (isset ( $object->overLimit )) {
				 ...
				 * the field returned is "retryAfter".
				 * If the doc changes,
				...
				if ($sleepInterval && $sleepInterval <= $this->getOverlimitTimeout ()) {
					...
					throw new Exceptions\HttpOverLimitError ( sprintf ( Lang::translate ( 'Over limit; next available request [%s][%s] is not for [%d] seconds at [%s]' ), $method, $url, $sleepInterval, $retryAfter ) );
				}
			}
			// @codeCoverageIgnoreEnd
		}
		...
		switch ($response->httpStatus ()) {
			case 401 :
				throw new Exceptions\HttpUnauthorizedError ( sprintf ( Lang::translate ( '401 Unauthorized for [%s] [%s]' ), $url, $response->HttpBody () ) );
				break;
			case 403 :
				throw new Exceptions\HttpForbiddenError ( sprintf ( Lang::translate ( '403 Forbidden for [%s] [%s]' ), $url, $response->HttpBody () ) );
				break;
			case 413 : // limit
				throw new Exceptions\HttpOverLimitError ( sprintf ( Lang::translate ( '413 Over limit for [%s] [%s]' ), $url, $response->HttpBody () ) );
				break;
			default :
	 ...
	 */
	public function setDefaults($service, $name = null, $region = null, $urltype = null) {
		if (! isset ( $this->defaults [$service] )) {
			throw new Exceptions\UnrecognizedServiceError ( sprintf ( Lang::translate ( 'Service [%s] is not recognized' ), $service ) );
		}
	...
	public function _write_cb($ch, $data) {
		...
		if (false === ($fp = $this->getFileDescriptor ( $url ))) {
			throw new Exceptions\HttpUrlError ( sprintf ( Lang::translate ( 'Cannot find file descriptor for URL [%s]' ), $url ) );
		}
	 ...
	 * @param string $urltype
	 *        	the URL type (normally "publicURL")
	 * @return Orchestration\Service @codeCoverageIgnore
	 */
	...
	public function service($class, $name = null, $region = null, $urltype = null) {
		...
		if (! $name = $name ?  : $default ['name']) {
			throw new Exceptions\ServiceValueError ( sprintf ( Lang::translate ( 'No value for %s name' ), $class ) );
		}
		
		if (! $region = $region ?  : $default ['region']) {
			throw new Exceptions\ServiceValueError ( sprintf ( Lang::translate ( 'No value for %s region' ), $class ) );
		}
		
		if (! $urltype = $urltype ?  : $default ['urltype']) {
			throw new Exceptions\ServiceValueError ( sprintf ( Lang::translate ( 'No value for %s URL type' ), $class ) );
		}
