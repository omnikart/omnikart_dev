...
class Service extends AbstractService {
	 ...
	 *
	 * @param \OpenCloud\OpenStack $conn
	 *        	connection object
	 ...
	 *
	 * @param \OpenCloud\Compute\Server $server
	 *        	the server for which to
	...
	public function asyncRequest($url, $method = 'GET', $headers = array(), $body = null) {
		...
		if ($resp->HttpStatus () > 204) {
			throw new Exceptions\AsyncHttpError ( sprintf ( Lang::translate ( 'Unexpected HTTP status for async request: URL [%s] method [%s] status [%s] response [%s]' ), $url, $method, $resp->HttpStatus (), $resp->HttpBody () ) );
		}
	...
	public function import($data) {
		...
		// debug it
		$this->getLogger ()->info ( 'Importing [{json}]', array (
				'json' => $json 
		) );
	...
	public function simpleRequest($url) {
		...
		if ($response->HttpStatus () > 202) {
			throw new Exceptions\HttpError ( sprintf ( Lang::translate ( 'Unexpected status [%s] for URL [%s], body [%s]' ), $response->HttpStatus (), $url, $response->HttpBody () ) );
		}
		// @codeCoverageIgnoreEnd
		
		// Decode the JSON
		$json = $response->httpBody ();
		$this->getLogger ()->info ( 'Limit Types JSON [{json}]', array (
				'json' => $json 
		) );
