...
class User extends PersistentObject {
	...
	public function getName() {
		if (empty ( $this->name )) {
			throw new Exceptions\DatabaseNameError ( Lang::translate ( 'This user does not have a name yet' ) );
		}
		return $this->name;
	}
	...
	public function delete() {
		...
		if ($response->HttpStatus () > 202) {
			throw new Exceptions\UserDeleteError ( sprintf ( Lang::translate ( 'Error deleting user [%s], status [%d] response [%s]', $this->name, $response->HttpStatus (), $response->HttpBody () ) ) );
		}
