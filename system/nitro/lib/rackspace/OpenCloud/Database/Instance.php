...
class Instance extends PersistentObject {
	 ...
	 *
	 * @param \OpenCloud\DbService $service
	 *        	the DbService object associated
	 *        	with this
	 * @param mixed $info
	 ...
	 * @api
	 * 
	 * @return s \OpenCloud\HttpResponse
	 */
	 ...
	 * @api
	 * 
	 * @param \OpenCloud\Compute\Flavor $flavor
	 *        	a flavor object
	 * @return s \OpenCloud\HttpResponse
	 */
	...
	public function enableRootUser() {
		...
		if ($response->HttpStatus () > 202) {
			throw new Exceptions\InstanceError ( sprintf ( Lang::translate ( 'Error enabling root user for instance [%s], status [%d] response [%s]' ), $this->name, $response->HttpStatus (), $response->HttpBody () ) );
		}
	...
	public function databaseList() {
		...
		if ($response->HttpStatus () > 200) {
			throw new Exceptions\DatabaseListError ( sprintf ( Lang::translate ( 'Error listing databases for instance [%s], status [%d] response [%s]' ), $this->name, $response->HttpStatus (), $response->HttpBody () ) );
		}
		// @codeCoverageIgnoreEnd
	...
	public function userList() {
		...
		// @codeCoverageIgnoreStart
		if ($response->HttpStatus () > 200) {
			throw new Exceptions\UserListError ( sprintf ( Lang::translate ( 'Error listing users for instance [%s], status [%d] response [%s]' ), $this->name, $response->HttpStatus (), $response->HttpBody () ) );
		}
		// @codeCoverageIgnoreEnd
