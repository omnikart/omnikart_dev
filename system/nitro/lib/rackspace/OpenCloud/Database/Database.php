...
class Database extends PersistentObject {
	 ...
	 * @param mixed $info
	 *        	if an array or object, treated as properties to set;
	...
	/**
	 * Returns name of this database.
	 * Because it's so important (i.e. as an
	...
	public function create($params = array()) {
		...
		if ($response->HttpStatus () != 202) {
			throw new Exceptions\DatabaseCreateError ( sprintf ( Lang::translate ( 'Error creating database [%s], status [%d] response [%s]' ), $this->name, $response->HttpStatus (), $response->HttpBody () ) );
		}
	...
	public function delete() {
		...
		// @codeCoverageIgnoreStart
		if ($response->HttpStatus () != 202) {
			throw new Exceptions\DatabaseDeleteError ( sprintf ( Lang::translate ( 'Error deleting database [%s], status [%d] response [%s]' ), $this->name, $response->HttpStatus (), $response->HttpBody () ) );
		}
		// @codeCoverageIgnoreEnd
	...
	protected function createJson(array $params = array()) {
		$database = ( object ) array_merge ( array (
				'name' => $this->getName (),
				$params 
		) );
