...
class Server extends PersistentObject {
	 ...
	 *        	- false (default) indicates you want to
	...
	public function imageSchedule($retention = false) {
		...
		if ($response->HttpStatus () >= 300) {
			throw new Exceptions\ServerImageScheduleError ( sprintf ( Lang::translate ( 'Error in Server::ImageSchedule(), status [%d], response [%s]' ), $response->HttpStatus (), $response->HttpBody () ) );
		}
	...
	public function rescue() {
		...
		if (! isset ( $newobj->adminPass )) {
			throw new Exceptions\ServerActionError ( sprintf ( Lang::translate ( 'Rescue() method failed unexpectedly, status [%s] response [%s]' ), $resp->httpStatus (), $resp->httpBody () ) );
			// @codeCoverageIgnoreEnd
		} else {
	 ...
	 * @api
	 * 
	 * @param OpenCloud\VolumeService\Volume $vol
	 *        	the volume to attach. If
	...
	public function volumeAttachmentList() {
		return $this->getService ()->collection ( '\OpenCloud\Compute\VolumeAttachment', NULL, $this );
	}
	 ...
	 *        	creates {server ...} by default, but can also
