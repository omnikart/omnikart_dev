...
class ServerMetadata extends Metadata {
	...
	public function __construct(Server $parent, $key = null) {
		...
		if ($this->Parent ()->id) {
			...
			if ($response->httpStatus () >= 300) {
				throw new Exceptions\MetadataError ( sprintf ( Lang::translate ( 'Unable to retrieve metadata [%s], response [%s]' ), $this->Url (), $response->HttpBody () ) );
			}
			// @codeCoverageIgnoreEnd
			
			$this->getLogger ()->info ( Lang::translate ( 'Metadata for [{url}] is [{body}]' ), array (
					'url' => $this->url (),
	...
	public function create() {
		// perform the request
		$response = $this->parent ()->Service ()->Request ( $this->Url (), 'PUT', array (), $this->GetMetadataJson () );
		
		// @codeCoverageIgnoreStart
		if ($response->HttpStatus () >= 300) {
			throw new \OpenCloud\Common\Exceptions\MetadataCreateError ( sprintf ( Lang::translate ( 'Error setting metadata on [%s], response [%s]' ), $this->Url (), $response->HttpBody () ) );
		}
		// @codeCoverageIgnoreEnd
	}
	...
	public function update() {
		// perform the request
		$response = $this->parent ()->getService ()->Request ( $this->url (), 'POST', array (), $this->getMetadataJson () );
		
		// @codeCoverageIgnoreStart
		if ($response->HttpStatus () >= 300) {
			throw new Exceptions\MetadataUpdateError ( sprintf ( Lang::translate ( 'Error updating metadata on [%s], response [%s]' ), $this->Url (), $response->HttpBody () ) );
		}
		// @codeCoverageIgnoreEnd
	}
	...
	public function delete() {
		// perform the request
		$response = $this->parent ()->getService ()->Request ( $this->url (), 'DELETE', array () );
		
		// @codeCoverageIgnoreStart
		if ($response->httpStatus () >= 300) {
			throw new Exceptions\MetadataDeleteError ( sprintf ( Lang::translate ( 'Error deleting metadata on [%s], response [%s]' ), $this->url (), $response->httpBody () ) );
		}
		// @codeCoverageIgnoreEnd
	}
	...
	public function __set($key, $value) {
		...
		if ($this->_key && $key != $this->_key) {
			throw new Exceptions\MetadataKeyError ( sprintf ( Lang::translate ( 'You cannot set extra values on [%s]' ), $this->Url () ) );
		}
