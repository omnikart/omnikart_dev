...
/**
 * A load balancer is a logical device which belongs to a cloud account.
 * It is
...
class LoadBalancer extends PersistentObject {
	 ...
	 * @return void
	 */
	public function addNode($address, $port, $condition = 'ENABLED', $type = null, $weight = null) {
		$node = $this->Node ();
		...
		switch ($cond) {
			...
			case 'DRAINING' :
				...
				break;
			default :
				throw new Exceptions\DomainError ( sprintf ( Lang::translate ( 'Value [%s] for Node::condition is not valid' ), $condition ) );
		}
		...
		if ($type !== null) {
			switch (strtoupper ( $type )) {
				...
				case 'SECONDARY' :
					...
					break;
				default :
					throw new Exceptions\DomainError ( sprintf ( Lang::translate ( 'Value [%s] for Node::type is not valid' ), $type ) );
			}
		}
		...
		if ($weight !== null) {
			if (is_integer ( $weight )) {
				...
				throw new Exceptions\DomainError ( sprintf ( Lang::translate ( 'Value [%s] for Node::weight must be integer' ), $weight ) );
			}
		}
	...
	public function addNodes() {
		if (count ( $this->nodes ) < 1) {
			throw new Exceptions\MissingValueError ( Lang::translate ( 'Cannot add nodes; no nodes are defined' ) );
		}
	 ...
	 * 
	 * @param string $id
	 *        	either 'public' or 'servicenet' or an ID of an
	 *        	existing IP address
		...
		if ($ipVersion) {
			switch ($ipVersion) {
				...
				case 6 :
					...
					break;
				default :
					throw new Exceptions\DomainError ( sprintf ( Lang::translate ( 'Value [%s] for ipVersion is not valid' ), $ipVersion ) );
			}
		}
