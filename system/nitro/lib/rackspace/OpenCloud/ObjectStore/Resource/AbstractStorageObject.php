 ...
 * resources.
 * Provides support, for example, for metadata-handling and other
...
abstract class AbstractStorageObject extends Base {
	...
	public function name() {
		if (property_exists ( $this, 'name' )) {
			...
			throw new NameError ( sprintf ( 'Name attribute does not exist for [%s]', get_class ( $this ) ) );
		}
	}
	 ...
	 *
	 * @param string $type
	 *        	The type of object; derived from `get_class()` if not
	 *        	specified.
	 *        	@codeCoverageIgnore
	 */
	private function prefix($type = null) {
		...
		switch ($type) {
			...
			case 'DataObject' :
				...
				break;
			default :
				throw new MetadataPrefixError ( sprintf ( 'Unrecognized metadata type [%s]', $type ) );
		}
