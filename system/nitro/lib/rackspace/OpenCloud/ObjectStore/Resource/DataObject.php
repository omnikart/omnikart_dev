...
/**
 * Objects are the basic storage entities in Cloud Files.
 * They represent the
...
class DataObject extends AbstractStorageObject {
	/**
	 * Object name.
	 * The only restriction on object names is that they must be
	...
	/**
	 * Other headers set for this object (e.g.
	 ...
	 * @param string $filename
	 *        	if provided, then the object is loaded from the
	 *        	specified file
	 * @return boolean
	...
	public function create($params = array(), $filename = null, $extractArchive = null) {
		...
		if ($filename) {
			...
			if (! $fp = @fopen ( $filename, 'r' )) {
				throw new Exceptions\IOError ( sprintf ( Lang::translate ( 'Could not open file [%s] for reading' ), $filename ) );
			}
		...
		// perform the request
		$response = $this->getService ()->request ( $this->url () . $extractArchiveUrlArg, 'PUT', $headers, $fp ? $fp : $this->data );
		
		// check the status
		// @codeCoverageIgnoreStart
		if (($status = $response->httpStatus ()) >= 300) {
			throw new Exceptions\CreateUpdateError ( sprintf ( Lang::translate ( 'Problem saving/updating object [%s] HTTP status [%s] response [%s]' ), $this->url () . $extractArchiveUrlArg, $status, $response->httpBody () ) );
		}
		// @codeCoverageIgnoreEnd
	...
	public function updateMetadata($params = array()) {
		...
		if (($stat = $response->httpStatus ()) >= 204) {
			throw new Exceptions\UpdateError ( sprintf ( Lang::translate ( 'Problem updating object [%s] HTTP status [%s] response [%s]' ), $this->url (), $stat, $response->httpBody () ) );
		}
		// @codeCoverageIgnoreEnd
	...
	public function delete($params = array()) {
		...
		if (($stat = $response->httpStatus ()) >= 300) {
			throw new Exceptions\DeleteError ( sprintf ( Lang::translate ( 'Problem deleting object [%s] HTTP status [%s] response [%s]' ), $this->url (), $stat, $response->httpBody () ) );
		}
		// @codeCoverageIgnoreEnd
	...
	public function copy(DataObject $target) {
		...
		$this->getLogger ()->info ( 'Copying object to [{uri}]', array (
				'uri' => $uri 
		) );
		
		$response = $this->getService ()->request ( $this->url (), 'COPY', array (
				'Destination' => $uri 
		) );
	...
	public function tempUrl($secret, $expires, $method) {
		...
		if ($method != 'GET' && $method != 'PUT') {
			throw new Exceptions\TempUrlMethodError ( sprintf ( Lang::translate ( 'Bad method [%s] for TempUrl; only GET or PUT supported' ), $method ) );
		}
	...
	public function saveToFilename($filename) {
		if (! $fp = @fopen ( $filename, "wb" )) {
			throw new Exceptions\IOError ( sprintf ( Lang::translate ( 'Could not open file [%s] for writing' ), $filename ) );
		}
	...
	public function saveToStream($resource) {
		if (! is_resource ( $resource )) {
			throw new Exceptions\ObjectError ( Lang::translate ( "Resource argument not a valid PHP resource." ) );
		}
	...
	public function purgeCDN($email) {
		...
		if ($response->httpStatus () > 204) {
			throw new Exceptions\CdnHttpError ( sprintf ( Lang::translate ( 'Error purging object, status [%d] response [%s]' ), $response->httpStatus (), $response->httpBody () ) );
		}
		// @codeCoverageIgnoreEnd
	 ...
	 * 
	 * @param string $type
	 *        	can be 'streaming', 'ssl', 'ios-streaming',
	...
	private function fetch() {
		...
		$response = $this->getService ()->request ( $this->url (), 'HEAD', array (
				'Accept' => '*/*' 
		) );
	 ...
	 * attributes.
	 * Additional name conversions are done where necessary.
	 *
	 * @param Http $response        	
	 */
	 ...
	 * @return boolean <kbd>TRUE</kbd> if successful
	 * @throws BadContentTypeException @codeCoverageIgnore
	 */
	private function inferContentType($handle) {
		...
		if (function_exists ( "finfo_open" )) {
			...
			if ($finfo) {
				...
				$contentType = is_file ( $filePath ) ? $finfo->file ( $handle ) : $finfo->buffer ( $handle );
