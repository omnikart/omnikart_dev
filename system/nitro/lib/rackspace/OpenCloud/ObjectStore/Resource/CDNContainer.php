...
/**
 * A container that has been CDN-enabled.
 * Each CDN-enabled container has a unique
...
class CDNContainer extends AbstractStorageObject {
	 ...
	 *
	 * @param OpenCloud\ObjectStore $service
	...
	public function create($params = array()) {
		...
		if ($response->httpStatus () > 202) {
			throw new Exceptions\ContainerCreateError ( sprintf ( Lang::translate ( 'Problem creating container [%s] status [%d] response [%s]' ), $this->url (), $response->httpStatus (), $response->httpBody () ) );
		}
	...
	public function update() {
		...
		if ($response->httpStatus () > 204) {
			throw new Exceptions\ContainerCreateError ( sprintf ( Lang::translate ( 'Problem updating container [%s] status [%d] response [%s]' ), $this->Url (), $response->httpStatus (), $response->httpBody () ) );
		}
		// @codeCoverageIgnoreEnd
	...
	public function delete() {
		...
		if ($response->httpStatus () == 404) {
			throw new Exceptions\ContainerNotFoundError ( sprintf ( Lang::translate ( 'Container [%s] not found' ), $this->name ) );
		}
		
		if ($response->httpStatus () == 409) {
			throw new Exceptions\ContainerNotEmptyError ( sprintf ( Lang::translate ( 'Container [%s] must be empty before deleting' ), $this->name ) );
		}
		
		if ($response->httpStatus () >= 300) {
			throw new Exceptions\ContainerDeleteError ( sprintf ( Lang::translate ( 'Problem deleting container [%s] status [%d] response [%s]' ), $this->url (), $response->httpStatus (), $response->httpBody () ) );
			return false;
		}
		// @codeCoverageIgnoreEnd
	...
	public function refresh($name = null, $url = null) {
		$response = $this->getService ()->request ( $this->url ( $name ), 'HEAD', array (
				'Accept' => '*/*' 
		) );
		...
		// @codeCoverageIgnoreStart
		if ($response->HttpStatus () == 404) {
			throw new Exceptions\ContainerNotFoundError ( sprintf ( 'Container [%s] (%s) not found', $this->name, $this->url () ) );
		}
		
		if ($response->HttpStatus () >= 300) {
			throw new Exceptions\HttpError ( sprintf ( 'Error retrieving Container, status [%d] response [%s]', $response->httpStatus (), $response->httpBody () ) );
		}
	...
	public function isValidName($name) {
		if (strlen ( $name ) == 0) {
			throw new Exceptions\ContainerNameError ( 'Container name cannot be blank' );
		}
		
		if (strpos ( $name, '/' ) !== false) {
			throw new Exceptions\ContainerNameError ( 'Container name cannot contain "/"' );
		}
		
		if (strlen ( $name ) > AbstractObjectService::MAX_CONTAINER_NAME_LEN) {
			throw new Exceptions\ContainerNameError ( 'Container name is too long' );
		}
