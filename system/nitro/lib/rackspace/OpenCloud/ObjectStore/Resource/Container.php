 ...
 * You can think of a container as a folder in Windows®
...
class Container extends CDNContainer {
	 ...
	 *
	 * @return s CDNContainer
	 */
	public function getCDN() {
		if (! $this->cdn) {
			throw new Exceptions\CdnNotAvailableError ( Lang::translate ( 'CDN-enabled container is not available' ) );
		}
	 ...
	 * 
	 * @param integer $TTL
	 *        	the Time-To-Live for the CDN container; if NULL,
	...
	public function enableCDN($ttl = null) {
		...
		if ($response->httpStatus () > 202) {
			throw new Exceptions\CdnHttpError ( sprintf ( Lang::translate ( 'HTTP error publishing to CDN, status [%d] response [%s]' ), $response->httpStatus (), $response->httpBody () ) );
		}
	...
	public function disableCDN() {
		...
		if ($response->httpStatus () != 201) {
			throw new Exceptions\CdnHttpError ( sprintf ( Lang::translate ( 'HTTP error disabling CDN, status [%d] response [%s]' ), $response->httpStatus (), $response->httpBody () ) );
		}
		// @codeCoverageIgnoreEnd
	...
	public function createStaticSite($indexHtml) {
		$headers = array (
				'X-Container-Meta-Web-Index' => $indexHtml 
		);
		...
		if ($response->HttpStatus () > 204) {
			throw new Exceptions\ContainerError ( sprintf ( Lang::translate ( 'Error creating static website for [%s], status [%d] response [%s]' ), $this->name, $response->httpStatus (), $response->httpBody () ) );
		}
		// @codeCoverageIgnoreEnd
	...
	public function staticSiteErrorPage($name) {
		$headers = array (
				'X-Container-Meta-Web-Error' => $name 
		);
		$response = $this->getService ()->request ( $this->url (), 'POST', $headers );
		...
		// @codeCoverageIgnoreStart
		if ($response->httpStatus () > 204) {
			throw new Exceptions\ContainerError ( sprintf ( Lang::translate ( 'Error creating static site error page for [%s], status [%d] response [%s]' ), $this->name, $response->httpStatus (), $response->httpBody () ) );
		}
	...
	public function CDNinfo($property = null) {
		...
		if (! isset ( $this->getCDN ()->metadata->Enabled ) || $this->getCDN ()->metadata->Enabled == 'False') {
			return null;
		}
	...
	public function objectList($params = array()) {
		...
		return $this->getService ()->collection ( 'OpenCloud\ObjectStore\Resource\DataObject', $url, $this );
	}
