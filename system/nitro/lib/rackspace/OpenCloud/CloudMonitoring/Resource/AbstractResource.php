 ...
 *
 * @abstract @extends PersistentObject
...
abstract class AbstractResource extends PersistentObject {
	...
	protected function createJson() {
		...
		foreach ( static::$requiredKeys as $requiredKey ) {
			if (! isset ( $object->$requiredKey )) {
				throw new Exceptions\CreateError ( sprintf ( "%s is required to create a new %s", $requiredKey, get_class () ) );
			}
		}
	...
	protected function updateJson($params = array()) {
		foreach ( static::$requiredKeys as $requiredKey ) {
			if (! isset ( $this->$requiredKey )) {
				throw new Exceptions\UpdateError ( sprintf ( "%s is required to create a new %s", $requiredKey, get_class () ) );
			}
		}
	...
	public function update($params = array()) {
		...
		// send the request
		$response = $this->Service ()->Request ( $this->updateUrl (), 'PUT', array (), $json );
