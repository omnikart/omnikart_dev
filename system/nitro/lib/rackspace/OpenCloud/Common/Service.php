...
abstract class Service extends Base {
	 ...
	 *        	- the specified URL from the catalog
	 *        	(e.g., "publicURL")
	 */
	public function __construct(OpenStack $conn, $type, $name, $region, $urltype = RAXSDK_URL_PUBLIC, $customServiceUrl = null) {
	...
	public function collection($class, $url = null, $parent = null) {
		...
		// Save debug info
		$this->getLogger ()->info ( '{class}:Collection({url}, {collectionClass}, {collectionName})', array (
		...
		if ($response->httpStatus () > 204) {
			throw new Exceptions\CollectionError ( sprintf ( Lang::translate ( 'Unable to retrieve [%s] list from [%s], status [%d] response [%s]' ), $class, $url, $response->httpStatus (), $response->httpBody () ) );
		}
			...
			foreach ( $object->links as $link ) {
				if (isset ( $link->rel ) && $link->rel == 'next') {
					if (isset ( $link->href )) {
						...
						$this->getLogger ()->warning ( 'Unexpected [links] found with no [href]' );
					}
				}
			}
		}
		...
		if (! empty ( $nextPageUrl )) {
			$collectionObject->setNextPageCallback ( array (
					$this,
					'Collection' 
			), $nextPageUrl );
		}
	 ...
	 *
	 * @param string $type
	 *        	The OpenStack service type ("compute" or
	...
	private function getEndpoint($type, $name, $region, $urltype = 'publicURL') {
		...
		foreach ( $catalog as $service ) {
			...
			if (! strcasecmp ( $service->type, $type ) && ! strcasecmp ( $service->name, $name )) {
				foreach ( $service->endpoints as $endpoint ) {
					...
					if (isset ( $endpoint->$urltype ) && (! isset ( $endpoint->region ) || ! strcasecmp ( $endpoint->region, $region ))) {
		...
		// error if not found
		if (empty ( $url )) {
			throw new Exceptions\EndpointError ( sprintf ( 'No endpoints for service type [%s], name [%s], region [%s] and urlType [%s]', $type, $name, $region, $urltype ) );
		}
	 ...
	 *
	 * @param string $resource
	 *        	The resource requested; should NOT have slashes
	...
	protected function resolveResourceClass($resourceName) {
		$className = substr_count ( $resourceName, '\\' ) ? $resourceName : $this->getCurrentNamespace () . '\\Resource\\' . ucfirst ( $resourceName );
		
		if (! class_exists ( $className )) {
			throw new Exceptions\UnrecognizedServiceError ( sprintf ( '%s resource does not exist, please try one of the following: %s', $resourceName, implode ( ', ', $this->getResources () ) ) );
		}
