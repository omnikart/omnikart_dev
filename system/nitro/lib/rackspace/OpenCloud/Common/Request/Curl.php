...
/**
 * The CurlRequest class is a simple wrapper to CURL functions.
 * Not only does
...
class Curl extends Base implements HttpRequestInterface {
	...
	public function __construct($url, $method = 'GET', array $options = array()) {
		...
		$this->setOption ( CURLOPT_HEADERFUNCTION, array (
				$this,
				'_get_header_cb' 
		) );
	...
	public function setheaders($array) {
		if (! is_array ( $array )) {
			throw new HttpError ( Lang::translate ( 'Value passed to CurlRequest::setheaders() must be array' ) );
		}
	 ...
	 * @throws OpenCloud\HttpError @codeCoverageIgnore
	 */
