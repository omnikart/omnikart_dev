...
abstract class PersistentObject extends Base {
	 ...
	 *
	 * @param mixed $service
	 *        	The service object for this resource
	 * @param mixed $info
	 ...
	 * requests, etc.
	 * because it has direct access to the Connection.
	 *
	 * @return \OpenCloud\Common\Service
	 */
	public function getService() {
		if (null === $this->service) {
			throw new Exceptions\ServiceValueError ( 'No service defined' );
		}
		return $this->service;
	}
	...
	public function create($params = array()) {
		...
		// send the request
		$response = $this->getService ()->request ( $this->createUrl (), 'POST', array (
		...
		if ($response->httpStatus () > 204) {
			throw new Exceptions\CreateError ( sprintf ( Lang::translate ( 'Error creating [%s] [%s], status [%d] response [%s]' ), get_class ( $this ), $this->Name (), $response->HttpStatus (), $response->HttpBody () ) );
		}
	...
	public function update($params = array()) {
		...
		// send the request
		$response = $this->getService ()->Request ( $this->url (), 'PUT', array (), $json );
		
		// check the return code
		// @codeCoverageIgnoreStart
		if ($response->HttpStatus () > 204) {
			throw new Exceptions\UpdateError ( sprintf ( Lang::translate ( 'Error updating [%s] with [%s], status [%d] response [%s]' ), get_class ( $this ), $json, $response->HttpStatus (), $response->HttpBody () ) );
		}
		// @codeCoverageIgnoreEnd
	...
	public function delete() {
		$this->getLogger ()->info ( '{class}::Delete()', array (
				'class' => get_class ( $this ) 
		) );
		...
		// @codeCoverageIgnoreStart
		if ($response->HttpStatus () > 204) {
			throw new Exceptions\DeleteError ( sprintf ( Lang::translate ( 'Error deleting [%s] [%s], status [%d] response [%s]' ), get_class (), $this->Name (), $response->HttpStatus (), $response->HttpBody () ) );
		}
		// @codeCoverageIgnoreEnd
	 ...
	 * @param array $qstr
	 *        	optional k/v pairs for query strings
	 * @return string
	 ...
	 * @return void
	 */
	public function waitFor($terminal = 'ACTIVE', $timeout = RAXSDK_SERVER_MAXTIMEOUT, $callback = NULL, $sleep = RAXSDK_POLL_INTERVAL) {
	...
	public function refresh($id = null, $url = null) {
		...
		if ($response->HttpStatus () == 404) {
			throw new Exceptions\InstanceNotFound ( sprintf ( Lang::translate ( '%s [%s] not found [%s]' ), get_class ( $this ), $this->$primaryKey, $url ) );
		}
		
		if ($response->HttpStatus () >= 300) {
			throw new Exceptions\UnknownError ( sprintf ( Lang::translate ( 'Unexpected %s error [%d] [%s]' ), get_class ( $this ), $response->HttpStatus (), $response->HttpBody () ) );
		}
		
		// check for empty response
		if (! $response->HttpBody ()) {
			throw new Exceptions\EmptyResponseError ( sprintf ( Lang::translate ( '%s::Refresh() unexpected empty response, URL [%s]' ), get_class ( $this ), $url ) );
		}
		
		// we're ok, reload the response
		if ($json = $response->HttpBody ()) {
			
			$this->getLogger ()->info ( 'refresh() JSON [{json}]', array (
					'json' => $json 
			) );
	 ...
	 * @return s boolean; TRUE if successful, FALSE otherwise
	 */
	protected function action($object) {
		...
		$json = json_encode ( $object );
		$this->getLogger ()->info ( 'JSON [{string}]', array (
				'json' => $json 
		) );
	...
	public function customAction($url, $method = 'GET', $body = null) {
		if (is_string ( $body ) && (json_decode ( $body ) === null)) {
			throw new Exceptions\InvalidArgumentError ( 'Please provide either a well-formed JSON string, or an object ' . 'for JSON serialization' );
		} else {
		...
		if (! is_object ( $response )) {
			throw new Exceptions\HttpError ( sprintf ( Lang::translate ( 'Invalid response for %s::customAction() request' ), get_class ( $this ) ) );
		}
		
		// check for errors
		// @codeCoverageIgnoreStart
		if ($response->HttpStatus () >= 300) {
			throw new Exceptions\ServerActionError ( sprintf ( Lang::translate ( '%s::customAction() [%s] failed; response [%s]' ), get_class ( $this ), $url, $response->HttpBody () ) );
		}
		// @codeCoverageIgnoreEnd
