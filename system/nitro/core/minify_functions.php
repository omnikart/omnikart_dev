...
function optimizeCSS($styles) {
	...
	foreach ( $styles as $hash => $style ) {
		if (empty ( $css_groups [$style ['rel'] . '/nitro_divider/' . $style ['media']] ))
			$css_groups [$style ['rel'] . '/nitro_divider/' . $style ['media']] = array ();
...
function is_url(&$string) {
	...
	if (! $standard_match) {
		...
		// if (stripos($trimmed, 'index.php?') === 0 || stripos($trimmed, 'stylesheet/cssminify1.php?') !== FALSE) {
...
function combine($type, $files, $excludes) {
	...
	if ($includedFiles > 0) {
		return array_merge ( $excludedFiles, array (
				md5 ( $target ) => trim ( $target, '/' ) 
		) );
	} else {
