...
class Bearer implements TokenTypeInterface {
	 ...
	 * be passed to getAccessTokenData().
	 * The constraints specified by the draft are
	 ...
	 * @see http://tools.ietf.org/html/rfc6750#section-2.3 Old Android version bug (at least with version 2.2)
	...
	public function getAccessTokenParameter(RequestInterface $request, ResponseInterface $response) {
		...
		if ($request->request ( $this->config ['token_param_name'] )) {
			...
			if (! in_array ( strtolower ( $request->server ( 'REQUEST_METHOD' ) ), array (
					'post',
					'put' 
			) )) {
