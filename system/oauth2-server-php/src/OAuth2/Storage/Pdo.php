    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    public function setAccessToken($access_token, $client_id, $user_id, $expires, $scope = null)
    {
        ...
        if ($this->getAccessToken($access_token)) {
            $stmt = $this->db->prepare(sprintf('UPDATE %s SET client_id=:client_id, expires=:expires, user_id=:user_id, scope=:scope, session_id=:session_id where access_token=:access_token', $this->config['access_token_table']));
        } else {
            $stmt = $this->db->prepare(sprintf('INSERT INTO %s (access_token, client_id, expires, user_id, scope, session_id) VALUES (:access_token, :client_id, :expires, :user_id, :scope, :session_id)', $this->config['access_token_table']));
        }

        $session_id = session_id();

        return $stmt->execute(compact('access_token', 'client_id', 'user_id', 'expires', 'scope', 'session_id'));
    }
