...
class MsStatistic extends Model {
	public function getSalesByDay($data) {
		...
		USING (order_id)
		WHERE  1 = 1" . (isset ( $data ['seller_id'] ) ? " AND seller_id =  " . ( int ) $data ['seller_id'] : '') . (isset ( $data ['date'] ) ? " AND DATE(date_added) =  DATE('" . $data ['date'] . "')" : '');
	...
	public function getSalesByMonth($data) {
		...
		INNER JOIN `" . DB_PREFIX . "ms_order_product_data` mopd
		USING (order_id)
		WHERE  1 = 1" . (isset ( $data ['seller_id'] ) ? " AND seller_id =  " . ( int ) $data ['seller_id'] : '') . (isset ( $data ['date'] ) ? " AND YEAR(date_added) =  YEAR('" . $data ['date'] . "') AND MONTH(date_added) = MONTH('" . $data ['date'] . "') " : '');
	...
	public function getGrandTotalSales($data) {
		...
		INNER JOIN `" . DB_PREFIX . "ms_order_product_data` mopd
		USING (order_id)
		WHERE  1 = 1" . (isset ( $data ['seller_id'] ) ? " AND seller_id =  " . ( int ) $data ['seller_id'] : '');
	...
	public function getYearsOfOrdersBySeller($seller_id) {
		$sql = "SELECT YEAR(date_added) as year
				...
				INNER JOIN `" . DB_PREFIX . "ms_order_product_data` mopd
				USING (order_id)
				WHERE seller_id = " . ( int ) $seller_id . " GROUP BY YEAR(date_added)
				ORDER BY year DESC";
	...
	public function getSalesByYear($seller_id, $year) {
		$sql = "SELECT SUM(quantity) as sales
				...
				USING ( order_id, product_id )
				WHERE seller_id =  " . ( int ) $seller_id . " AND YEAR( date_added ) = " . ( int ) $year;
	...
	public function getStatsByYears($data, $sort = array()) {
		...
		INNER JOIN `" . DB_PREFIX . "ms_order_product_data` mopd
		USING (order_id)
		WHERE  1 = 1" . (isset ( $data ['seller_id'] ) ? " AND seller_id =  " . ( int ) $data ['seller_id'] : '') . (isset ( $data ['year'] ) ? " AND YEAR(date_added) =  " . ( int ) $data ['year'] : '') . " GROUP BY month" . 
