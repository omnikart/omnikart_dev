...
class MsProduct extends Model {
	...
	public function saveProduct($data) {
		reset ( $data ['languages'] );
		$first = key ( $data ['languages'] );
		...
		foreach ( $data ['languages'] as $language_id => $language ) {
			...
			if (isset ( $language ['product_attributes'] )) {
				foreach ( $language ['product_attributes'] as $attribute_id => $attr ) {
					if (in_array ( $attr ['attribute_type'], array (
							MsAttribute::TYPE_TEXT,
							MsAttribute::TYPE_TEXTAREA,
							MsAttribute::TYPE_DATE,
							MsAttribute::TYPE_DATETIME,
							MsAttribute::TYPE_TIME 
					) )) {
		...
		$sql = "INSERT INTO " . DB_PREFIX . "ms_product
				SET product_id = " . ( int ) $product_id . ",
					...
					product_approved = " . ( int ) $data ['product_approved'] . ((isset ( $data ['list_until'] ) && $data ['list_until'] != NULL) ? ", list_until = '" . $this->db->escape ( $data ['list_until'] ) . "'" : "");
		...
		if (isset ( $data ['product_attributes'] )) {
			foreach ( $data ['product_attributes'] as $attribute_id => $attr ) {
				...
				} else if (in_array ( $attr ['attribute_type'], array (
						MsAttribute::TYPE_TEXT,
						MsAttribute::TYPE_TEXTAREA,
						MsAttribute::TYPE_DATE,
						MsAttribute::TYPE_DATETIME,
		...
		if (isset ( $data ['product_option'] )) {
			foreach ( $data ['product_option'] as $product_option ) {
				// unset sample
				if (isset ( $product_option ['product_option_value'] [0] ))
					unset ( $product_option ['product_option_value'] [0] );
					
					// get type
				$o = $this->MsLoader->MsOption->getOptions ( array (
	...
	public function editProduct($data) {
		...
		$sql = "UPDATE " . DB_PREFIX . "ms_product
				SET product_status = " . ( int ) $data ['product_status'] . ",
					product_approved = " . ( int ) $data ['product_approved'] . ((isset ( $data ['list_until'] ) && $data ['list_until'] != NULL && $data ['list_until'] != 0) ? ", list_until = '" . $this->db->escape ( $data ['list_until'] ) . "'" : " ") . "WHERE product_id = " . ( int ) $product_id;
		$this->db->query ( $sql );
	...
	public function getTotalProducts($data) {
		$sql = "
			...
			LEFT JOIN " . DB_PREFIX . "ms_seller ms
				USING (seller_id)
			WHERE 1 = 1 " . (isset ( $data ['seller_id'] ) ? " AND seller_id =  " . ( int ) $data ['seller_id'] : '') . (isset ( $data ['product_status'] ) ? " AND product_status IN  (" . $this->db->escape ( implode ( ',', $data ['product_status'] ) ) . ")" : '') . (isset ( $data ['enabled'] ) ? " AND status =  " . ( int ) $data ['enabled'] : '');
	...
	public function getProducts($data = array(), $sort = array(), $cols = array()) {
		...
		(isset ( $data ['seller_id'] ) ? " AND ms.seller_id =  " . ( int ) $data ['seller_id'] : '') . (isset ( $data ['language_id'] ) ? " AND pd.language_id =  " . ( int ) $data ['language_id'] : '') . (isset ( $data ['product_status'] ) ? " AND product_status IN  (" . $this->db->escape ( implode ( ',', $data ['product_status'] ) ) . ")" : '') . 
	...
	public function createRecord($product_id, $data = array()) {
		$sql = "INSERT IGNORE INTO " . DB_PREFIX . "ms_product
				SET	product_id =  " . ( int ) $product_id . ",
					product_status = " . ( int ) MsProduct::STATUS_INACTIVE . (isset ( $data ['seller_id'] ) ? ", seller_id =  " . ( int ) $data ['seller_id'] : '');
