	...
	public function getSellerGroups($data = array(), $sort = array(), $cols = array()) {
		...
		$sql = "SELECT 
					...
					*
					...
					WHERE msgd.language_id = '" . ( int ) $this->config->get ( 'config_language_id' ) . "'" . $filters . (isset ( $sort ['order_by'] ) ? " ORDER BY {$sort['order_by']} {$sort['order_way']}" : '') . (isset ( $sort ['limit'] ) ? " LIMIT " . ( int ) $sort ['offset'] . ', ' . ( int ) ($sort ['limit']) : '');
		$res = $this->db->query ( $sql );
