...
final class Ebay {
	...
	public function endItem($item_id) {
		...
		if ($this->config->get ( 'ebay_enditems' ) == 1) {
			...
			return array (
					'error' => true,
					'msg' => 'Settings do not allow you to end items, but the link has been removed . ' 
			);
		}
	}
	...
	private function eBayShippingStatus($item, $txn, $status, $tracking_no = '', $carrier_id = '') {
		...
		return $this->call ( 'order/shippingStatus/', array (
				'item' => $item,
				'txn' => $txn,
				'status' => $status,
				'carrier' => $carrier_id,
	...
	private function eBayPaymentStatus($item, $txn, $status) {
		...
		return $this->call ( 'order/paymentStatus/', array (
				'item' => $item,
				'txn' => $txn,
				'status' => $status 
		) );
	...
	private function getSaleRecord($sale_id) {
		...
		return $this->call ( 'order/getSmpRecord/', array (
				'id' => $sale_id 
		) );
	}
	...
	public function getEbayItemList($limit = 100, $page = 1, $filter = array()) {
		...
		return $this->call ( 'item/getItemListLimited/', array (
				'page' => $page,
				'limit' => $limit,
				'filter' => $filter 
		) );
	...
	public function putStockUpdate($item_id, $stock, $sku = null) {
		...
		$listing = $this->call ( 'item/getItem', array (
				'itemId' => $item_id 
		) );
		...
		if ($listing ['status'] == 1) {
			...
			if ($sku == null) {
				...
				if ($stock <= 0) {
					...
					$this->call ( 'item/reviseStock/', array (
							'itemId' => $item_id,
							'stock' => $stock 
					) );
	...
	public function getProductStockLevel($product_id, $sku = '') {
		...
		if ($sku == '' || $sku == null) {
			...
			return array (
					'quantity' => ( int ) $qry->row ['stock'],
					'status' => ($qry->row ['active']) 
			);
		}
	}
				...
				foreach ( $variants as $option ) {
					if (! empty ( $option ['sku'] )) {
						...
						foreach ( $variant_option_values as $variant_option_value ) {
							$variant_data ['opt'] [$v] ['specifics'] [] = array (
									'name' => $variant_option_value ['option_name'],
	...
	public function getImages() {
		...
		if ($qry->num_rows) {
			foreach ( $qry->rows as $img ) {
				...
				$img_large = str_replace ( array (
						'$_1.JPG',
						'$_01.JPG',
						'$_12.JPG' 
				), '$_57.JPG', $img ['image_original'] );
	...
	public function getEbayListing($item_id) {
		...
		return $this->call ( 'item/getItem/', array (
				'itemId' => $item_id 
		) );
	}
	public function relistItem($item_id, $product_id, $qty) {
		...
		$response = $this->call ( 'listing/relistItem/', array (
				'itemId' => $item_id,
				'qty' => $qty 
		) );
