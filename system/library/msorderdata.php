...
class MsOrderData extends Model {
	...
	public function getOrders($data = array(), $sort = array(), $cols = array()) {
		...
		USING (order_id)
		WHERE seller_id = " . ( int ) $data ['seller_id'] . (isset ( $data ['order_status'] ) && $data ['order_status'] ? " AND o.order_status_id IN  (" . $this->db->escape ( implode ( ',', $data ['order_status'] ) ) . ")" : '') . 
	...
	public function getOrderTotal($order_id, $data) {
		...
		$sql = "SELECT SUM(seller_net_amt) as 'total'
				FROM `" . DB_PREFIX . "ms_order_product_data` mopd
				WHERE order_id = " . ( int ) $order_id . (isset ( $data ['seller_id'] ) ? " AND seller_id =  " . ( int ) $data ['seller_id'] : '');
	...
	public function getOrderData($data = array()) {
		$sql = "SELECT *
				...
				WHERE 1 = 1" . (isset ( $data ['product_id'] ) ? " AND product_id =  " . ( int ) $data ['product_id'] : '') . (isset ( $data ['order_id'] ) ? " AND order_id =  " . ( int ) $data ['order_id'] : '');
	...
	public function getOrderComment($data = array()) {
		$sql = "SELECT *
				...
				WHERE 1 = 1 " . (isset ( $data ['product_id'] ) ? " AND product_id =  " . ( int ) $data ['product_id'] : '') . (isset ( $data ['seller_id'] ) ? " AND seller_id =  " . ( int ) $data ['seller_id'] : '') . (isset ( $data ['order_id'] ) ? " AND order_id =  " . ( int ) $data ['order_id'] : '');
	...
	public function getOrderProducts($data) {
		$sql = "SELECT *
				...
				LEFT JOIN " . DB_PREFIX . "ms_order_product_data
					USING(order_id, product_id)
				WHERE 1 = 1" . (isset ( $data ['order_id'] ) ? " AND order_id =  " . ( int ) $data ['order_id'] : '') . (isset ( $data ['seller_id'] ) ? " AND seller_id =  " . ( int ) $data ['seller_id'] : '');
	...
	public function getTotalSales($data = array()) {
		$sql = "SELECT SUM(quantity) as 'total' FROM (
					...
					INNER JOIN `" . DB_PREFIX . "order` o
						USING(order_id)
					WHERE 1 = 1" . (isset ( $data ['order_id'] ) ? " AND mopd.order_id =  " . ( int ) $data ['order_id'] : '') . (isset ( $data ['seller_id'] ) ? " AND seller_id =  " . ( int ) $data ['seller_id'] : '') . (isset ( $data ['product_id'] ) ? " AND mopd.product_id =  " . ( int ) $data ['product_id'] : '') . (isset ( $data ['period_start'] ) ? " AND DATEDIFF(o.date_added, '{$data['period_start']}') >= 0" : "") . " AND o.order_status_id IN  (" . $this->db->escape ( implode ( ',', $this->config->get ( 'msconf_credit_order_statuses' ) ) ) . ")" . " GROUP BY order_product_id
				) t";
	...
	public function getSuborderHistory($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "ms_suborder_history
				WHERE 1 = 1" . (isset ( $data ['suborder_id'] ) ? " AND suborder_id =  " . ( int ) $data ['suborder_id'] : '');
	...
	public function getSuborders($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "ms_suborder
				WHERE 1 = 1 " . (isset ( $data ['seller_id'] ) ? " AND seller_id =  " . ( int ) $data ['seller_id'] : '') . (isset ( $data ['order_id'] ) ? " AND order_id =  " . ( int ) $data ['order_id'] : '') . (isset ( $data ['suborder_id'] ) ? " AND suborder_id =  " . ( int ) $data ['suborder_id'] : '');
