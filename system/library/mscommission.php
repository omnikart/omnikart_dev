	...
	public function calculateCommission($data) {
		...
		if ($group_commission_id != $default_commission_id) {
			...
			foreach ( $group_commissions as $rate_type => $rate_val ) {
				if (! is_null ( $rate_val ['flat'] ))
					$commissions [$rate_type] ['flat'] = $rate_val ['flat'];
				if (! is_null ( $rate_val ['percent'] ))
					$commissions [$rate_type] ['percent'] = $rate_val ['percent'];
				if (! is_null ( $rate_val ['payment_method'] ))
					$commissions [$rate_type] ['payment_method'] = $rate_val ['payment_method'];
			}
		}
