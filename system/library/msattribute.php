...
class MsAttribute extends Model {
	...
	public function getAttributes($data = array(), $sort = array()) {
		...
		$sql = "SELECT
				...
				LEFT JOIN " . DB_PREFIX . "ms_attribute_description mad
					ON (ma.attribute_id = mad.attribute_id)
				WHERE 1 = 1 " . (isset ( $data ['language_id'] ) ? " AND language_id =  " . ( int ) $data ['language_id'] : "AND language_id =  " . ( int ) $this->config->get ( 'config_language_id' )) . (isset ( $data ['multilang'] ) ? " AND multilang =  " . ( int ) $data ['multilang'] : '') . (isset ( $data ['enabled'] ) ? " AND enabled =  " . ( int ) $data ['enabled'] : '') . $filters . (isset ( $sort ['order_by'] ) ? " ORDER BY {$sort['order_by']} {$sort['order_way']}" : '') . (isset ( $sort ['limit'] ) ? " LIMIT " . ( int ) $sort ['offset'] . ', ' . ( int ) ($sort ['limit']) : '');
	...
	public function getAttributeValue($attribute_value_id) {
		$sql = "SELECT *
				...
				WHERE mav.attribute_value_id = " . ( int ) $attribute_value_id . (isset ( $data ['language_id'] ) ? " AND mavd.language_id =  " . ( int ) $data ['language_id'] : " AND mavd.language_id =  " . ( int ) $this->config->get ( 'config_language_id' ));
	...
	public function getAttributeValues($attribute_id) {
		...
		$sql = "SELECT *
				...
				LEFT JOIN " . DB_PREFIX . "ms_attribute_value_description mavd
					ON (mav.attribute_value_id = mavd.attribute_value_id)
				WHERE mav.attribute_id = " . ( int ) $attribute_id . (isset ( $data ['language_id'] ) ? " AND mavd.language_id =  " . ( int ) $data ['language_id'] : " AND mavd.language_id =  " . ( int ) $this->config->get ( 'config_language_id' )) . "
	...
	public function getAttributeValueDescriptions($attribute_value_id) {
		...
		foreach ( $attribute_value_query->rows as $attribute_value ) {
			$attribute_value_data [$attribute_value ['language_id']] = array (
					'name' => $attribute_value ['name'] 
			);
		}
	...
	public function getProductAttributes($product_id, $data = array()) {
		...
		$sql = "SELECT *,
				...
				WHERE mpa.product_id = '" . ( int ) $product_id . "'" . (isset ( $data ['mad.language_id'] ) ? " AND mad.language_id =  " . ( int ) $data ['mad.language_id'] : " AND mad.language_id =  " . ( int ) $this->config->get ( 'config_language_id' )) . (isset ( $data ['mavd.language_id'] ) ? " AND mavd.language_id =  " . ( int ) $data ['mavd.language_id'] : " AND mavd.language_id =  " . ( int ) $this->config->get ( 'config_language_id' )) . (isset ( $data ['multilang'] ) ? " AND multilang =  " . ( int ) $data ['multilang'] : '') . (isset ( $data ['enabled'] ) ? " AND enabled =  " . ( int ) $data ['enabled'] : '') . (isset ( $data ['attribute_type'] ) ? " AND attribute_type IN  (" . $this->db->escape ( implode ( ',', $data ['attribute_type'] ) ) . ")" : '') . "
	...
	public function getProductAttributeValues($product_id, $data = array()) {
		...
		$sql = "SELECT *,
				...
				LEFT JOIN " . DB_PREFIX . "ms_attribute_value_description mavd
					ON (mpa.attribute_value_id = mavd.attribute_value_id)
				WHERE mpa.product_id = '" . ( int ) $product_id . "'" . (isset ( $data ['multilang'] ) ? " AND multilang =  " . ( int ) $data ['multilang'] : '') . (isset ( $data ['attribute_type'] ) ? " AND attribute_type IN  (" . $this->db->escape ( implode ( ',', $data ['attribute_type'] ) ) . ")" : '') . "
				ORDER BY ma.sort_order ASC";
