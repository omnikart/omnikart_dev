...
class MsBalance extends Model {
	...
	public function getSellerBalanceEntries($seller_id, $sort) {
		$sql = "SELECT *
				...
				ORDER BY {$sort['order_by']} {$sort['order_way']}" . ($sort ['limit'] ? " LIMIT " . ( int ) (($sort ['page'] - 1) * $sort ['limit']) . ', ' . ( int ) ($sort ['limit']) : '');
	...
	public function getBalanceEntries($data = array(), $sort = array()) {
		...
		$sql = "SELECT
				...
				WHERE 1 = 1" . (isset ( $data ['order_id'] ) ? " AND mb.order_id =  " . ( int ) $data ['order_id'] : '') . (isset ( $data ['product_id'] ) ? " AND mb.product_id =  " . ( int ) $data ['product_id'] : '') . (isset ( $data ['seller_id'] ) ? " AND mb.seller_id =  " . ( int ) $data ['seller_id'] : '') . (isset ( $data ['withdrawal_id'] ) ? " AND seller_id =  " . ( int ) $data ['withdrawal_id'] : '') . (isset ( $data ['balance_type'] ) ? " AND balance_type =  " . ( int ) $data ['balance_type'] : '') . $filters . (isset ( $sort ['order_by'] ) ? " ORDER BY {$sort['order_by']} {$sort['order_way']}" : '') . (isset ( $sort ['limit'] ) ? " LIMIT " . ( int ) $sort ['offset'] . ', ' . ( int ) ($sort ['limit']) : '');
		$res = $this->db->query ( $sql );
	...
	public function getBalanceEntry($data) {
		$sql = "SELECT *
				FROM " . DB_PREFIX . "ms_balance mb
				WHERE 1 = 1 " . (isset ( $data ['order_id'] ) ? " AND order_id =  " . ( int ) $data ['order_id'] : '') . (isset ( $data ['product_id'] ) ? " AND product_id =  " . ( int ) $data ['product_id'] : '') . (isset ( $data ['seller_id'] ) ? " AND seller_id =  " . ( int ) $data ['seller_id'] : '') . (isset ( $data ['withdrawal_id'] ) ? " AND seller_id =  " . ( int ) $data ['withdrawal_id'] : '') . (isset ( $data ['balance_type'] ) ? " AND balance_type =  " . ( int ) $data ['balance_type'] : '') . " LIMIT 1";
					...
					(SELECT SUM(balance) FROM " . DB_PREFIX . "ms_balance
					 WHERE balance_id IN (
						...
						LEFT JOIN " . DB_PREFIX . "ms_seller
							USING(seller_id)" . (isset ( $data ['seller_status'] ) ? " WHERE seller_status IN  (" . $this->db->escape ( implode ( ',', $data ['seller_status'] ) ) . ")" : '') . " GROUP BY seller_id
					)),
					0
				) as total";
		$res = $this->db->query ( $sql );
