...
class ModelMenuWidget extends Model {
	...
	protected function getLayout($layout) {
		if (file_exists ( DIR_TEMPLATE . $this->config->get ( 'config_template' ) . '/template/module/pavmegamenu/widgets/' . $layout . '.tpl' )) {
			$template = DIR_TEMPLATE . $this->config->get ( 'config_template' ) . '/template/module/pavmegamenu/widgets/' . $layout . '.tpl';
		} else {
			$template = DIR_TEMPLATE . 'default/template/module/pavbuilder/widgets/' . $layout . '.tpl';
		}
		return $template;
	}
	...
	public function renderButton($id) {
		...
		if (isset ( $this->widgets [$id] )) {
			...
			if (file_exists ( $f )) {
				...
				$cb_args = array ();
				$info = call_user_func_array ( array (
						$class,
						'getWidgetInfo' 
				), $cb_args );
