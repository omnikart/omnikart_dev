...
class ModelModuleExcelport extends Model {
	...
	/* {EXTRA_FUNCTIONS} */
	// Important hook! Do not delete.
	public function __construct($register) {
		if (! defined ( 'IMODULE_ROOT' ))
			define ( 'IMODULE_ROOT', substr ( DIR_APPLICATION, 0, strrpos ( DIR_APPLICATION, '/', - 2 ) ) . '/' );
		if (! defined ( 'IMODULE_ADMIN_ROOT' ))
			define ( 'IMODULE_ADMIN_ROOT', DIR_APPLICATION );
		if (! defined ( 'IMODULE_SERVER_NAME' ))
			define ( 'IMODULE_SERVER_NAME', substr ( (defined ( 'DIR_APPLICATION' ) ? DIR_APPLICATION : HTTP_SERVER), 7, strlen ( (defined ( 'DIR_APPLICATION' ) ? DIR_APPLICATION : HTTP_SERVER) ) - 8 ) );
		if (! defined ( 'IMODULE_TEMP_FOLDER' ))
			define ( 'IMODULE_TEMP_FOLDER', 'system/cache/temp_excelport' );
		if (! defined ( 'IMODULE_UPMOST_VERSION' ))
			define ( 'IMODULE_UPMOST_VERSION', '2.99' );
	...
	public function folderCheck($folder) {
		if (! is_dir ( $folder )) {
			if (! mkdir ( $folder, 0755 ))
				throw new Exception ( 'Error: Temp directory does not exist and cannot be created! Please check the root folder permissions.' );
		}
	}
	...
	public function cleanTemp($tempDir = 'system/cache/temp_excelport', $exclude = array()) {
		...
		foreach ( $files as $file ) {
			if (! in_array ( $file, array_merge ( array (
					'.',
					'..',
					'index.html' 
			), $exclude ) )) {
	...
	public function getStandardFile($file) {
		...
		$result = false;
		if ($file ['size'] <= $this->returnMaxUploadSize () && in_array ( $extension, $allowedExts )) { // file limit = post_max_size - 512KB
			if ($file ['error'] > 0)
				throw new Exception ( "Upload Error Code: " . $file ['error'] );
			$dest = IMODULE_ROOT . IMODULE_TEMP_FOLDER . '/' . $this->customer->getId () . '/' . $name;
			if (! move_uploaded_file ( $file ['tmp_name'], $dest ))
				throw new Exception ( $this->language->get ( 'excelport_unable_upload' ) );
			else
				$result = $dest;
		} else
			throw new Exception ( $this->language->get ( 'excelport_invalid_file' ) );
	...
	public function returnMaxUploadSize($readable = false) {
		...
		if ($upload >= $post)
			return $readable ? $this->sizeToString ( $post - 524288 ) : $post - 524288;
		else
			return $readable ? $this->sizeToString ( $upload ) : $upload;
	}
	...
	public function createDownload($file, $die = true) {
		$this->cleanTemp ( IMODULE_TEMP_FOLDER . '/' . $this->customer->getId (), array (
				basename ( $file ) 
		) );
		
		if (stripos ( $this->request->server ['REQUEST_URI'], 'com_mijoshop' ) !== FALSE) {
			header ( "Location:" . dirname ( HTTP_SERVER ) . "/components/com_mijoshop/opencart/" . IMODULE_TEMP_FOLDER . "/" . $this->customer->getId () . "/" . basename ( $file ) );
		} elseif (stripos ( $this->request->server ['REQUEST_URI'], 'com_opencart' ) !== FALSE) {
			header ( "Location:" . dirname ( HTTP_SERVER ) . "/components/com_opencart/" . IMODULE_TEMP_FOLDER . "/" . $this->customer->getId () . "/" . basename ( $file ) );
		} else {
			header ( "Location:" . HTTP_SERVER . IMODULE_TEMP_FOLDER . "/" . $this->customer->getId () . "/" . basename ( $file ) );
		}
	...
	public function getProgress($error = NULL) {
		...
		if (! empty ( $error )) {
			...
			if (file_exists ( IMODULE_ROOT . IMODULE_TEMP_FOLDER . '/' . $this->customer->getId () . '/' . $this->get_progress_name () )) {
				$data = file_get_contents ( IMODULE_ROOT . IMODULE_TEMP_FOLDER . '/' . $this->customer->getId () . '/' . $this->get_progress_name () );
				$result = json_decode ( $data, true );
			} else {
				$result ['populateAll'] = true;
				file_put_contents ( IMODULE_ROOT . IMODULE_TEMP_FOLDER . '/' . $this->customer->getId () . '/' . $this->get_progress_name (), json_encode ( $result ) );
			}
		}
		clearstatcache ();
		return $result;
	...
	public function setProgress($progress) {
		...
		file_put_contents ( IMODULE_ROOT . IMODULE_TEMP_FOLDER . '/' . $this->customer->getId () . '/' . $this->get_progress_name (), json_encode ( $progress ) );
		clearstatcache ();
	}
	public function deleteProgress() {
		if (file_exists ( IMODULE_ROOT . IMODULE_TEMP_FOLDER . '/' . $this->customer->getId () . '/' . $this->get_progress_name () ))
			unlink ( IMODULE_ROOT . IMODULE_TEMP_FOLDER . '/' . $this->get_progress_name () );
		clearstatcache ();
	}
	...
	public function prepareUploadedFile($file) {
		...
		if (! file_exists ( $file ))
			throw new Exception ( 'excelport_invalid_import_file' );
		...
		switch ($ext) {
			...
			case 'zip' :
				{
					return $this->unzip ( $file, IMODULE_ROOT . IMODULE_TEMP_FOLDER . '/' . $this->customer->getId () . '/' . $info ['filename'] . '/' );
				}
				break;
		}
	}
	public function unzip($file, $decompressFolder) {
		...
		if ($zip->open ( $file, ZIPARCHIVE::CREATE ) !== true)
			throw new Exception ( $this->language->get ( 'excelport_unable_zip_file_open' ) );
		
		if (! file_exists ( $decompressFolder ) || (file_exists ( $decompressFolder ) && ! is_dir ( $decompressFolder ))) {
			if (! mkdir ( $decompressFolder, 0755 ))
				throw new Exception ( $this->language->get ( 'excelport_unable_create_unzip_folder' ) );
		}
		
		if (! $zip->extractTo ( $decompressFolder ))
			throw new Exception ( $this->language->get ( 'excelport_unable_zip_file_extract' ) );
			
			// check the files
		$files = scandir ( $decompressFolder );
		
		foreach ( $files as $tempFile ) {
			if (in_array ( $tempFile, array (
					'.',
					'..' 
			) ))
				continue;
			if (! file_exists ( $decompressFolder . $tempFile ) || is_dir ( $decompressFolder . $tempFile ))
				continue;
	...
	public function setData($type, $destinationDirectory, $language_id) {
		foreach ( $this->exportData [$type] as $name => $query ) {
			...
			switch ($type) {
				case 'Products' :
					{
						if (version_compare ( VERSION, '1.5.3.1', '>' ) && $name == 'tag')
							continue 2;
						if (version_compare ( VERSION, '1.5.5', '<' ) && $name == 'filters')
							continue 2;
						if (version_compare ( VERSION, '1.5.6', '<' ) && $name == 'recurrings')
							continue 2;
					}
					break;
				case 'Categories' :
	...
	public function getTabs() {
		$dir = IMODULE_ADMIN_ROOT . 'view' . DIRECTORY_SEPARATOR . 'template' . DIRECTORY_SEPARATOR . 'module' . DIRECTORY_SEPARATOR . 'excelport' . DIRECTORY_SEPARATOR;
		...
		foreach ( $files as $file ) {
			if (! in_array ( $file, array_keys ( $name_map ) ))
				continue;
	...
	public function importXLS($type, $language, $file = '', $settings, $addAsNew = false) {
		$this->language->load ( 'module/excelport' );
		if (! file_exists ( $file ))
			throw new Exception ( $this->language->get ( 'excelport_file_not_exists' ) );
	...
	public function exportXLS($type, $language, $store, $destinationFolder = '', $settings, $quickExport = false, $filter = false, $filters = array()) {
		...
		$valid = false;
		if (version_compare ( VERSION, '1.5.1.3', '>=' ) && version_compare ( VERSION, IMODULE_UPMOST_VERSION, '<=' ))
			$valid = true;
		if (! $valid)
			throw new Exception ( str_replace ( '{VERSION}', '1.5.1.3 - ' . IMODULE_UPMOST_VERSION, $this->language->get ( 'text_feature_unsupported' ) ) );
		
		$this->load->model ( 'module/excelport_product' );
		
		if (! is_numeric ( $settings ['ExportLimit'] ) || $settings ['ExportLimit'] < 50 || $settings ['ExportLimit'] > 800)
			throw new Exception ( $this->language->get ( 'excelport_export_limit_invalid' ) );
