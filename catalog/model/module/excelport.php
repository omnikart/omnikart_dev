...
class ModelModuleExcelport extends Model {
	...
	public function __construct($register) {
		...
		if (!defined('IMODULE_SERVER_NAME')) define('IMODULE_SERVER_NAME', substr((defined('DIR_APPLICATION') ? DIR_APPLICATION : HTTP_SERVER), 7, strlen((defined('DIR_APPLICATION') ? DIR_APPLICATION : HTTP_SERVER)) - 8));
	...
	public function getStandardFile($file) {
		...
		if ($file['size'] <= $this->returnMaxUploadSize() && in_array($extension, $allowedExts)) { //file limit = post_max_size - 512KB
			if ($file['error'] > 0) throw new Exception("Upload Error Code: " . $file['error']);
			$dest = IMODULE_ROOT . IMODULE_TEMP_FOLDER .'/'. $this->customer->getId() . '/' . $name;
			if (!move_uploaded_file($file['tmp_name'], $dest)) throw new Exception($this->language->get('excelport_unable_upload'));
			else $result = $dest;
	...
	public function createDownload($file, $die = true) {
		$this->cleanTemp(IMODULE_TEMP_FOLDER .'/'. $this->customer->getId(), array(basename($file)));
	...
	public function deleteProgress() {
		if (file_exists(IMODULE_ROOT . IMODULE_TEMP_FOLDER . '/'. $this->customer->getId() . '/' . $this->get_progress_name())) unlink(IMODULE_ROOT . IMODULE_TEMP_FOLDER . '/' . $this->get_progress_name());
		clearstatcache();	
	}
