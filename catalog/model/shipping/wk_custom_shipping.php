		...
		if ($this->config->get ( 'wk_custom_shipping_method' ) == 'flat') {
			foreach ( $seller as $pro_det ) {
				...
				if ($pro_det ['seller'] == 'Admin') {
					$result_csv [] = array (
							'shipping_price' => $this->config->get ( 'wk_custom_shipping_admin_flatrate' ) 
					);
				} else {
				...
				if ($res_error) {
					...
					if ($this->config->get ( 'wk_custom_shipping_error_msg' ) != '') {
						...
						$error = $error . 'For Shipping method "' . $this->config->get ( 'wk_custom_shipping_title' ) . '" : ' . $pro_det ['name'] . " Vendor does not provide services for zip : " . $zipcod . ', so this method is not selectable.';
					}
				}
			}
		}
