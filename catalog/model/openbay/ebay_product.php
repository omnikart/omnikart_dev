...
class ModelOpenbayEbayProduct extends Model {
	public function getRelistRule($id) {
		return $this->openbay->ebay->call ( 'item/getAutomationRule', array (
				'id' => $id 
		) );
	}
	...
	private function getOption($name) {
		...
		if ($qry->num_rows) {
			return array (
					'id' => ( int ) $qry->row ['option_id'],
					'sort' => ( int ) $qry->row ['sort_order'] 
			);
		} else {
	...
	private function createOption($name) {
		...
		return array (
				'id' => ( int ) $option_id,
				'sort' => ( int ) $qry_sort->row ['sort_order'] 
		);
	}
	private function getOptionValue($name, $option_id) {
		...
		if ($qry->num_rows) {
			return array (
					'id' => ( int ) $qry->row ['option_value_id'],
					'sort' => ( int ) $qry->row ['sort_order'] 
			);
		} else {
