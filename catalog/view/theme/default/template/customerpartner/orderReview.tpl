<?php echo $header; ?><div id="columns">
  ...
  <?php if ($product_warning) { ?>
    <div class="alert alert-danger">
			<i class="fa fa-exclamation-circle"></i><?php echo $product_warning; ?></div>
  <?php } ?>
  <?php if ($success) { ?>
    ...
    <div id="content" class="<?php echo $class; ?>">
      ...
      <h1>
        ...
        <?php if(isset($forReview) && $forReview) { ?>
          <form action="<?php echo $review; ?>" method="post"
						enctype="multipart/form-data" id="form-order-review">
                  ...
                  <?php if($productsToReview) { ?>
                    <?php foreach ($productsToReview as $key => $product) { ?>
                      <tr>
										<td><input type="checkbox"
											name="selected[<?php echo $product['key']; ?>]"
                      ...
                      <tr>
									...
									<td class="text-center">
										<button class="btn btn-warning" type="button"
											data-toggle="tooltip"
											data-original-title="<?php echo $entry_toottip_getproductlist; ?>"
											value="<?php echo $request['customer_id']; ?>"
											name="cartGetter">
        ...
        <?php } else if($isSubUser) { ?>
          <div class="row">
						<h3 class="text-warning">You have already submitted products to
							get reviewed. Let them approve and then send request for review.</h3>
					</div>
        <?php } ?>
      </fieldset>
				<div id="productModal" class="modal fade bs-example-modal-sm"
					...
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="modal-body">
								...
								<form action="<?php echo $approveAction; ?>" method="POST"
							...
							<div class="modal-footer">
								<button class="btn btn-primary" type="button"
									...
									onclick="$('#approve-form').attr('action','<?php echo $approveAction; ?>');$('#approve-form').submit();">
                <?php echo $button_approve; ?>
              </button>
								<button class="btn btn-primary" type="submit"
									data-dismiss="modal" aria-hidden="true"
									onclick="$('#approve-form').attr('action','<?php echo $disapproveAction; ?>');$('#approve-form').submit();">
									Disapprove</button>
