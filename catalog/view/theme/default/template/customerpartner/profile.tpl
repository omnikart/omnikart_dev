...
.row #content .text-white{
  color: <?php echo $partner['backgroundcolor'] ? $partner['backgroundcolor'] : '#ffffff'; ?>;
}
.seller_name{
  position: absolute;
  bottom: 0px;
...
.banner-data{
  <?php if($partner['companylogo']) { ?>
    background:<?php echo $partner["companylogo"]; ?>;
  ...
  <div class="row">
    <div id="content" class="<?php echo $class; ?>">
      <div class="banner">
        ...
        <div class="text-center banner-data <?php echo $banner_class; ?>">
          <?php if($partner['companylogo']){ ?>
            <div class="img-banner-div">
              <img src="<?php echo $partner['companylogo']; ?>" class="img-responsive img-circle"/>
            </div>
          <?php }else{ ?>
            <br/>
          <?php } ?>
          ...
          <div class="rating">
            <?php for ($i = 1; $i <= 5; $i++) { ?>
              ...
              <span class="fa fa-stack"><i class="fa fa-star-o fa-stack-2x <?php echo $banner_class; ?>"></i></span>
              <?php } else { ?>
          ...
          <div class="seller-social-media">
            <?php if($partner['facebookid']){ ?>
              <a href="http://facebook.com/<?php echo $partner['facebookid'];?>" target="_blank"> <i class="fa fa-facebook-square"></i></a>
            <?php } ?>
            <?php if($partner['twitterid']){ ?>
              <a href="http://twitter.com/<?php echo $partner['twitterid'];?>" target="_blank"> <i class="fa fa-twitter-square"></i></a>
            <?php } ?>
            <?php if($logged){ ?>
            <a data-toggle="modal" data-target="#myModal-seller-mail"><i class="fa fa-envelope"></i></a>
            <?php } else { ?>
            <a href="<?php echo $login; ?>" data-toggle="tooltip" data-original-title="Login for contact"><i class="fa fa-envelope"></i></a>
            <?php } ?>
          </div>
      ...
      <div class="img-div">
        <?php if($partner['avatar']) { ?>
    		  <img src="<?php echo $partner['avatar']; ?>" class="img-responsive"/>
        <?php } ?>
    	</div> 
      <div class="clear-fix"></div>
      <div class="mp-tabs addbgtome">
        <ul class="nav nav-tabs">
          <li class="active"><a href="#tab-profile" data-toggle="tab"><i class="fa fa-user"></i> <?php echo $text_profile; ?></a></li>       
          ...
          <?php if(isset($public_seller_profile) && in_array('review',$public_seller_profile)) { ?>
            <li><a href="#tab-reviews" data-toggle="tab"><i class="fa fa-comment-o"></i> <?php echo $text_reviews.' ('.($feedback_total ? $feedback_total : '0').')'; ?></a></li>
          <?php } ?>
          <?php if(isset($public_seller_profile) && in_array('productReview',$public_seller_profile)) { ?>
            <li><a href="#tab-product-reviews" data-toggle="tab"><i class="fa fa-comments-o"></i> <?php echo $text_product_reviews.' ('.($product_feedback_total ? $product_feedback_total : '0').')'; ?></a></li>
          <?php } ?>
          <?php if(isset($public_seller_profile) && in_array('location',$public_seller_profile)) { ?>
            <li><a href="#tab-location" data-toggle="tab"><i class="fa fa-globe"></i></span> <?php echo $text_location; ?></a></li>
          <?php } ?>
        </ul>
      </div>
            ...
            <form class="form-horizontal">
                ...
                <div class="buttons">
                  <div class="pull-right">
                    <button type="button" id="button-feedback" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary"><?php echo $button_continue; ?></button>
                  </div>
                </div>
            </form>
          <?php } else { ?>
            <a href="<?php echo $login; ?>" class="btn btn-primary" target="_blank"><?php echo $text_login; ?></a>
          <?php } ?>
        </div>
        ...
        <form id="seller-mail-form">
          <div class="modal-body">
            <div class="form-group required">
              ...
              <input type="text" name="message" id="input-subject" class="form-control" />
