<div class="hover-content">
	<?php if (isset($dbe) && $dbe) { ?>
		...
		<?php } else {?>
			<div>This Product Contains more variations. Please click on the	image below.</div>
		<?php } ?>
	<?php } ?>
</div>
<div class="image">
...
<div>
	       ...
	       <div class="price type-<?php echo $product['type']?>">
	          <?php if (!$product['special']) { ?>
					<?php if ($product['discount']) { ?>
	      				<span style="text-decoration: line-through; color: #aaa;"><?php echo $product['original_price']; ?></span>
	      				<span class="discount">
	      					<span class="text"><?php echo $product['discount']; ?>% Off</span>
	      					<span class="img"></span>
	      				</span>
	              	<?php } ?>
	...
	<?php } else { ?>
	  	<div class="cart-button">
			<a class="btn btn-default btn-block" type="button"	href="<?php echo $product['href']; ?>"><i class="fa fa-shopping-cart"></i>
				<span class="hidden-xs hidden-sm hidden-md">Add to Quotation</span>
			</a>
		</div>
	<?php } ?>
              <!-- hello test change -->
