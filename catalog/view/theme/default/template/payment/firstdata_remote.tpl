<form class="form-horizontal" id="payment_form">
    ...
    <div class="form-group">
        ...
        <?php foreach ($stored_cards as $card) { ?>
          <p>
					<input type="radio" name="cc_choice"
						value="<?php echo $card['token']; ?>" class="stored_card"
						<?php echo $i == 0 ? 'checked="checked"' : ''; ?> /> <?php echo $card['card_type'] . ' xxxx ' . $card['digits'] . ', ' . $entry_cc_expire_date . ' ' . $card['expire_month'] . '/' . $card['expire_year']; ?></p>
          <?php $i++; ?>
        <?php } ?>

        <p>
					<input type="radio" name="cc_choice" value="new"
						class="stored_card" />New card
				</p>
			</div>
		</div>
    <?php } ?>
    ...
    <div id="card_info" style="display: none;">
			<div class="form-group required">
				...
				<div class="col-sm-10">
					<input type="text" name="cc_name" value=""
						placeholder="<?php echo $entry_cc_name; ?>" id="input-cc-name"
						class="form-control" />
				</div>
			</div>
			<div class="form-group required">
      ...
      <div class="form-group">
				<label class="col-sm-2 control-label" for="input-cc-cvv2">Store card
					details?</label>

				<div class="col-sm-10">
					<input type="hidden" name="cc_store" value="0" /> <input
						type="checkbox" name="cc_store" value="1" checked />
				</div>
			</div>
      <?php } ?>
    </div>
