      ...
      <h1>
        ...
        <div class="pull-right">
						<a href="<?php echo $insert; ?>" data-toggle="tooltip"
							...
							onclick="$('#form-product').submit();" data-toggle="tooltip"
                ...
                <?php if($userlist) { ?>
                  <?php foreach ($userlist as $key => $list) { ?>
                    <tr>
										<td width="1"><input type="checkbox" name="selected[]"
											value="<?php echo $list['customer_id']; ?>"></td>
										<td class="text-left"><?php echo $list['firstname']." ".$list['lastname']; ?></td>
										<td class="text-left"><?php echo $list['email']; ?></td>
										<td class="text-left text-success"><?php
																		
$s = '';
																		foreach ( $list ['customerRights'] ['rights'] as $key => $value ) {
																			$s .= $value . ", ";
																		}
																		$s = rtrim ( $s, ',' );
																		echo $s;
										...
										<td class="text-center"><input type="hidden"
											id="cg-<?php echo $list['customer_id']; ?>"
											name="customer_group_id"
											value="<?php echo $list['customer_group_id']; ?>"> <input
											type="hidden" id="mi-<?php echo $list['customer_id']; ?>"
											name="manager_id" value="<?php echo $list['manager_id']; ?>">
											<input type="hidden"
											id="pl-<?php echo $list['customer_id']; ?>" name="p_limit"
											value="<?php echo $list['p_limit']; ?>">
											<button class="btn btn-primary" name="alter_customer_group"
												type="button" value="<?php echo $list['customer_id']; ?>"
												data-toggle="modal" data-target="#customergroup">

												<i class="fa fa-pencil"></i>
											</button>
                        <?php if($list['status'] == '1') { ?>
                          <a class="btn btn-danger changeStatusSubUser"
											data-toggle="tooltip"
											data-original-title="<?php echo $button_disable; ?>"
											data-value="disable-<?php echo $list['customer_id']; ?>"> <i
												class="fa fa-thumbs-o-down"></i>
										</a>
                        <?php } else { ?>
                          <a class="btn btn-success changeStatusSubUser"
											data-toggle="tooltip"
											data-original-title="<?php echo $button_enable; ?>"
											data-value="enable-<?php echo $list['customer_id']; ?>"> <i
				...
				<div id="customergroup" class="modal fade bs-example-modal-sm"
							...
							<div class="modal-body">
								<form class="form-horizontal">
									<div class="form-group required">
										<label class="col-sm-2 control-label" for="customer_group">Customer
											Group</label>
										<div class="col-sm-10">
                    ...
                    <?php if($customer_groups) { ?>
                      <?php foreach ($customer_groups as $key => $customer_group) { ?>
                        <option
													value="<?php echo $customer_group['id']; ?>"><?php echo $customer_group['name']; ?></option>
                      <?php } ?>
                    <?php } ?>
                  </select>
										</div>
                    ...
                    <?php if($allusers) { ?>
                      <?php foreach ($allusers as $key => $user) { ?>
                        <option
													value="<?php echo $user['customer_id']; ?>"><?php echo $user['firstname'].' '.$user['lastname']; ?></option>
                      <?php } ?>
                    <?php } ?>
                  </select>
										</div>
									...
									<div class="form-group required">
										<label class="col-sm-2 control-label" for="p_limit">Purchasing
											Limit</label>
										<div class="col-sm-10">
											<input type="text" name="p_limit" value=""
												placeholder="Limit" id="p_limit" class="form-control">
										</div>
									</div>
							...
							<div class="modal-footer">
								<button class="btn btn-primary" type="button"
									data-dismiss="modal" aria-hidden="true" name="update_button">
                <?php echo $button_update; ?>
              </button>
								<button class="btn btn-danger" data-dismiss="modal"
									aria-hidden="true" id="closeButton">
