    ...
    <h1>
      <?php echo $heading_title; ?>
	  	<div class="pull-right">
	        ...
	        <?php } ?>
	       	<a
							href="index.php?route=account/customerpartner/excelport/ajaxgenerate"
							class="btn btn-primary"> <span data-toggle="tooltip"
							title="Excel Port Products Download product database in Excel format, Edit is and upload the updated workbook">
      ...
      <div class="well">
							...
							<div class="col-sm-4">
								<div class="form-group">
									...
									<div class='input-group'>
										<input type="text" name="filter_model"
											...
											placeholder="<?php echo $column_model; ?>" id="input-model"
											class="form-control" /> <span class="input-group-addon"><span
											class="fa fa-angle-double-down"></span></span>
									</div>
								</div>
								<div class="form-group">
									...
									<input type="text" name="filter_quantity"
										...
										placeholder="<?php echo $column_quantity; ?>" id="input-model"
										class="form-control" />
								</div>
							</div>
                ...
                <?php } ?>
					...
					<form action="<?php echo $delete; ?>" method="post"
						...
						<div class="table-responsive">
							<table class="table table-bordered table-hover" id="product-list">
								<thead>
									<tr>
										<td width="1" style="text-align: center;"><input
											type="checkbox"
                  ...
                  <a href="<?php echo $sort_name; ?>"
											class="<?php echo strtolower($order); ?>"><?php echo str_replace(' ', '', $column_name); ?></a>
                  <?php } else { ?>
                  ...
                  <?php } ?></td>
										<td class="text-left" style="min-width: 110px"
											data-toggle="tooltip"
											data-original-title="Please mention price for one/minimum quantity of product. If you offer quantity based discount please edit the product using the button in an action and edit 'discounts' tab"><?php if ($sort == 'p.price') { ?>
                  ...
                  <?php } ?></td>
										<td class="text-right" style="min-width: 90px"
											data-toggle="tooltip"
											data-original-title="Minimum Order Quantity">(MOQ)</td>
										<td class="text-right" style="min-width: 100px"
											data-toggle="tooltip" title=""
											data-original-title="Status shown when a product is out of stock">Availability</td>
              ...
              <tr>
                  ...
                  <?php } ?>
                  <input type="hidden"
											name="products[<?php echo $product['product_id']; ?>][id]"
										...
										<td class="text-left"><input type="text" class="form-control"
											name="products[<?php echo $product['product_id']; ?>][price]"
											value="<?php echo $product['price']; ?>" /></td>
										<td class="text-right"><input type="text"
											name="products[<?php echo $product['product_id']; ?>][quantity]"
											class="form-control <?php if ($product['quantity'] <= 0) { echo "alert-danger"; } elseif ($product['quantity'] <= 5) { echo "alert-warning"; } else { echo "alert-success"; } ?>"
											value="<?php echo $product['quantity']; ?>" /></td>
										<td class="text-right"><input type="text"
											name="products[<?php echo $product['product_id']; ?>][minimum]"
											class="form-control"
											value="<?php echo $product['minimum']; ?>" /></td>
										<td class="text-right"><select
											name="products[<?php echo $product['product_id']; ?>][stock_status_id]"
											id="input-stock-status" class="form-control">
					...
					<?php } ?>
										...
										<td class="text-left"><select class="form-control"
											name="products[<?php echo $product['product_id']; ?>][status]">
												<option value="1"
													<?php echo $product['status'] ? 'selected' : '' ; ?>>Enabled</option>
												<option value="0"
													<?php echo $product['status'] ? '' : 'selected' ; ?>>Disabled</option>
										</select></td>
										<td class="text-right"><a <?php if($product['sold']){ ?>
											href="<?php echo $product['soldlink']; ?>" <?php } ?>
                    ...
                    <span class="label label-warning"
											data-toggle="tooltip"
											title="<?php echo $text_soldlist_info; ?>"><?php echo $product['sold']; ?></span>
                    <?php } else { ?>
                    ...
                    <?php } ?></td>
										...
										<td class="text-right"><span class="text-success"><?php echo $product['totalearn']; ?></span>
										</td>

										<td class="text-right">
                  <?php if($product['action']){ ?>
                    <?php foreach ($product['action'] as $action) { ?>
                      <a <?php if(!$allowedAddEdit) echo "disabled"; ?>
											href="<?php echo $action['href']; ?>" class="btn btn-info"><span
												data-toggle="tooltip" title="<?php echo $action['text']; ?>"><i
