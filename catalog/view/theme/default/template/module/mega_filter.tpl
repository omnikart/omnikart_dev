...
<div
		class="box mfilter-box mfilter-<?php echo $_position; ?><?php echo ! empty( $hide_container ) ? ' mfilter-hide-container' : '' ?><?php echo ! empty( $_displayOptionsAs ) && $_displayOptionsAs == 'modern_horizontal' ? ' mfilter-hide' : ''; ?>"
...
/*
	...
	?>
	<div
			class="box-content mfilter-content<?php echo empty( $settings['calculate_number_of_products'] ) || empty( $settings['show_number_of_products'] ) ? ' mfilter-hide-counter' : ''; ?>">
		...
		<ul>
			<?php foreach( $filters as $kfilter => $filter ) { ?>
				...
				?>
				<li data-type="<?php echo $filter['type']; ?>"
					...
					data-auto-levels="<?php echo $filter['auto_levels']; ?>" <?php } ?>
				...
				?>>
							...
							<div class="mfilter-content-wrapper">
								<div class="mfilter-options">
									<?php if( $base_type == 'categories' ) { ?>
										<div
										class="mfilter-option mfilter-category mfilter-category-<?php echo $filter['type']; ?>">
											<?php if( $filter['type'] == 'related' ) { ?>
												<ul
											data-labels="<?php echo str_replace( '"', '&quot;', implode( '#|#', $filter['labels'] ) ); ?>">
										...
										<ul class="mfilter-tb"
											...
											data-top-path="<?php echo $filter['top_path']; ?>">
												...
												<div class="mfilter-tb-as-td mfilter-col-count">
													<span class="mfilter-counter"><?php echo $category['cnt']; ?></span>
												</div>
											</li>
													<?php } ?>
												</ul>
						...
						?>
							...
							?>
														<div
												class="mfilter-option mfilter-tb-as-tr<?php echo $category['cnt'] ? '' : ' mfilter-hide'; ?><?php echo $tmpCatShown == 1 && $tmpCatIdx > $tmpCatShown ? ' mfilter-first-child' : ''; ?>">
									...
									<?php } else if( $filter['type'] == 'search' || $filter['type'] == 'search_oc' ) { ?>
										<div
										class="mfilter-option mfilter-search<?php echo ! empty( $filter['button'] ) ? ' mfilter-search-button' : ''; ?>">
										<input id="mfilter-opts-search" type="text"
											class="form-control"
											...
											<?php if( ! empty( $filter['button'] ) ) { ?>
												<i id="mfilter-opts-search_button" type="submit"></i>
											<?php } ?>
										</div>
									<?php } else if( $filter['type'] == 'price' ) { ?>
										...
										<div class="mfilter-price-inputs">
												...
												<input id="mfilter-opts-price-min" type="text"
												class="form-control"
												value="<?php echo isset( $params['price'][0] ) ? $params['price'][0] : ''; ?>" />
													<?php echo $getSymbolRight; ?>
													-
												<?php echo $getSymbolLeft; ?>
												<input id="mfilter-opts-price-max" type="text"
												class="form-control"
									...
									<?php } else if( $filter['type'] == 'slider' ) { ?>
										...
										<div class="mfilter-slider-inputs">
											<input class="mfilter-opts-slider-min form-control"
												...
												class="mfilter-opts-slider-max form-control" type="text"
					...
					?>
										...
										<div class="mfilter-option mfilter-select">
											<select>
												<option value="">---</option>
													...
													<?php echo ! empty( $params[$filter['seo_name']] ) && in_array( $option['value'], $params[$filter['seo_name']] ) ? ' selected="selected"' : ''; ?>><?php echo $option['name']; ?></option>
													<?php } ?>
												</select>
										</div>
									</div>
									<?php } else if( $filter['type'] == 'image' ) { ?>
										...
										<ul>
												<?php foreach( $filter['options'] as $option_id => $option ) { ?>
													<li class="mfilter-option mfilter-image"><input
												id="mfilter-opts-attribs-<?php echo (int) $_idx; ?>-<?php echo $base_id; ?>-<?php echo $option['key']; ?>"
												name="<?php echo $filter['seo_name']; ?>" type="checkbox"
												style="display: none"
												<?php echo ! empty( $params[$filter['seo_name']] ) && in_array( $option['value'], $params[$filter['seo_name']] ) ? ' checked="checked"' : ''; ?>
												value="<?php echo str_replace( '"', '&quot;', $option['value'] ); ?>" />
												<label
												...
												title="<?php echo $option['name']; ?>"><img
													src="<?php echo $option['image']; ?>" /></label></li>
												<?php } ?>
											</ul>
									</div>
									<?php } ?>
