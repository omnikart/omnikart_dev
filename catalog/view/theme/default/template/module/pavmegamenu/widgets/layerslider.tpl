...
foreach ( $sliders as $_key => $slider ) {
								...
								<li <?php echo $link.' ' .$sliderDelay; 	?>
					data-masterspeed="<?php echo $slider['params']['slider_duration'];?>"
					data-transition="<?php echo $slider['params']['slider_transition'];?>"
					data-slotamount="<?php echo $slider['params']['slider_slot'];?>"
					data-thumb="<?php echo $slider['thumbnail'];?>">
										...
										?>
								<div class="caption fade fullscreenvideo" data-autoplay="true"
						data-x="0" data-y="0" data-speed="500" data-start="10"
						data-easing="easeOutBack">
						<iframe
							src="<?php echo $vurl;?>?title=0&amp;byline=0&amp;portrait=0;api=1"
							width="100%" height="100%"></iframe>
					</div>
										<?php }elseif( $slider['main_image'] ) { ?>
											
											<img src="<?php echo $slider['main_image']; ?>"
					alt="Image <?php echo $_key; ?>" />
										<?php } ?>
									...
									foreach ( $slider ['layersparams']->layers as $i => $layer ) {
										...
										<div
						class="caption <?php echo $layer['layer_class']; ?> <?php echo $layer['layer_animation'];?> 
											<?php echo $layer['layer_easing'];?>   <?php echo $layer['layer_easing'];?> 
											<?php echo $layer['layer_endanimation'];?>"
						data-x="<?php echo $layer['layer_left']; ?>"
						data-y="<?php echo $layer['layer_top']; ?>" data-speed="300"
						data-start="<?php echo $layer['time_start']; ?>"
						data-easing="easeOutExpo" <?php echo $endeffect;?>>
											 	<?php if( $type=='image') { ?> 
											 	<img
							src="<?php echo $url."image/".$layer['layer_content'];?>"
							alt="<?php echo $layer['layer_content']; ?>" />
												 	<?php } else if( $type == 'video' ) { ?>
													 	<?php if( $layer['layer_video_type'] == 'vimeo')  { ?>
												 		<iframe
							src="http://player.vimeo.com/video/<?php echo $layer['layer_video_id'];?>?wmode=transparent&amp;title=0&amp;byline=0&amp;portrait=0;api=1"
							width="<?php echo $layer['layer_video_width'];?>"
							height="<?php echo $layer['layer_video_height'];?>"></iframe>
												 		<?php } else { ?>
												 			<iframe
							width="<?php echo $layer['layer_video_width'];?>"
							height="<?php echo $layer['layer_video_height'];?>"
							src="http://www.youtube.com/embed/<?php echo $layer['layer_video_id'];?>?wmode=transparent"
						...
						<div
				class="tp-bannertimer tp-<?php echo $sliderParams['time_line_position']; ?>"></div>
						<?php } ?>
					</div>
	</div>
