...
class ControllerAccountCustomerpartnerExcelport extends Controller {
	...
	public function ajaxgenerate() {
		...
		try {
			...
			if ($this->model_module_excelport->exportXLS('Products','1','0', IMODULE_ROOT . IMODULE_TEMP_FOLDER .'/'. $this->customer->getId(), 
				array('ExportLimit' => 500,'ImportLimit' => 800), false, false, array())) {
	...
	public function ajaximport() {
		...
		if (!empty($this->session->data['uploaded_files'])) {
			...
			try {
				...
				$this->model_module_excelport->importXLS('Products', '1', $file, array('ExportLimit' => 500,'ImportLimit' => 800),false);
			} catch (Exception $e) {
				$error = $e->getMessage();	
			}
