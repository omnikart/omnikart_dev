...
class ControllerAccountCustomerpartnerDashboardsOrder extends Controller {
	public function index() {
		...
		$today = $this->model_account_customerpartner->getSellerOrdersTotal ( array (
				'filter_date' => date ( 'Y-m-d', strtotime ( '-1 day' ) ) 
		), $sellerId );
		
		$yesterday = $this->model_account_customerpartner->getSellerOrdersTotal ( array (
				'filter_date' => date ( 'Y-m-d', strtotime ( '-2 day' ) ) 
		), $sellerId );
		...
		$order_total = $this->model_account_customerpartner->getSellerOrdersTotal ( array (), $sellerId );
