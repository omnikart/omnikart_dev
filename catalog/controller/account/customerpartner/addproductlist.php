<?php
class ControllerAccountCustomerpartnerAddproductlist extends Controller {
	...
	public function index() {
		...
		if($this->config->get('wkmpuseseo')) $url = '';
		...
		if (isset($this->request->get['filter_category'])) $url1 .= '&filter_category=' . $this->request->get['filter_category'];
		if ($order == 'ASC') $url1 .= '&order=DESC'; else $url1 .= '&order=ASC';
		if (isset($this->request->get['page'])) $url1 .= '&page=' . $this->request->get['page'];

		$this->data['sort_name'] = $this->url->link('account/customerpartner/productlist', '' . '&sort=pd.name' . $url1, 'SSL');
		$this->data['sort_model'] = $this->url->link('account/customerpartner/productlist', '' . '&sort=p.model' . $url1, 'SSL');
		$this->data['sort_order'] = $this->url->link('account/customerpartner/productlist', '' . '&sort=p.sort_order' . $url1, 'SSL');
		...
		if (isset($this->request->get['sort'])) $url1 .= '&sort=' . $this->request->get['sort'];
		if (isset($this->request->get['order'])) $url1 .= '&order=' . $this->request->get['order'];
		...
		if (!empty($filter_name)) {
			...
			foreach ($results as $key => $result) {
				...
				foreach ($results[$key]['category'] as $category){
					if (!isset($this->data['categories'][$category['parent_id']])) {
						$this->data['categories'][$category['parent_id']] = $this->model_account_customerpartner->getCategory($category['parent_id']);
					}
				}
			}
			foreach ($this->data['categories'] as $key => $category){
				$this->data['categories'][$key]['link'] = $this->url->link('account/customerpartner/addproductlist', '' . '&filter_category='.$key. $url, 'SSL');
			}
		...
		$this->data['addproducts'] = $this->url->link('account/customerpartner/addproductlist/addProduct', ''.$url, 'SSL');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/customerpartner/addproductlist.tpl')) {
			$this->response->setOutput($this->load->view( $this->config->get('config_template') . '/template/account/customerpartner/addproductlist.tpl' , $this->data));			
		} else {
			$this->response->setOutput($this->load->view('default/template/account/customerpartner/addproductlist.tpl' , $this->data));
		}
