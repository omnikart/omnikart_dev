...
class ControllerPaymentGlobalpayRemote extends Controller {
	...
	public function send() {
		...
		$capture_result = $this->model_payment_globalpay_remote->capturePayment ( $account, $amount, $currency, $order_id, $order_ref, $this->request->post ['cc_number'], $this->request->post ['cc_expire_date_month'] . $this->request->post ['cc_expire_date_year'], $this->request->post ['cc_name'], $this->request->post ['cc_type'], $this->request->post ['cc_cvv2'], $this->request->post ['cc_issue'], $eci_ref, $eci, $cavv, $xid );
	...
	public function acsReturn() {
		if (isset ( $this->session->data ['order_id'] )) {
			...
			$capture_result = $this->model_payment_globalpay_remote->capturePayment ( $md ['account'], $md ['amount'], $md ['currency'], $md ['order_id'], $md ['order_ref'], $md ['cc_number'], $md ['cc_expire'], $md ['cc_name'], $md ['cc_type'], $md ['cc_cvv2'], $md ['cc_issue'], $eci_ref, $eci, $cavv, $xid );
