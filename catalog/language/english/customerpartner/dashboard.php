...
class ModelCustomerpartnerDashboard extends Model {
	...
	public function getTotalCustomersByDay() {
		...
		$query = $this->db->query ( "SELECT COUNT(*) AS total, HOUR(date_added) AS hour FROM `" . DB_PREFIX . "customer` WHERE DATE(c2o.date_added) = DATE(NOW()) GROUP BY HOUR(date_added) ORDER BY date_added ASC" );
	...
	public function getTotalOrdersByCountry() {
		$query = $this->db->query ( "SELECT COUNT(DISTINCT c2o.order_id) AS total, SUM(c2o.customer) AS amount, c.iso_code_2 FROM `" . DB_PREFIX . "order_product` op LEFT JOIN " . DB_PREFIX . "customerpartner_to_order c2o ON (op.product_id = c2o.product_id AND op.order_id = c2o.order_id) LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) LEFT JOIN `" . DB_PREFIX . "country` c ON (o.payment_country_id = c.country_id) WHERE c2o.customer_id='" . $this->customer->getId () . "' AND o.order_status_id > '0' GROUP BY o.payment_country_id" );
	...
	public function getTotalOrdersByYear() {
		...
		$query = $this->db->query ( "SELECT COUNT(*) AS total, c2o.date_added FROM `" . DB_PREFIX . "order` WHERE order_status_id = '" . ( int ) $this->config->get ( 'config_complete_status_id' ) . "' AND YEAR(c2o.date_added) = YEAR(NOW()) GROUP BY MONTH(c2o.date_added)" );
