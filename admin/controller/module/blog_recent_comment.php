	...
	public function index() {
		...
		if (($this->request->server ['REQUEST_METHOD'] == 'POST') && $this->validate ()) {
			if (! isset ( $this->request->get ['module_id'] )) {
				$this->model_extension_module->addModule ( 'blog_recent_comment', $this->request->post );
			} else {
		...
		if (isset ( $this->error ['listicon'] )) {
			$data ['error_listicon'] = $this->error ['listicon'];
		} else {
			$data ['error_listicon'] = '';
		}
		...
		$data ['breadcrumbs'] [] = array (
				...
				'href' => $this->url->link ( 'module/blog_recent_comment', 'token=' . $this->session->data ['token'], 'SSL' ) 
		);
		
		if (! isset ( $this->request->get ['module_id'] )) {
			$data ['action'] = $this->url->link ( 'module/blog_recent_comment', 'token=' . $this->session->data ['token'], 'SSL' );
		} else {
			$data ['action'] = $this->url->link ( 'module/blog_recent_comment', 'token=' . $this->session->data ['token'] . '&module_id=' . $this->request->get ['module_id'], 'SSL' );
		}
		...
		$this->response->setOutput ( $this->load->view ( 'module/blog_recent_comment.tpl', $data ) );
	}
	protected function validate() {
		if (! $this->user->hasPermission ( 'modify', 'module/blog_recent_comment' )) {
