...
class ControllerModuleExcelport extends Controller {
	...
	public function ajaxgenerate() {
		...
		try {
			...
			if ($this->model_module_excelport->exportXLS ( $this->request->post ['ExcelPort'] ['Export'] ['DataType'], $this->request->post ['ExcelPort'] ['Export'] ['Language'], $this->request->post ['ExcelPort'] ['Export'] ['Store'], IMODULE_ROOT . IMODULE_TEMP_FOLDER, $this->request->post ['ExcelPort'] ['Settings'], ! empty ( $this->request->post ['ExcelPort'] ['Export'] ['QuickExport'] ), ! empty ( $this->request->post ['ExcelPort'] ['Export'] ['Filter'] ), ! empty ( $this->request->post ['ExcelPort'] ['Export'] ['Filters'] ) ? $this->request->post ['ExcelPort'] ['Export'] ['Filters'] : array () )) {
