...
class ControllerModulePavmegamenu extends Controller {
	...
	public function index() {
		...
		$this->mdata ['layouts'] [] = array (
				'layout_id' => 99999,
				'name' => $this->language->get ( 'all_page' ) 
		);
	...
	public function ajxmenuinfo() {
		...
		if (isset ( $this->request->post ['params'] )) {
			...
			$this->model_setting_setting->editSetting ( 'pavmegamenu_params', array (
					'pavmegamenu_params' => $params 
			) );
	...
	public function widgetform() {
		...
		if (isset ( $this->request->post ['id'] ) && $this->hasPermssion ()) {
			...
			$form = $this->model_menu_widget->renderForm ( $data ['type'], array (), array (
					'params' => $data ['params'] 
			) );
