...
class ControllerModuleMegaFilter extends Controller {
		...
		if ($this->request->server ['REQUEST_METHOD'] == 'POST' && $this->checkPermission ()) {
			...
			if (! $pager && $idx == $count) {
				...
				if (empty ( $this->session->data ['mf_data_to_save'] ['store_id'] )) {
					$this->session->data ['mf_data_to_save'] ['store_id'] = array (
							0 
					);
				}
	...
	public function index() {
		...
		foreach ( $this->data ['modules'] as $module ) {
			if (empty ( $module ['hide_category_id'] ))
				continue;
	...
	private function _getItems($start = NULL, $limit = NULL, $filter = NULL) {
		...
		foreach ( $this->model_catalog_attribute->getAttributes ( array (
				'start' => $start,
				'limit' => $limit,
				'filter_name' => $filter 
		) ) as $attribute ) {
	...
	public function attributes() {
		...
		if ($this->request->server ['REQUEST_METHOD'] == 'POST' && $this->checkPermission ()) {
			...
			// główny sklep
			$this->model_setting_setting->editSetting ( $this->_name . '_attribs', array (
					$this->_name . '_attribs' => $config 
			) );
	...
	public function getAttribsByGroupAsJson() {
		...
		if (! empty ( $this->request->get ['attribute_group_id'] )) {
			$this->load->model ( 'catalog/attribute' );
			$json = $this->model_catalog_attribute->getAttributes ( array (
					'filter_attribute_group_id' => ( int ) $this->request->get ['attribute_group_id'] 
			) );
		}
	...
	public function options() {
		...
		if ($this->request->server ['REQUEST_METHOD'] == 'POST' && $this->checkPermission ()) {
			...
			// główny sklep
			$this->model_setting_setting->editSetting ( $this->_name . '_options', array (
					$this->_name . '_options' => $config 
			) );
	...
	public function filters() {
		...
		if ($this->request->server ['REQUEST_METHOD'] == 'POST' && $this->checkPermission ()) {
			...
			// główny sklep
			$this->model_setting_setting->editSetting ( $this->_name . '_filters', array (
					$this->_name . '_filters' => $config 
			) );
	...
	public function aliases() {
		...
		if (! empty ( $this->request->get ['action'] ) && $this->checkPermission ()) {
			switch ($this->request->get ['action']) {
				case 'insert' :
					{
						if (NULL != ($url = isset ( $this->request->get ['url'] ) ? $this->request->get ['url'] : '') && NULL != ($alias = isset ( $this->request->get ['alias'] ) ? $this->request->get ['alias'] : '')) {
