...
class ControllerModuleMMOSCheckoutOnepage extends Controller {
	...
	public function index($render_content = false) {
		...
		if (($this->request->server ['REQUEST_METHOD'] == 'POST') && $this->validate ()) {
			...
			$this->model_setting_setting->editSetting ( 'mmos_checkout', array (
					'mmos_checkout' => $this->request->post ['mmos_checkout'],
					'mmos_checkout_tips' => $this->request->post ['tips'],
					'mmos_checkout_css' => $this->request->post ['css'],
					'mmos_checkout_langs' => $this->request->post ['langs'] 
	...
	public function install() {
		if ($this->user->hasPermission ( 'modify', 'extension/module' )) {
			...
			$this->model_setting_setting->editSetting ( 'mmos_checkout_themes', array (
					'mmos_checkout_themes' => $themes 
			) );
			$this->model_setting_setting->editSetting ( 'mmos_checkout', array (
					'mmos_checkout' => $base_config,
					'mmos_checkout_css' => $css,
					'mmos_checkout_tips' => $tips,
					'mmos_checkout_langs' => $langs 
			), 0 );
