	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('blog_tag', $this->request->post);
			} else {
		...
		$data['text_layout_flat'] = $this->language->get('text_layout_flat');
		$data['text_layout_rotation'] = $this->language->get('text_layout_rotation');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/blog_tag', 'token=' . $this->session->data['token'], 'SSL')
		);

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('module/blog_tag', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('module/blog_tag', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		if (isset($this->request->post['layout'])) {
			...
			$data['tag_layout'] = $module_info['tag_layout'];
		} else {
			$data['layout'] = 'flat';
		}
		...
		$this->response->setOutput($this->load->view('module/blog_tag.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/blog_tag')) {
