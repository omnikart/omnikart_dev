<?php    
class ControllerCustomerpartnerMail extends Controller { 
	...
	private $data = array();
  	...
  	public function getlist() {	
		...
		foreach ($filter_array as $unsetKey => $key) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('customerpartner/mail', 'token=' . $this->session->data['token']. $url, 'SSL'), 		
      		'separator' => ' :: '
   		);		
				
		$this->data['delete'] = $this->url->link('customerpartner/mail/delete', 'token=' . $this->session->data['token'] , 'SSL');
		$this->data['insert'] = $this->url->link('customerpartner/mail/addmail', 'token=' . $this->session->data['token'] , 'SSL');
	    ...
	    foreach ($results as $result) {		
	      	...
	      	$this->data['mails'][] = array(
				...
				'action' => $this->url->link('customerpartner/mail/addMail', 'token=' . $this->session->data['token'] . '&mail_id=' . $result['id'], 'SSL'),				
			);
		...
		$this->data['sort_name'] = $this->url->link('customerpartner/mail', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$this->data['sort_id'] = $this->url->link('customerpartner/mail', 'token=' . $this->session->data['token'] . '&sort=id' . $url, 'SSL');
		$this->data['sort_message'] = $this->url->link('customerpartner/mail', 'token=' . $this->session->data['token'] . '&sort=message' . $url, 'SSL');
		$this->data['sort_subject'] = $this->url->link('customerpartner/mail', 'token=' . $this->session->data['token'] . '&sort=subject' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('customerpartner/mail', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
  	...
  	public function addmail() {
    	...
    	$this->data['cancel'] = $this->url->link('customerpartner/mail', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['save'] = $this->url->link('customerpartner/mail/mailSave', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	public function mailSave() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if(!isset($this->error['warning'])){	
				...
				$this->response->redirect($this->url->link('customerpartner/mail', 'token=' . $this->session->data['token'], 'SSL'));
			}
		}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validate()) {
			...
			$this->response->redirect($this->url->link('customerpartner/mail', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	private function validate() {
		...
		if (!$this->user->hasPermission('modify', 'customerpartner/mail')) {
