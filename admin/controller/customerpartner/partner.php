...
class ControllerCustomerpartnerPartner extends Controller {
	...
	private function getForm() {
		...
		if ($partner_info) {
			...
			foreach ( $this->data ['partner_orders'] as $key => $value ) {
				...
				$this->data ['partner_orders'] [$key] ['edit'] = $this->url->link ( 'sale/order/update', 'token=' . $this->session->data ['token'] . '&order_id=' . $value ['order_id'], 'SSL' );
			}
	...
	public function transaction() {
		...
		if (($this->request->server ['REQUEST_METHOD'] == 'POST') && $this->user->hasPermission ( 'modify', 'customerpartner/partner' )) {
			$this->model_customerpartner_transaction->addTransaction ( array (
					'customer_id' => $this->request->get ['customer_id'],
					'details' => $this->request->post ['description'],
		...
		$results = $this->model_customerpartner_transaction->getTransactions ( $this->request->get ['customer_id'], ($page - 1) * 10, 10 );
		...
		$pagination->limit = 10;
		$pagination->text = $this->language->get ( 'text_pagination' );
		$pagination->url = $this->url->link ( 'customerpartner/partner/transaction', 'token=' . $this->session->data ['token'] . '&customer_id=' . $this->request->get ['customer_id'] . '&page={page}', 'SSL' );
