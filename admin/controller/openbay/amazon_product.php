...
class ControllerOpenbayAmazonProduct extends Controller {
	public function index() {
		...
		if (($this->request->server ['REQUEST_METHOD'] == 'POST') && $this->validateForm ()) {
			...
			if ($data_array ['upload_after'] === 'true') {
				...
				if ($upload_result ['status'] == 'ok') {
					...
					$data ['errors'] [] = Array (
							'message' => $upload_result ['error_message'] 
					);
				}
			} else {
		...
		$data ['marketplaces'] = array (
				...
				array (
						'name' => $this->language->get ( 'text_france' ),
						'id' => 'A13V1IB3VIYZZH',
						'code' => 'fr' 
				),
				array (
						'name' => $this->language->get ( 'text_italy' ),
						'id' => 'APJ6JRA9NG5V4',
						'code' => 'it' 
				),
				array (
						'name' => $this->language->get ( 'text_spain' ),
						'id' => 'A1RKKUPIHCS9HS',
						'code' => 'es' 
				),
				array (
						'name' => $this->language->get ( 'text_united_kingdom' ),
						'id' => 'A1F83G8C2ARO7P',
						'code' => 'uk' 
				) 
		);
		...
		if ($this->config->get ( 'openbay_amazon_default_listing_marketplace' )) {
			$data ['default_marketplaces'] = array (
					$marketplace_mapping [$this->config->get ( 'openbay_amazon_default_listing_marketplace' )] 
			);
		} else {
			$data ['default_marketplaces'] = array ();
		}
	...
	private function uploadItems() {
		...
		foreach ( $saved_products as $saved_product ) {
			...
			$response_data = array (
					"response_url" => $catalog . 'index.php?route=openbay/amazon/product' 
			);
			$category_data = array (
					'category' => ( string ) $saved_product ['category'] 
			);
			$fields_data = array (
					'fields' => ( array ) $product_data_decoded ['fields'] 
			);
			
			$mp_array = ! empty ( $saved_product ['marketplaces'] ) ? ( array ) unserialize ( $saved_product ['marketplaces'] ) : array ();
			$marketplaces_data = array (
					'marketplaces' => $mp_array 
			);
	...
	private function fillDefaultValues($product_id, $fields_array, $var = '') {
		...
		foreach ( $fields_array as $field ) {
			...
			if (isset ( $defaults [strtolower ( $field ['name'] )] )) {
				$value_array = array (
						'value' => $defaults [strtolower ( $field ['name'] )] 
				);
			}
