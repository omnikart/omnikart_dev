...
class ControllerOpenbayAmazonus extends Controller {
	...
	public function settings() {
		...
		$amazonus_order_statuses = array (
				'unshipped' => array (
						'name' => $this->language->get ( 'text_unshipped' ),
						'order_status_id' => $unshipped_status_id 
				),
				'partially_shipped' => array (
						'name' => $this->language->get ( 'text_partially_shipped' ),
						'order_status_id' => $partially_shipped_status_id 
				),
				'shipped' => array (
						'name' => $this->language->get ( 'text_shipped' ),
	...
	public function addLink() {
		if (isset ( $this->request->get ['product_id'] ) && isset ( $this->request->get ['amazon_sku'] )) {
			...
			if ($var != '' && $this->openbay->addonLoad ( 'openstock' )) {
				...
				$this->openbay->amazonus->putStockUpdateBulk ( array (
						$product_id 
				) );
			}
	...
	public function loadListingReport() {
		...
		$request_data = array (
				'response_url' => HTTPS_CATALOG . 'index.php?route=openbay/amazonus/listingreport' 
		);
