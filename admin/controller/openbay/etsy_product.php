...
class ControllerOpenbayEtsyProduct extends Controller {
	...
	public function addLink() {
		...
		if (! isset ( $data ['product_id'] )) {
			echo json_encode ( array (
					'error' => $this->language->get ( 'error_product_id' ) 
			) );
			die ();
		}
		...
		if (! isset ( $data ['etsy_id'] )) {
			echo json_encode ( array (
					'error' => $this->language->get ( 'error_etsy_id' ) 
			) );
			die ();
		}
		...
		if ($links != false) {
			echo json_encode ( array (
					'error' => $this->language->get ( 'error_link_exists' ) 
			) );
			die ();
		}
		...
		if (isset ( $get_response ['data'] ['error'] )) {
			echo json_encode ( array (
					'error' => $this->language->get ( 'error_etsy' ) . $get_response ['data'] ['error'] 
			) );
			die ();
		} else {
			if (( int ) $get_response ['quantity'] != ( int ) $product ['quantity']) {
				...
				if (isset ( $update_response ['data'] ['error'] )) {
					echo json_encode ( array (
							'error' => $this->language->get ( 'error_etsy' ) . $update_response ['data'] ['error'] 
					) );
					die ();
				}
			}
	...
	public function deleteLink() {
		...
		if (! isset ( $data ['etsy_link_id'] )) {
			echo json_encode ( array (
					'error' => $this->language->get ( 'error_link_id' ) 
			) );
			die ();
		}
	...
	public function endListing() {
		...
		if (isset ( $response ['data'] ['error'] )) {
			echo json_encode ( array (
					'error' => $this->language->get ( 'error_etsy' ) . $response ['data'] ['error'] 
			) );
			die ();
		} else {
