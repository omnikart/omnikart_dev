...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		if (isset ( $this->request->get ['directory'] )) {
			$directory = rtrim ( DIR_IMAGE . 'catalog/' . str_replace ( array (
					'../',
					'..\\',
					'..' 
	...
	public function ckeditor() {
		...
		foreach ( $images as $image ) {
			...
			if (is_dir ( $image )) {
				...
				$data ['images'] [] = array (
						...
						'type' => 'directory',
						'path' => utf8_substr ( $image, utf8_strlen ( DIR_IMAGE ) ),
						'href' => $this->url->link ( 'common/filemanager/ckeditor', 'token=' . $this->session->data ['token'] . '&directory=' . urlencode ( utf8_substr ( $image, utf8_strlen ( DIR_IMAGE . 'catalog/' ) ) ) . $url, 'SSL' ) 
				);
			} elseif (is_file ( $image )) {
				// Find which protocol to use to pass the full image link back
				if ($this->request->server ['HTTPS']) {
		...
		$data ['parent'] = $this->url->link ( 'common/filemanager/ckeditor', 'token=' . $this->session->data ['token'] . $url, 'SSL' );
		...
		$pagination->page = $page;
		$pagination->limit = 16;
		$pagination->url = $this->url->link ( 'common/filemanager/ckeditor', 'token=' . $this->session->data ['token'] . $url . '&page={page}', 'SSL' );
		$data ['header'] = $this->load->controller ( 'common/header' );
		$data ['footer'] = $this->load->controller ( 'common/footer' );
		$data ['pagination'] = $pagination->render ();
		
		$this->response->setOutput ( $this->load->view ( 'common/ckeditor.tpl', $data ) );
	}
