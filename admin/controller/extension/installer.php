...
class ControllerExtensionInstaller extends Controller {
	...
	public function localcopy() {
		...
		if (VERSION == '2.0.0.0') {
			$directory = DIR_DOWNLOAD . str_replace ( array (
					'../',
					'..\\',
					'..' 
			), '', $this->request->post ['path'] ) . '/upload/';
		} else {
			$directory = DIR_UPLOAD . str_replace ( array (
					'../',
					'..\\',
					'..' 
			), '', $this->request->post ['path'] ) . '/upload/';
	...
	public function unzip() {
		...
		$file = DIR_UPLOAD . str_replace ( array (
				'../',
				'..\\',
				'..' 
		), '', $this->request->post ['path'] ) . '/upload.zip';
		...
		if (! $json) {
			...
			if ($zip->open ( $file )) {
				$zip->extractTo ( DIR_UPLOAD . str_replace ( array (
						'../',
						'..\\',
						'..' 
	...
	public function sql() {
		...
		$file = DIR_UPLOAD . str_replace ( array (
				...
				'..\\',
				'..' 
		), '', $this->request->post ['path'] ) . '/install.sql';
	...
	public function xml() {
		...
		$file = DIR_UPLOAD . str_replace ( array (
				...
				'..\\',
				'..' 
		), '', $this->request->post ['path'] ) . '/install.xml';
	...
	public function php() {
		...
		$file = DIR_UPLOAD . str_replace ( array (
				...
				'..\\',
				'..' 
		), '', $this->request->post ['path'] ) . '/install.php';
	...
	public function remove() {
		...
		if (! $json) {
			...
			while ( count ( $path ) != 0 ) {
				...
				foreach ( array_diff ( scandir ( $next ), array (
						'.',
						'..' 
				) ) as $file ) {
					$file = $next . '/' . $file;
