	...
	<div class="page-header">
		<div class="container-fluid">
			<div class="pull-right">
				<button type="submit" form="form-combo" data-toggle="tooltip"
      ...
      <div class="panel panel-default">
			<div class="panel-heading">
				<h3 class="panel-title">
					<i class="fa fa-pencil"></i> <?php echo $add_title; ?></h3>
			</div>
			<div class="panel-body">
				<form action="<?php echo $action; ?>" method="post"
					enctype="multipart/form-data" id="form-combo">
							...
							<tbody>
								<tr>
									<td class="text-left" style="vertical-align: top;">
										<div
											class="form-group required <?php if ($error_combo_name) {echo 'has-error';}?>">
											<input type="text" name="combo_name"
												value="<?php echo $combo['combo_name']; ?>"
												class="form-control" id="combo-name" />
					...
					<?php } ?>
									...
									<td class="text-left" style="vertical-align: top;">
										<div
											class="form-group required <?php if ($error_combo_products) {echo 'has-error';}?>">
											<input type="text" name="combo-product" value=""
												id="input-product" class="form-control" />
											<div id="combo-products" class="well well-sm"
												style="height: 100px; overflow: auto;">
						<?php foreach ($combo['combo_products'] as $combo_products) { ?>
						<div
													id="combo-products-<?php echo $combo_products['product_id']; ?>">
													<i style="float: left; line-height: 4; padding-right: 2px;"
						  ...
						  <input type="hidden" name="combo_products[]"
														value="<?php echo $combo_products['product_id']; ?>" />
												</div>
						<?php } ?>
					  </div>
					  ...
					  <?php } ?>
											...
											<div class="col-sm-9">
												<select name="discount_type" class="form-control"
													...
													<option value="fixed amount"
														<?php if($combo['discount_type'] == "fixed amount") echo "selected "; ?>><?php echo $text_fixed; ?></option>
													<option value="percentage"
														<?php if($combo['discount_type'] == "percentage") echo "selected "; ?>><?php echo $text_percent; ?></option>
												</select>
												<p></p>
											</div>
										</div>
										<div
											class="form-group required <?php if ($error_discount_number) {echo 'has-error';}?>">
											<label class="col-sm-3 control-label"><?php echo $text_discount_numb; ?></label>
											<div class="col-sm-9">
												<input type="text" name="discount_number"
													value="<?php echo $combo['discount_number']; ?>"
							...
							<?php } ?>
									...
									<td class="text-left" style="vertical-align: top;">
										<div>
											<input type="checkbox" name="display_detail" value="1"
												<?php if ($combo['display_detail']) echo "checked=checked"; ?> /> <?php echo $text_detail_page?>
					</div>
										<div>
					...
					<?php foreach ($combo['category_list'] as $category_id => $category_list) {;?>
						<option value="<?php echo $category_id;?>"
													<?php if (in_array($category_id,$combo['combo_category'])) echo "selected=selected"; ?>><?php echo $category_list; ?></option>
					<?php }?>
					</select>
										</div>
									...
									<td class="text-left">
										<div>
											<input type="text" name="priority"
												value="<?php echo $combo['priority']; ?>"
												id="input-priority" class="form-control"
												style="width: 40px;" />
										</div>
									</td>
								</tr>
						...
						<div>
							<input type="checkbox" name="override" value="1"
								<?php if ($combo['override']) echo "checked=checked"; ?> /> <?php echo $text_override?>
			</div>
					</div>
				</form>
			</div>
