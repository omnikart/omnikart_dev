...
$corePath = HTTP_SERVER.'view/image/magic360-opencart-module';
?>
...
<div class="box">
  ...
  <?php 
    $first = false;
    foreach ($blocks as $bid => $btitle) { ?>
    <div id="tab-<?php echo $bid ?>" <?php if (!$first)  echo 'style="display: block"'; else echo 'style="display: none"'; ?> >
  ...
  <?php trial_bage(); ?>
    ...
    <h3 class="settingsTitle">Module status</h3>
        ...
        <table class="params" cellspacing="0">
            <tr class="last">
                <th width="30%">
                    <label for="magic360settingStatus">Enable/Disable module</label>
                </th>
                <td width="70%">
                    <select name="<?php echo $bid ?>_magic360_status">
                    <?php if (this->config->get($bid.'magic360_status')) { ?>
    ...
    <?php
        ...
        $groups = array();
        foreach ($parameters[$bid] as $name => $s) {
                            ...
                            switch($s["type"]) {
                                case "array":
                                        ...
                                        foreach($s["values"] as $p) {
                                            ...
                                            $rButtons[strtolower($p)] = '<label><input type="radio" value="'.$p.'"'. ($s["value"]==$p?"checked=\"checked\"":"").' name="'.$s['id'].'" id="magic360settings'. ucwords(strtolower($name)).$p.'">';
                                            $pName = ucwords($p);
                                            if(strtolower($p) == "yes")
                                ...
                                default:
                                    ...
                                    if (!isset($s["value"])) $s["value"] = $s["default"];
                                    $html .= '<input '.$width.' type="text" name="'.$s['id'].'" id="magic360settings'. ucwords(strtolower($name)).'" value="'.$s["value"].'" />';
                                    break;
                            }
        ...
        <table class="params" cellspacing="0">
            <tr>
                <th width="30%">
                    <label for="magic360settingChache-path">Path to cache folder<br/>Relative for site base path</label>
                </th>
                <td width="70%">
            ...
            <tr class="back">
                <th width="30%">
                    <label for="magic360settingTotal-items">Total items</label>
                </th>
                <td width="70%">
        ...
        <tr>
            <th width="30%">
                <label for="magic360settingUnuser-items">Unused items</label>
            </th>
            <td width="70%">
        ...
        <tr class="last back">
            <th width="30%"><label for="magic360settingEmpte-chache">Empty cache</label></th>
            <td width="70%">
...
<?php
function trial_bage() {
if (!preg_match('/DEMO/is',@file_get_contents(DIR_CATALOG.'view/javascript/magic360.js'))) return;
echo '
