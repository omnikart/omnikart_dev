...
<?php
	...
	<ul class="nav nav-tabs" id="language-<?php echo $module_row; ?>">
		...
		<li><a href="#tab-language-<?php echo $language['language_id']; ?>"
			data-toggle="tab"><img
	...
	<div class="tab-content">
		<?php foreach ($languages as $language) { ?>
		<div class="tab-pane"
			id="tab-language-<?php echo $language['language_id']; ?>">
			<table class="table no-border">
				<tr>
					<td><?php echo $objlang->get('entry_menu_title');?></td>
					<td><input class="form-control"
						name="megamenu_description[<?php echo $language['language_id'];?>][title]"
						value="<?php echo (isset($menu_description[$language['language_id']]['title'])?$menu_description[$language['language_id']]['title']:"" );?>" />
					</td>
				</tr>
				<tr>
					<td><?php echo $objlang->get('entry_description');?></td>
					<td><textarea class="form-control"
							name="megamenu_description[<?php echo $language['language_id'];?>][description]"
							value=""><?php echo (isset($menu_description[$language['language_id']]['description'])?$menu_description[$language['language_id']]['description']:"");?></textarea>
					</td>
				</tr>
			</table>
		</div>
	...
	<input type="hidden" name="megamenu[item]"
		value="<?php echo $menu['item'];?>" />
	<table class="table">
		<tr>
			<td class="col-sm-3"><?php echo $objlang->get('entry_publish');?></td>

			<td class="col-sm-9"><select class="form-control" type="list"
				name="megamenu[published]">
					<?php foreach( $yesno as $key => $val ){ ?>
					<option value="<?php echo $key;?>"
						<?php if( $key==$menu['published']){ ?> selected="selected"
		...
		<tr>
			...
			<td class="col-sm-9"><select class="form-control"
				name="megamenu[type]" id="megamenutype">
					<?php foreach(  $megamenutypes as $mt => $val ){ ?>
					<option value="<?php echo $mt; ?>"
						<?php if($mt == $menu['type']) {?> selected="selected" <?php } ?>><?php echo $val; ?></option>
					<?php } ?>
				</select></td>
		</tr>
		<tr id="megamenutype-url" class="megamenutype">
			<td class="col-sm-3"><?php echo $objlang->get('entry_url');?></td>
			<td class="col-sm-9"><input type="text" class="form-control"
				name="megamenu[url]" value="<?php echo $menu['url'];?>" size="50" />
			</td>
		</tr>
		<tr id="megamenutype-category" class="megamenutype">
			<td class="col-sm-3"><?php echo $objlang->get('entry_category');?></td>
			<td class="col-sm-9"><input type="text" class="form-control"
				name="path" value="<?php echo $menu['megamenu-category'];?>"
				size="100" /> <i><?php echo $objlang->get('text_explain_input_auto');?></i>
			</td>
		</tr>
		<tr id="megamenutype-product" class="megamenutype">
			...
			<td class="col-sm-9"><input type="text" class="form-control"
				...
				value="<?php echo $menu['megamenu-product'];?>" size="50" /> <i><?php echo $objlang->get('text_explain_input_auto');?></i>
			</td>
		</tr>
		<tr id="megamenutype-manufacturer" class="megamenutype">
			<td class="col-sm-3"><?php echo $objlang->get('entry_manufacturer');?></td>
			<td class="col-sm-9"><input type="text" class="form-control"
				name="megamenu-manufacturer"
				value="<?php echo $menu['megamenu-manufacturer'];?>" size="50" /> <i><?php echo $objlang->get('text_explain_input_auto');?></i>
			</td>
		</tr>
		<tr id="megamenutype-information" class="megamenutype">
			<td class="col-sm-3"><?php echo $objlang->get('entry_information');?></td>
			<td class="col-sm-9"><select class="form-control" type="text"
				name="megamenu-information" id="megamenu-information">
					<?php foreach( $informations as $info ){ ?>
					<option value="<?php echo $info['information_id'];?>"
						<?php if( $menu['megamenu-information'] == $info['information_id']){ ?>
						selected="selected" <?php } ?>><?php echo $info['title'];?></option>
					<?php } ?>
				</select></td>
		</tr>
		<tr id="megamenutype-html" class="megamenutype">
			<td class="col-sm-3"><?php echo $objlang->get('entry_html');?></td>
			<td class="col-sm-9"><textarea type="text"
					name="megamenu[content_text]" size="50"><?php echo $menu['content_text'];?></textarea>
				<i><?php echo $objlang->get('text_explain_input_html');?></i></td>
		</tr>
	</table>
	...
	<table class="table">
		...
		<tr>
			...
			<td class="col-sm-10"><a href="" id="thumb-img" data-toggle="image"
				class="img-thumbnail"> <img
					src="<?php echo empty($thumb)?$placeholder:$thumb; ?>" alt=""
					title="" data-placeholder="<?php echo $placeholder; ?>" />
			</a> <input type="hidden" name="megamenu[image]"
				value="<?php echo $menu['image']; ?>" id="input-icon-menu" /></td>
		</tr>
		<tr>
			<td class="col-sm-3"><?php echo $objlang->get('entry_menuclass');?></td>

			<td class="col-sm-9"><input type="text" class="form-control"
				name="megamenu[menu_class]" value="<?php echo $menu['menu_class']?>" />
		...
		<tr>
			...
			<td class="col-sm-9"><select class="form-control" type="list"
				name="megamenu[show_title]">
					<?php foreach( $yesno as $key => $val ){ ?>
					<option value="<?php echo $key;?>"
						<?php if( $key==$menu['show_title']){ ?> selected="selected"
						<?php } ?>><?php echo $val; ?></option>
					<?php } ?>
				</select></td>
		</tr>
		<tr class="hiade">
			...
			<td class="col-sm-9"><select class="form-control" type="list"
				name="megamenu[is_group]" value="">
					<?php foreach( $yesno as $key => $val ){ ?>
					<option value="<?php echo $key;?>"
						<?php if( $key==$menu['is_group']){ ?> selected="selected"
						<?php } ?>><?php echo $val; ?></option>
					<?php } ?>
				</select> <i><?php echo $objlang->get('text_explain_group');?></i></td>
		</tr>
		<tr style="display: none">
			...
			<td class="col-sm-9"><select class="form-control" type="list"
				name="megamenu[is_content]">
					<?php foreach( $yesno as $key => $val ){ ?>
					<option value="<?php echo $key;?>"
						<?php if( $key==$menu['is_content']){ ?> selected="selected"
						<?php } ?>><?php echo $val; ?></option>
					<?php } ?>
				</select></td>
		</tr>
		...
		<tr class="hiade">
			...
			<td class="col-sm-9"><input type="text" class="form-control"
				name="megamenu[colums]" value="<?php echo $menu['colums']?>" /> <i><?php echo $objlang->get('text_explain_columns');?></i>
			</td>
		</tr>
		...
		<tr class="hide">
			...
			<td class="col-sm-9"><textarea type="text"
					name="megamenu[submenu_colum_width]" rows="5"><?php echo $menu['submenu_colum_width']?></textarea>
				<i><?php echo $objlang->get('text_explain_submenu_cols');?></i></td>
		</tr>
		<tr>
			<td class="col-sm-3"><?php echo $objlang->get("entry_sub_menutype");?></td>
			<td class="col-sm-9">
				<?php //echo '<pre>'.print_r( $menu,1 ); die ;?>
				<select class="form-control" name="megamenu[type_submenu]"
				id="megamenu-type_submenu">
					<?php foreach( $submenutypes as $stype => $text ) { ?>
					<option value="<?php echo $stype;?>"
						<?php if($stype==$menu['type_submenu']) { ?> selected="selected"
						<?php } ?>><?php echo $text;?></option>
					<?php } ?>
				</select> <i><?php echo $objlang->get('text_explain_submenu_type');?></i>
			</td>
		</tr>
		<tr class="type_submenu" id="type_submenu-html" style="display: none;">
			...
			<td class="col-sm-9"><textarea name="megamenu[submenu_content]"
					id="submenu_content"><?php echo $menu['submenu_content'];?></textarea>
		...
		<tr class="type_submenu" id="type_submenu-widget"
			style="display: none;">
			...
			<td class="col-sm-9">
				 ...
				 <select class="form-control" name="megamenu[widget_id]">
				 	...
				 	<option <?php if($t['id'] == $menu['widget_id']) { ?>
						selected="selected" <?php } ?> value="<?php echo $t['id']; ?>"><?php echo $t['name']; ?></option>
				 	<?php } ?>
				 </select>
				 <?php } ?>
			</td>
	...
	<input type="hidden" name="megamenu[megamenu_id]"
		value="<?php echo $menu['megamenu_id']?>" />
</div>
<?php endif; ?>
