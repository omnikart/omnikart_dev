...
<div id="content">
    ...
    <div class="panel panel-default">
          ...
          <li><a class="excelport_tab"
							href="#tab-<?php echo $tab['id']; ?>" data-toggle="tab"><?php echo $tab['name']; ?></a></li>
          <?php } ?>
        </ul>
...
jq(window).load(function() {
  ...
  var refreshOperation = function(field, type, predefine) {
    jq(field).children('option').attr('disabled', 'disabled');
    if (typeof(type) != 'undefined') {
      jq(field).closest('.hideable').show();
      
      if (type == 'text') jq(field).children('option[value^="text_"]').removeAttr('disabled');
      else if (type=='number') jq(field).children('option[value^="number_"]').removeAttr('disabled');
      else jq(field).children('option').removeAttr('disabled');
      
      if (predefine) {
        jq(field).children('option').removeAttr('selected');
        jq(field).children('option[disabled!="disabled"]:first').attr('selected', 'selected');
      }
    }
  }
