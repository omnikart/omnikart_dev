	...
	<div class="page-header">
		<div class="container-fluid">
			<div class="pull-right">
				<select id="partnet_status" class="form-control"
				...
				<button id="prs_action" type="submit" form="form-seller"
					formaction="<?php echo $approve; ?>" data-toggle="tooltip"
					title="<?php echo $button_approve; ?>" class="btn btn-success">
					<i class="fa fa-thumbs-o-up"></i>
				</button>
				<button type="button" data-toggle="tooltip"
					title="<?php echo $button_delete; ?>" class="btn btn-danger"
					onclick="confirm('<?php echo $text_confirm; ?>') ? $('#form-seller').submit() : false;">
			...
			<div class="panel-body">
				<div class="well">
						...
						<div class="col-sm-4">
							...
							<div class="form-group">
								...
								<input type="text" name="filter_email"
									...
									placeholder="<?php echo $column_email; ?>" id="input-model"
									class="form-control" />
							</div>
						</div>
						...
						<div class="col-sm-4">
							<div class="form-group">
								...
								<select name="filter_customer_group_id"
									id="input-customer_group" class="form-control">
									<option value="*"></option>
                  ...
                  <?php } ?>
						...
						<div class="col-sm-4">
							<div class="form-group">
								...
								<select name="filter_approved" id="input-approved"
									class="form-control">
									<option value="*"></option>
                  ...
                  <?php } ?>
							...
							<div class="form-group">
								<label class="control-label" for="input-approved"><span
									data-toggle="tooltip"
									title="<?php echo $entry_customer_type_info; ?>"><?php echo $entry_customer_type; ?></span></label>
								<select id="view_all" name="wk_viewall" class="form-control">
									...
									<option
										<?php if($wk_viewall AND $wk_viewall=='2'){ echo 'selected';} ?>
										value="2"><?php echo $text_view_requested; ?></option>
									<option
										<?php if($wk_viewall AND $wk_viewall=='1'){ echo 'selected';} ?>
										value="1"><?php echo $text_view_all; ?></option>
								</select>
							</div>
				...
				<form action="<?php echo $delete; ?>" method="post"
					enctype="multipart/form-data" id="form-seller">
					<div class="table-responsive">
						<table class="table table-bordered table-hover">
							<thead>
								<tr>
									<td width="1" style="text-align: center;" class="text-center"><input
										type="checkbox"
                  ...
                  <a href="<?php echo $sort_approved; ?>"
										class="<?php echo strtolower($order); ?>"><?php echo $column_approved; ?></a>
                  <?php } else { ?>
              ...
              <tr>
                  ...
                  <?php } ?></td>
									<td class="text-left"><a
										href="index.php?route=sale/customer/edit&token=<?php echo $token; ?>&customer_id=<?php echo $customer['customer_id']; ?>"><?php echo $customer['name']; ?></a></td>
									...
									<td class="text-left"><b><?php echo $customer['is_partner']?></b>
									</td>
                ...
                <td class="text-right">
                  <?php if ($customer['is_partner']=="Normal customer") { ; ?>
                  <a
										onclick="$(this).parents('tr').find('input[type=\'checkbox\']').prop('checked',true);$('form').attr('action', '<?php echo $approve; ?>&set_status=1'); $('form').submit();"
										data-toggle="tooltip" title="<?php echo $button_approve; ?>"
                  ...
                  <div class="btn-group" data-toggle="tooltip"
											...
											<ul class="dropdown-menu pull-right">
												<li><a
													href="index.php?route=sale/customer/login&token=<?php echo $token; ?>&customer_id=<?php echo $customer['customer_id']; ?>&store_id=0"
													target="_blank"><?php echo $text_default; ?></a></li>
                      <?php foreach ($stores as $store) { ?>
                      <li><a
													href="index.php?route=sale/customer/login&token=<?php echo $token; ?>&customer_id=<?php echo $customer['customer_id']; ?>&store_id=<?php echo $store['store_id']; ?>"
...
$('input[name=\'filter_email\']').autocomplete({
  ...
  source: function(request, response) {
    $.ajax({
      url: 'index.php?route=customerpartner/partner/autocomplete&token=<?php echo $token; ?>&filter_email=' +  encodeURIComponent(request.term)+'&filter_view=' +  jQuery('#view_all').val(),
      dataType: 'json',
      success: function(json) {   
        response($.map(json, function(item) {
          return {
