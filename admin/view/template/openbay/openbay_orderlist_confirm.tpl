...
<div id="content">
	...
	<div class="container-fluid">
		<div class="alert alert-warning"><?php echo $text_confirm_change_text; ?>: <strong><?php echo $status_mapped[$change_order_status_id]; ?></strong>
		</div>
		<form action="<?php echo $link_complete; ?>" method="post"
			...
			<table class="table table-bordered table-hover">
              ...
              <input type="checkbox" name="notify_all" id="notify_all"
							value="1" onchange="notifyAll();" /></td>
					</tr>
				</thead>
				<tbody>
					<input type="hidden" name="order_status_id"
						value="<?php echo $change_order_status_id; ?>" />
          <?php foreach ($orders as $order) { ?>
            <input type="hidden" name="order_id[]"
						value="<?php echo $order['order_id']; ?>" />
					<input type="hidden"
						name="old_status[<?php echo $order['order_id']; ?>]"
					...
					<tr>
						...
						<td class="text-left"><input type="hidden"
							name="channel[<?php echo $order['order_id']; ?>]"
                  ...
                  <?php if ($change_order_status_id == $ebay_status_shipped_id) { ?>
                    <div class="form-group">
                        ...
                        <?php foreach($market_options['ebay']['carriers'] as $carrier) { ?>
                          <option
										<?php echo ($carrier['description'] == $order['shipping_method'] ? ' selected' : ''); ?>><?php echo $carrier['description']; ?></option>
                        <?php } ?>
                      </select>
							</div>
							<div class="form-group">
								...
								<input type="text"
									name="tracking[<?php echo $order['order_id']; ?>]" value=""
									placeholder="<?php echo $entry_tracking_no; ?>"
									class="form-control" />
							</div>
                  <?php } ?>
                <?php } ?>
                    ...
                    <div class="form-group">
                        ...
                        <option value="other"><?php echo $text_other; ?></option>
							...
							<div class="form-group">
								...
								<input type="text"
									name="carrier_other[<?php echo $order['order_id']; ?>]"
									value="" placeholder="<?php echo $entry_other; ?>"
									class="form-control"
                ...
                <?php } ?>
						...
						<td class="text-left"><textarea
								name="comments[<?php echo $order['order_id']; ?>]"
								class="form-control" rows="3"></textarea></td>
						<td class="text-center"><input type="hidden"
							name="notify[<?php echo $order['order_id']; ?>]" value="0" /> <input
							type="checkbox" name="notify[<?php echo $order['order_id']; ?>]"
							class="notify_checkbox" value="1" /></td>
					</tr>
          <?php } ?>
        </tbody>
			</table>
