...
<div id="content">
    ...
    <div class="alert alert-info"><?php echo $text_load_listings; ?></div>
		<div class="well">
			<div class="row">
				<div class="col-sm-12">
					<div class="form-group">
						...
						<select name="marketplace_select" id="marketplace-select"
							class="form-control">
              <?php foreach ($marketplaces as $marketplace) { ?>
              <option value="<?php echo $marketplace['link']; ?>"
								<?php if ($marketplace['code'] == $marketplace_code) { echo ' selected'; } ?>><?php echo $marketplace['name'] ?></option>
              <?php } ?>
            </select>
					</div>
				</div>
          ...
          <div class="pull-right">
						<a class="btn btn-primary" disabled="disabled"><i
							class="fa fa-cog fa-lg fa-spin"></i> <?php echo $text_loading; ?></a>
					</div>
          <?php } else { ?>
          <div class="pull-right">
						<a id="button-load-listings" class="btn btn-primary"
					...
					<thead>
						...
						<tr>
							<th class="text-center"><input type="checkbox"
								id="master-checkbox" value="<?php echo $marketplace['code'] ?>" /></th>
                ...
                <?php } else { ?>
						...
						<tr class="success">
							<td class="text-center"><input type="checkbox"
								class="link-checkbox link-checkbox-<?php echo $marketplace['code'] ?>" /></td>
                <?php } ?>

                  <td class="text-left"><a
								href="<?php echo $product['href_amazon'] ?>" target="_blank"><?php echo $product['asin'] ?></a></td>
							...
							<td class="text-left"><a
								href="<?php echo $product['href_product'] ?>" target="_blank"><?php echo $product['name'] ?></a><?php echo (!empty($product['combination']) ? '<br />' . $product['combination'] : ''); ?></td>
				...
				<div class="well">
					<div class="row">
						<div class="col-sm-12 text-right">
							<div class="pull-right">
								<a id="link-button" class="btn btn-primary"
									href="<?php echo $button_save; ?>" data-toggle="tooltip"
									title="<?php echo $button_load ?>"><i class="fa fa-save"></i></a>
							</div>
						</div>
					</div>
				</div>
