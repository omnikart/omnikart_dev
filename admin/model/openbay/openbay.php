...
class ModelOpenbayOpenbay extends Model {
	...
	public function updateV2CheckVersion($beta = 0) {
		...
		if ($this->lasterror == true) {
			...
			if ($data ['version'] > $current_version) {
				...
				return array (
						'error' => 1,
						'response' => $this->language->get ( 'text_version_ok' ) . $current_version 
				);
			}
		}
	}
	...
	public function updateTest() {
		...
		if (empty ( $data ['user'] )) {
			return array (
					'connection' => false,
					'msg' => $this->language->get ( 'error_username' ) 
			);
		}
		if (empty ( $data ['pw'] )) {
			return array (
					'connection' => false,
					'msg' => $this->language->get ( 'error_password' ) 
			);
		}
		if (empty ( $data ['server'] )) {
			return array (
					'connection' => false,
					'msg' => $this->language->get ( 'error_server' ) 
			);
		}
		...
		if ($connection != false) {
			if (@ftp_login ( $connection, $data ['user'], $data ['pw'] )) {
				...
				if ($folder_error_admin == true) {
					return array (
							'connection' => false,
							'msg' => $this->language->get ( 'error_no_admin' ) 
					);
				} else {
					if ($folder_error == true) {
						return array (
								'connection' => false,
								'msg' => $this->language->get ( 'error_no_files' ),
								'dir' => json_encode ( $directory_list ) 
						);
					} else {
						return array (
								'connection' => true,
								'msg' => $this->language->get ( 'text_connection_ok' ) 
						);
					}
				}
			} else {
				return array (
						'connection' => false,
						'msg' => $this->language->get ( 'error_ftp_login' ) 
				);
			}
		} else {
			return array (
					'connection' => false,
					'msg' => $this->language->get ( 'error_ftp_connect' ) 
			);
		}
	}
	public function update() {
		...
		if (empty ( $data ['adminDir'] )) {
			return array (
					'connection' => false,
					'msg' => $this->language->get ( 'error_admin' ) 
			);
		}
		...
		if ($connection != false) {
			...
			if (@ftp_login ( $connection, $data ['user'], $data ['pw'] )) {
				...
				return array (
						'connection' => true,
						'msg' => sprintf ( $this->language->get ( 'text_updated' ), $files_update ['version'] ),
