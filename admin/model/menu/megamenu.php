...
class ModelMenuMegamenu extends Model {
	...
	public function getInfo($id) {
		$sql = ' SELECT m.*, md.title,md.description FROM ' . DB_PREFIX . 'megamenu m LEFT JOIN ' . DB_PREFIX . 'megamenu_description md ON m.megamenu_id=md.megamenu_id AND language_id=' . ( int ) $this->config->get ( 'config_language_id' );
		...
		if (isset ( $data ["megamenu_description"] )) {
			...
			foreach ( $languages as $language ) {
				$sql = "INSERT INTO " . DB_PREFIX . "megamenu_description(`language_id`, `megamenu_id`,`title`,`description`) 
							VALUES(" . $language ['language_id'] . ",'" . $data ['megamenu'] ['megamenu_id'] . "','" . $this->db->escape ( $data ["megamenu_description"] [$language ['language_id']] ['title'] ) . "','" . $this->db->escape ( $data ["megamenu_description"] [$language ['language_id']] ['description'] ) . "') ";
				$this->db->query ( $sql );
			}
		}
