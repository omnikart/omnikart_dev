...
class ModelModuleExcelport extends Model {
	...
	public function __construct($register) {
		...
		if (! defined ( 'IMODULE_SERVER_NAME' ))
			define ( 'IMODULE_SERVER_NAME', substr ( (defined ( 'HTTP_CATALOG' ) ? HTTP_CATALOG : HTTP_SERVER), 7, strlen ( (defined ( 'HTTP_CATALOG' ) ? HTTP_CATALOG : HTTP_SERVER) ) - 8 ) );
	...
	public function getStandardFile($file, $arrayName, $indexName) {
		...
		if ($file ['size'] [$arrayName] [$indexName] <= $this->returnMaxUploadSize () && in_array ( $extension, $allowedExts )) { // file limit = post_max_size - 512KB
			if ($file ['error'] [$arrayName] [$indexName] > 0)
				throw new Exception ( "Upload Error Code: " . $file ['error'] [$arrayName] [$indexName] );
			$dest = IMODULE_ROOT . IMODULE_TEMP_FOLDER . '/' . $name;
			if (! move_uploaded_file ( $file ['tmp_name'] [$arrayName] [$indexName], $dest ))
	...
	public function createDownload($file, $die = true) {
		$this->cleanTemp ( '../' . IMODULE_TEMP_FOLDER, array (
				basename ( $file ) 
		) );
