...
class ModelModuleExcelportproduct extends ModelModuleExcelport {
	public function importXLSProductsFull($language, &$allLanguages, $file, $importLimit, $addAsNew = false) {
		...
		if (! is_numeric ( $importLimit ) || $importLimit < 10 || $importLimit > 800)
			throw new Exception ( $this->language->get ( 'excelport_import_limit_invalid' ) );
		...
		$chunkFilter = new CustomReadFilter ( array (
				"Products" => array (
						'A',
						($this->productSize * $progress ['importedCount'] + 2),
						'Q',
						($this->productSize * ($progress ['importedCount'] + $importLimit) + 1) 
				),
				"products" => array (
						'A',
						($this->productSize * $progress ['importedCount'] + 2),
		...
		$source = array (
				0,
				2 + $this->productSize * ($progress ['importedCount']) 
		);
		...
		foreach ( $attributes as $attribute ) {
			...
			$name = $attribute ['name'];
			$attributeSortOrder = $attribute ['sort_order'];
		...
		foreach ( $attributeGroups as $attributeGroup ) {
			if (! isset ( $attributeGroupIds [$attributeGroup ['name']] )) {
				$attributeGroupIds [$attributeGroup ['name']] = $attributeGroup ['attribute_group_id'];
			}
			if ($maxAttributeGroupId < $attributeGroup ['attribute_group_id']) {
				$maxAttributeGroupId = $attributeGroup ['attribute_group_id'];
			}
			if ($maxAttributeGroupSortOrder < $attributeGroup ['sort_order']) {
				$maxAttributeGroupSortOrder = $attributeGroup ['sort_order'];
			}
		}
		...
		do {
			...
			if ((empty ( $safe_mode ) || strtolower ( $safe_mode ) == 'off') && function_exists ( 'set_time_limit' ) && stripos ( ini_get ( 'disable_functions' ), 'set_time_limit' ) === FALSE)
				set_time_limit ( 60 );
			...
			if (! empty ( $product_name )) {
				...
				$product_price = ( float ) str_replace ( array (
						' ',
						',' 
				), array (
						'',
				...
				$product_parent = $productSheetObj->getCell ( PHPExcel_Cell::stringFromColumnIndex ( $source [0] + $map ['gp_parent_id'] [0] ) . ($source [1] + $map ['gp_parent_id'] [1]) )->getValue ();
				...
				$product_length = ( float ) str_replace ( array (
						' ',
						',' 
				), array (
						'',
				...
				$product_width = ( float ) str_replace ( array (
						' ',
						',' 
				), array (
						'',
				...
				$product_height = ( float ) str_replace ( array (
						' ',
						',' 
				), array (
						'',
				...
				$product_weight = ( float ) str_replace ( array (
						' ',
						',' 
				), array (
						'',
				...
				$product_type = $productSheetObj->getCell ( PHPExcel_Cell::stringFromColumnIndex ( $source [0] + $map ['type'] [0] ) . ($source [1] + $map ['type'] [1]) )->getValue ();
				...
				foreach ( $categories as $category ) {
					...
					if (! empty ( $category ))
						$product_category [] = trim ( $category );
				}
				...
				if (version_compare ( VERSION, '"show"', '>=' )) {
					...
					foreach ( $recurring_customer_groups as $recurring_customer_group ) {
						...
						if (! empty ( $recurring_customer_group [0] ) && ! empty ( $recurring_customer_group [1] ))
							$product_recurring [] = array (
				...
				foreach ( $downloads as $download ) {
					...
					if (! empty ( $download ))
						$product_download [] = trim ( $download );
				}
				...
				while ( ! empty ( $attributeName ) ) {
					...
					if (! empty ( $attributeName [1] )) {
						if (! empty ( $attributeName [0] )) {
							...
							$attribute_group_name = trim ( $attributeName [0] );
							
							if (! isset ( $attributeGroupIds [$attribute_group_name] )) {
								$maxAttributeGroupId += 1;
								$maxAttributeGroupSortOrder += 1;
								$attributeGroupId = $maxAttributeGroupId;
								$attributeGroupIds [$attribute_group_name] = $attributeGroupId;
								...
								$sql .= "($attributeGroupId,$language,'" . $this->db->escape ( $attribute_group_name ) . "');";
								$this->db->query ( $sql );
							}
							if (! isset ( $attributeIds [$attribute_group_name] )) {
								$attributeIds [$attribute_group_name] = array ();
							}
						}
						
						if (! isset ( $attributeIds [$attribute_group_name] [$attributeName [1]] )) {
							$maxAttributeId += 1;
							$attributeId = $maxAttributeId;
							$attributeIds [$attribute_group_name] [$attributeName [1]] = $attributeId;
							$attributeGroupId = $attributeGroupIds [$attribute_group_name];
							if (! isset ( $maxAttributeSortOrders [$attribute_group_name] )) {
								$maxAttributeSortOrders [$attribute_group_name] = 0;
							}
							$maxAttributeSortOrders [$attribute_group_name] += 1;
							$attributeSortOrder = $maxAttributeSortOrders [$attribute_group_name];
							...
							$sql .= "($attributeId,$language,'" . $this->db->escape ( $attributeName [1] ) . "');";
							$this->db->query ( $sql );
						}
						$product_attribute [] = array (
								'name' => $attributeName [1],
								'attribute_id' => $attributeIds [$attribute_group_name] [$attributeName [1]],
					...
					/*
					 * if (!empty($attribute_group_name) && !empty($attributeName[1])) {
					 * foreach ($attributes as $attribute) {
					 * foreach ($attributeGroups as $attributeGroup) {
					 * if ($attribute['name'] == $attributeName[1] && $attributeGroup['name'] == $attribute_group_name && $attribute['attribute_group_id'] == $attributeGroup['attribute_group_id']) {
					 * $product_attribute[] = array(
					 * 'name' => $attributeName[1],
					...
					if (! empty ( $optionValue )) {
						...
						if ($isParent) {
							...
							if (! empty ( $product_option [$j] ['option_id'] )) {
								if (in_array ( $product_option [$j] ['type'], array (
										'radio',
										'checkbox',
										'select',
										'image' 
									...
									foreach ( $option_values as $option_value ) {
										if ($option_value ['name'] == $optionValue) {
											...
											$product_option_value_price = ( float ) str_replace ( array (
													'+',
													'-',
													' ',
													',' 
											...
											$product_option_value_points = ( float ) str_replace ( array (
													'+',
													'-',
													' ',
													',' 
											...
											$product_option_value_weight = ( float ) str_replace ( array (
													'+',
													'-',
													' ',
													',' 
					...
					foreach ( $customer_groups as $customer_group ) {
						if ($customer_group ['name'] == $discountCustomerGroupName) {
							$product_discount [] = array (
									...
									'price' => ( float ) trim ( str_replace ( array (
											' ',
											',' 
									), array (
											'',
											'.' 
									), $productSheetObj->getCell ( PHPExcel_Cell::stringFromColumnIndex ( $source [0] + $map ['discount'] [0] + $i ) . ($source [1] + $map ['discount'] [1] + 2) )->getValue () ) ),
						...
						if ($customer_group ['name'] == $specialCustomerGroupName) {
							$product_special [] = array (
									...
									'price' => ( float ) trim ( str_replace ( array (
											...
											'',
											'.' 
									), $productSheetObj->getCell ( PHPExcel_Cell::stringFromColumnIndex ( $source [0] + $map ['special'] [0] + $i ) . ($source [1] + $map ['special'] [1] + 2) )->getValue () ) ),
				...
				if (! (($product_type == '2') || ($product_type == 'Group') || ($product_type == 'group'))) {
					if (! (empty ( $product_parent ) || ($product_parent == '0'))) {
						$product ['gp_parent_id'] = $product_id_map [$product_parent];
					}
				}
				...
				if (! empty ( $product_id )) {
					...
					if ($exists) {
						...
						$last_product_id = $this->addProduct ( '', $product, $allLanguages );
					}
				} else {
					$last_product_id = $this->addProduct ( '', $product, $allLanguages );
				}
				
				$product_id_map [$product_model] = $last_product_id;
	...
	public function importXLSProductsLight($language, &$allLanguages, $file, $importLimit, $addAsNew = false) {
		...
		$chunkFilter = new CustomReadFilter ( array (
				"Products" => array (
						'A',
						($progress ['importedCount'] + 2),
						'AM',
						(($progress ['importedCount'] + $importLimit) + 1) 
				),
				"products" => array (
						'A',
						($progress ['importedCount'] + 2),
		...
		do {
			...
			if (! empty ( $product_name )) {
				...
				$type = $productSheetObj->getCell ( PHPExcel_Cell::stringFromColumnIndex ( $source [0] + $map ['type'] ) . ($source [1]) )->getValue ();
				$found = false;
				foreach ( $product_length_classes as $product_length_class ) {
	...
	public function exportXLSProductsFull($language, $store, $destinationFolder = '', $productNumber, $export_filters = array()) {
		...
		$taxClassesStart = array (
				0,
				2 
		);
		$this->load->model ( 'localisation/tax_class' );
		$taxClasses = array_merge ( array (
				0 => array (
						'tax_class_id' => 0,
						'title' => '--- None ---',
						'description' => '--- None ---',
		...
		$dataValidations = array (
				array (
						...
						'data' => array (
								...
								$taxClassesStart [1],
								$taxClassesStart [0],
								$taxClassesStart [1] + count ( $taxClasses ) - 1 
						),
						'range' => '',
				...
				array (
						...
						'data' => array (
								...
								$stockStatesStart [1],
								$stockStatesStart [0],
								$stockStatesStart [1] + count ( $stockStates ) - 1 
						),
						'range' => '',
				...
				array (
						...
						'data' => array (
								$lengthClassesStart [0],
								$lengthClassesStart [1],
								$lengthClassesStart [0],
								$lengthClassesStart [1] + count ( $lengthClasses ) - 1 
						),
						'range' => '',
				...
				array (
						...
						'data' => array (
								$weightClassesStart [0],
								$weightClassesStart [1],
								$weightClassesStart [0],
								$weightClassesStart [1] + count ( $weightClasses ) - 1 
						),
						'range' => '',
				...
				array (
						...
						'data' => array (
								$manufacturersStart [0],
								$manufacturersStart [1],
								$manufacturersStart [0],
								$manufacturersStart [1] + count ( $manufacturers ) - 1 
						),
						'range' => '',
		...
		$leftColumnStaticTextColumn1 = array (
				'General',
		...
		$leftColumnStaticTextColumn3 = array (
				'Attribute',
				null,
				'Option',
				null,
		...
		$leftColumnStaticTextColumn4 = array (
				'Attribute Name',
				'Attribute Text',
				'Option Value',
				'Quantity',
		...
		for($i = 0; $i < count ( $leftColumnStaticTextColumn1 ); $i ++) {
			$leftColumnStaticText [$i + 1] = array (
					$leftColumnStaticTextColumn1 [$i],
					$leftColumnStaticTextColumn2 [$i],
					$leftColumnStaticTextColumn3 [$i],
					$leftColumnStaticTextColumn4 [$i] 
			);
		}
		...
		$objPHPExcel->getProperties ()->setCreator ( $this->user->getUserName () )->setLastModifiedBy ( $this->user->getUserName () )->setTitle ( $name )->setSubject ( $name )->setDescription ( "Backup for Office 2007 and later, generated using PHPExcel and ExcelPort." )->setKeywords ( "office 2007 2010 2013 xlsx openxml php phpexcel excelport" )->setCategory ( "Backup" );
		...
		if ($products->num_rows > 0) {
			foreach ( $products->rows as $myProductIndex => $row ) {
				...
				foreach ( $taxClasses as $taxClass ) {
					if ($taxClass ['tax_class_id'] == $row ['tax_class_id']) {
						$row ['tax_class'] = $taxClass ['title'];
						break;
					}
				}
				if (empty ( $row ['tax_class'] ))
					$row ['tax_class'] = $taxClasses [0] ['title'];
				$row ['subtract'] = empty ( $row ['subtract'] ) ? 'No' : 'Yes';
				foreach ( $stockStates as $stockStatus ) {
					if ($stockStatus ['stock_status_id'] == $row ['stock_status_id']) {
						$row ['stock_status'] = $stockStatus ['name'];
					}
				...
				if (empty ( $row ['stock_status'] ))
					$row ['stock_status'] = $defaultStockStatus;
				...
				foreach ( $lengthClasses as $lengthClass ) {
					if ($lengthClass ['length_class_id'] == $row ['length_class_id']) {
						$row ['length_class'] = $lengthClass ['title'];
						break;
					}
				}
				$row ['weight_class'] = ! empty ( $weightClasses [0] ['title'] ) ? $weightClasses [0] ['title'] : '';
				foreach ( $weightClasses as $weightClass ) {
					if ($weightClass ['weight_class_id'] == $row ['weight_class_id']) {
						$row ['weight_class'] = $weightClass ['title'];
						break;
					}
				}
				...
				foreach ( $manufacturers as $manufacturer ) {
					if ($manufacturer ['manufacturer_id'] == $row ['manufacturer_id']) {
						$row ['manufacturer'] = $manufacturer ['name'];
						break;
					}
				}
				if (empty ( $row ['manufacturer'] ))
					$row ['manufacturer'] = $manufacturers [0] ['name'];
				if (empty ( $row ['filters'] ))
					$row ['filters'] = '';
				if (empty ( $row ['recurrings'] ))
					$row ['recurrings'] = '';
				if (empty ( $row ['ean'] ))
					$row ['ean'] = '';
				if (empty ( $row ['jan'] ))
				...
				$attributesRange = array (
						PHPExcel_Cell::stringFromColumnIndex ( $target [0] + $i3 ) . ($target [1] + $dynamicTemplates ['attributes'] [1]),
						'' 
				);
				...
				$optionsRange = array (
						PHPExcel_Cell::stringFromColumnIndex ( $target [0] + $i3 ) . ($target [1] + $dynamicTemplates ['option_types'] [1]),
						'' 
				);
					...
					if (! empty ( $productOption ['product_option_value'] ) && is_array ( $productOption ['product_option_value'] )) {
						foreach ( $productOption ['product_option_value'] as $product_option_value ) {
							...
							if (version_compare ( VERSION, '1.5.5', '>=' )) {
								...
								if (empty ( $productOptionValue ['name'] ))
									continue;
				...
				$customerGroupRange = array (
						PHPExcel_Cell::stringFromColumnIndex ( $target [0] + $i3 ) . ($target [1] + $dynamicTemplates ['discounts'] [1]),
						'' 
				);
				...
				$customerGroupRange = array (
						PHPExcel_Cell::stringFromColumnIndex ( $target [0] + $i3 ) . ($target [1] + $dynamicTemplates ['specials'] [1]),
						'' 
				);
				...
				$imagePathRange = array (
						PHPExcel_Cell::stringFromColumnIndex ( $target [0] + $i3 ) . ($target [1] + $dynamicTemplates ['images'] [1]),
						'' 
				);
						...
						for($j = 0; $j <= $dynamicTemplates ['reward_points'] [3] - $dynamicTemplates ['reward_points'] [1]; $j ++) {
							...
							if ($j == 0) {
								if (($i3 - $dynamicTemplates ['reward_points'] [0]) == 1) {
									$customerGroupRange = array (
											PHPExcel_Cell::stringFromColumnIndex ( $target [0] + $i3 ) . ($target [1] + $dynamicTemplates ['reward_points'] [1] + $j),
											'' 
									);
					...
					for($j = 0; $j <= $dynamicTemplates ['designs'] [3] - $dynamicTemplates ['designs'] [1]; $j ++) {
						if ($j == 0) {
							...
							if (empty ( $designLayoutRange )) {
								$designLayoutRange = array (
										PHPExcel_Cell::stringFromColumnIndex ( $target [0] + $i3 ) . ($target [1] + $dynamicTemplates ['designs'] [1] + $j),
										'' 
								);
							}
							
							if (empty ( $designLayoutTemplateRange )) {
								$designLayoutTemplateRange = array (
										PHPExcel_Cell::stringFromColumnIndex ( $template [0] + $i3 ) . ($template [1] + $dynamicTemplates ['designs'] [1] + $j),
										'' 
								);
							}
				...
				foreach ( $dataValidations as $dataValidationIndex => $dataValidation ) {
					if (isset ( $dataValidations [$dataValidationIndex] ['count'] ) && $dataValidations [$dataValidationIndex] ['count'] == 0)
						continue;
					...
					if (empty ( $dataValidations [$dataValidationIndex] ['root'] ))
						$dataValidations [$dataValidationIndex] ['root'] = PHPExcel_Cell::stringFromColumnIndex ( $target [0] + $dataValidation ['field'] [0] ) . ($target [1] + $dataValidation ['field'] [1]);
				}
		...
		if ($products->num_rows > 0) {
			foreach ( $products->rows as $myProductIndex => $row ) {
				...
				foreach ( $dataValidations as $dataValidationIndex => $dataValidation ) {
					...
					if (empty ( $dataValidations [$dataValidationIndex] ['root'] ))
						$dataValidations [$dataValidationIndex] ['root'] = PHPExcel_Cell::stringFromColumnIndex ( $target [0] + $dataValidation ['field'] ) . ($target [1]);
				}
			...
			foreach ( $dataValidations as $dataValidation ) {
				$range = trim ( $dataValidation ['range'] );
				if (isset ( $dataValidation ['count'] ) && $dataValidation ['count'] == 0)
					continue;
				if ($dataValidation ['type'] == 'list' && ! empty ( $dataValidation ['root'] ) && ! empty ( $range )) {
					$objValidation = $productSheetObj->getCell ( $dataValidation ['root'] )->getDataValidation ();
					$objValidation->setType ( PHPExcel_Cell_DataValidation::TYPE_LIST );
	...
	public function getQuery($filters = array(), $store = 0, $language = 1, $count = false) {
		if (empty ( $filters ) || ! in_array ( $filters ['Conjunction'], array (
				'AND',
				'OR' 
		) ))
		...
		$joins ['gp_product'] = "LEFT JOIN " . DB_PREFIX . "gp_grouped gpp ON (p.product_id=gpp.product_id)";
		$joins ['gp_grouped_child'] = "LEFT JOIN " . DB_PREFIX . "gp_grouped_child gpc ON (p.product_id=gpc.child_id)";
				...
				if (! is_array ( $this->conditions ['Products'] [$filter ['Field']] ['field_name'] )) {
					if ($this->conditions ['Products'] [$filter ['Field']] ['type'] !== 'not_exists') {
						$condition = str_replace ( array (
								...
								$this->conditions ['Products'] [$filter ['Field']] ['field_name'],
								stripos ( $this->conditions ['Products'] [$filter ['Field']] ['type'], 'number' ) !== FALSE ? ( int ) $this->db->escape ( $filter ['Value'] ) : $this->db->escape ( $filter ['Value'] ) 
						), $this->operations [$filter ['Condition']] ['operation'] );
						if (in_array ( $this->conditions ['Products'] [$filter ['Field']] ['field_name'], array (
								'pdis.date_start',
								'pdis.date_end',
								'pspe.date_start',
								'pspe.date_end' 
						...
						if (! in_array ( $alter_joins_array_entry, $alter_joins_array ))
							$alter_joins_array [] = $alter_joins_array_entry;
					}
				} else {
					$sub_conditions = array ();
					foreach ( $this->conditions ['Products'] [$filter ['Field']] ['field_name'] as $field_name ) {
						$sub_conditions [] = str_replace ( array (
								'{FIELD_NAME}',
								'{WORD}' 
						), array (
								$field_name,
		...
		$select = $count ? "COUNT(*)" : "*, pd.name as name, pd.description as description, pd.meta_description as meta_description, pd.meta_keyword as meta_keyword, pd.meta_title as meta_title, gpp.gp_price_min as minprice, gpp.gp_price_max as maxprice, gpp.gp_template as template, (SELECT p2.model FROM " . DB_PREFIX . "product p2 WHERE (gpc.product_id=p2.product_id)) as parent_id, " . (version_compare ( VERSION, '1.5.4', '<' ) ? 'pt.tag as tag' : 'pd.tag as tag') . ", p.*";
	...
	public function addProduct($product_id = '', $data, &$allLanguages) {
		...
		if ($data ['type'] == '2' || $data ['type'] == 'group' || $data ['type'] == 'GROUP' || $data ['type'] == 'Group') {
			$this->db->query ( "DELETE FROM `" . DB_PREFIX . "gp_grouped` WHERE product_id = " . ( int ) $product_id );
			
			$this->db->query ( "INSERT INTO `" . DB_PREFIX . "gp_grouped` (`product_id`, `gp_price_min`, `gp_price_max`, `gp_template`) VALUES (" . ( int ) $product_id . ",'0','0','default')" );
		}
		
		if (isset ( $data ['gp_parent_id'] )) {
			$this->db->query ( "INSERT INTO `" . DB_PREFIX . "gp_grouped_child`  (`product_id`,`child_id`,`child_sort_order`) VALUES (" . ( int ) $data ['gp_parent_id'] . "," . ( int ) $product_id . ",0)" );
		}
		// Extras
		foreach ( $this->extraGeneralFields ['Products'] as $extra ) {
	...
	public function editProduct($product_id, $data, &$languages, $light = false) {
		...
		if (! $light) {
			...
			if (isset ( $data ['gp_parent_id'] )) {
				$this->db->query ( "DELETE FROM `" . DB_PREFIX . "gp_grouped_child` WHERE child_id = " . ( int ) $product_id );
				$this->db->query ( "INSERT INTO `" . DB_PREFIX . "gp_grouped_child`  (`product_id`,`child_id`,`child_sort_order`) VALUES (" . ( int ) $data ['gp_parent_id'] . "," . ( int ) $product_id . ",0)" );
			}
		}
