			...
			if (is_array ( $filter )) {
				...
				if (! is_array ( $this->conditions ['CustomerGroups'] [$filter ['Field']] ['field_name'] )) {
					$condition = str_replace ( array (
							...
							$this->conditions ['CustomerGroups'] [$filter ['Field']] ['field_name'],
							stripos ( $this->conditions ['CustomerGroups'] [$filter ['Field']] ['type'], 'number' ) !== FALSE ? ( int ) $this->db->escape ( $filter ['Value'] ) : $this->db->escape ( $filter ['Value'] ) 
