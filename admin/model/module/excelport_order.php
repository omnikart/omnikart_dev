...
class ModelModuleExcelportorder extends ModelModuleExcelport {
		...
		do {
			...
			if ($order_store_id !== '') {
				...
				$order = array (
						...
						'total' => ( float ) str_replace ( array (
								' ',
								',' 
						), array (
								'',
								'.' 
						), strval ( trim ( $orderSheetObj->getCell ( PHPExcel_Cell::stringFromColumnIndex ( $source [0] + $map ['total'] ) . ($source [1]) )->getValue () ) ) ),
						...
						'commission' => ( float ) str_replace ( array (
								' ',
								',' 
						), array (
								'',
								'.' 
						), strval ( trim ( $orderSheetObj->getCell ( PHPExcel_Cell::stringFromColumnIndex ( $source [0] + $map ['commission'] ) . ($source [1]) )->getValue () ) ) ),
						...
						'currency_value' => ( float ) str_replace ( array (
								' ',
								',' 
						), array (
								'',
								'.' 
						), strval ( trim ( $orderSheetObj->getCell ( PHPExcel_Cell::stringFromColumnIndex ( $source [0] + $map ['currency_value'] ) . ($source [1]) )->getValue () ) ) ),
	...
	public function exportXLSOrders($destinationFolder = '', $orderNumber, $export_filters = array()) {
		...
		$dataValidations = array (
				array (
						...
						'data' => array (
								...
								$countriesStart [1],
								$countriesStart [0],
								$countriesStart [1] + count ( $countries ) - 1 
						),
						'range' => '' 
				) 
		);
		...
		$target_histories = array (
				0,
				2 
		);
		$target_vouchers = array (
				0,
				2 
		);
			...
			if (is_array ( $filter )) {
				...
				if (! is_array ( $this->conditions ['Orders'] [$filter ['Field']] ['field_name'] )) {
					$condition = str_replace ( array (
							...
							$this->conditions ['Orders'] [$filter ['Field']] ['field_name'],
							stripos ( $this->conditions ['Orders'] [$filter ['Field']] ['type'], 'number' ) !== FALSE ? ( int ) $this->db->escape ( $filter ['Value'] ) : $this->db->escape ( $filter ['Value'] ) 
