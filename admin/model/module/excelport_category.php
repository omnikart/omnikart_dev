		...
		if ($categories_result->num_rows > 0) {
			foreach ( $categories_result->rows as $myCategoriesIndex => $category_row ) {
				...
				foreach ( $categoriesDataValidations as $dataValidationIndex => $dataValidation ) {
					if (isset ( $categoriesDataValidations [$dataValidationIndex] ['count'] ) && $categoriesDataValidations [$dataValidationIndex] ['count'] == 0)
						continue;
					...
					if (empty ( $categoriesDataValidations [$dataValidationIndex] ['root'] ))
						$categoriesDataValidations [$dataValidationIndex] ['root'] = PHPExcel_Cell::stringFromColumnIndex ( $categories_target [0] + $dataValidation ['field'] ) . ($categories_target [1]);
				}
		...
		foreach ( $filters as $i => $filter ) {
			if (is_array ( $filter )) {
				...
				$condition = str_replace ( array (
						...
						$this->conditions ['Categories'] [$filter ['Field']] ['field_name'],
						stripos ( $this->conditions ['Categories'] [$filter ['Field']] ['type'], 'number' ) !== FALSE ? ( int ) $this->db->escape ( $filter ['Value'] ) : $this->db->escape ( $filter ['Value'] ) 
