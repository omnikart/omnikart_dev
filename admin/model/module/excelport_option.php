	...
	public function importXLSOptions($language, &$allLanguages, $file) {
		...
		do {
			...
			if (! empty ( $option_name ) && ! empty ( $option_type )) {
				...
				if (array_key_exists ( $option_type, $types ))
	...
	public function exportXLSOptions($language, $destinationFolder = '', $optionNumber = 800, $export_filters = array()) {
		...
		if ($options_result->num_rows > 0) {
			foreach ( $options_result->rows as $myOptionsIndex => $option_row ) {
				...
				if (empty ( $option_row ['name'] ))
					$option_row ['name'] = '-';
				if (in_array ( $option_row ['type'], $typesKeys ))
					$option_row ['type'] = $types [$option_row ['type']];
				...
				foreach ( $optionsDataValidations as $dataValidationIndex => $dataValidation ) {
					if (isset ( $optionsDataValidations [$dataValidationIndex] ['count'] ) && $optionsDataValidations [$dataValidationIndex] ['count'] == 0)
						continue;
					...
					if (empty ( $optionsDataValidations [$dataValidationIndex] ['root'] ))
						$optionsDataValidations [$dataValidationIndex] ['root'] = PHPExcel_Cell::stringFromColumnIndex ( $options_target [0] + $dataValidation ['field'] ) . ($options_target [1]);
				}
		...
		foreach ( $filters as $i => $filter ) {
			if (is_array ( $filter )) {
				...
				$condition = str_replace ( array (
						...
						$this->conditions ['Options'] [$filter ['Field']] ['field_name'],
						stripos ( $this->conditions ['Options'] [$filter ['Field']] ['type'], 'number' ) !== FALSE ? ( int ) $this->db->escape ( $filter ['Value'] ) : $this->db->escape ( $filter ['Value'] ) 
