		...
		do {
			...
			if (! empty ( $customer_email ) && ! empty ( $customer_id )) {
				...
				if (! empty ( $customer_history ))
					$customer_history = json_decode ( $customer_history, true );
				else
				...
				if (! empty ( $customer_reward_points ))
					$customer_reward_points = json_decode ( $customer_reward_points, true );
				else
					$customer_reward_points = array ();
				...
				if (! empty ( $customer_ip_addresses ))
					$customer_ip_addresses = json_decode ( $customer_ip_addresses, true );
				else
					$customer_ip_addresses = array ();
	...
	public function exportXLSCustomers($language, $store, $destinationFolder = '', $customerNumber, $export_filters = array()) {
		...
		$dataValidations = array (
				...
				array (
						...
						'data' => array (
								$customerGroupsStart [0],
								$customerGroupsStart [1],
								$customerGroupsStart [0],
								$customerGroupsStart [1] + count ( $customerGroups ) - 1 
						),
						'range' => '',
		...
		if ($customers->num_rows > 0) {
			foreach ( $customers->rows as $myCustomerIndex => $row ) {
				...
				foreach ( $customerGroups as $customerGroup ) {
					if ($customerGroup ['customer_group_id'] == $row ['customer_group_id']) {
						$row ['customer_group'] = $customerGroup ['name'];
					}
					if ($customerGroup ['customer_group_id'] == $this->config->get ( 'config_customer_group_id' )) {
						$defaultCustomerGroup = $customerGroup ['name'];
					}
				}
				if (empty ( $row ['customer_group'] ))
					$row ['customer_group'] = $defaultCustomerGroup;
			...
			if (is_array ( $filter )) {
				...
				if (! is_array ( $this->conditions ['Customers'] [$filter ['Field']] ['field_name'] )) {
					$condition = str_replace ( array (
							...
							$this->conditions ['Customers'] [$filter ['Field']] ['field_name'],
							stripos ( $this->conditions ['Customers'] [$filter ['Field']] ['type'], 'number' ) !== FALSE ? ( int ) $this->db->escape ( $filter ['Value'] ) : $this->db->escape ( $filter ['Value'] ) 
