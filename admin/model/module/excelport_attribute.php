...
class ModelModuleExcelportattribute extends ModelModuleExcelport {
	public function importXLSAttributes($language, &$allLanguages, $file) {
		...
		do {
			...
			if ((empty ( $safe_mode ) || strtolower ( $safe_mode ) == 'off') && function_exists ( 'set_time_limit' ) && stripos ( ini_get ( 'disable_functions' ), 'set_time_limit' ) === FALSE && stripos ( ini_get ( 'disable_functions' ), 'set_time_limit' ) === FALSE)
				set_time_limit ( 60 );
	...
	public function exportXLSAttributes($language, $destinationFolder = '', $export_filters) {
		...
		if ($attribute_groups_result->num_rows > 0) {
			foreach ( $attribute_groups_result->rows as $myAttributeGroupsIndex => $attribute_group_row ) {
				...
				if (empty ( $attribute_group_row ['name'] ))
					$attribute_group_row ['name'] = '-';
		...
		foreach ( $filters as $i => $filter ) {
			if (is_array ( $filter )) {
				...
				$condition = str_replace ( array (
						...
						$this->conditions ['Attributes'] [$filter ['Field']] ['field_name'],
						stripos ( $this->conditions ['Attributes'] [$filter ['Field']] ['type'], 'number' ) !== FALSE ? ( int ) $this->db->escape ( $filter ['Value'] ) : $this->db->escape ( $filter ['Value'] ) 
