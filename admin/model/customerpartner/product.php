	...
	public function getProducts($data = array()) {
		...
		if ($data) {
			...
			if (isset ( $data ['filter_for_seller'] ) and $data ['filter_for_seller'])
				$sql = "SELECT DISTINCT p.product_id,p.*,pd.name,GROUP_CONCAT( DISTINCT CAST(c2p.customer_id AS CHAR(11)) SEPARATOR \",\" ) AS customer_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "customerpartner_to_product c2p ON (c2p.product_id = p.product_id)";
