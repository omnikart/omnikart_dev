...
class ModelToolNitro extends Model {
	...
	public function cdn_precache_images(&$found_items) {
			...
			function apply_module_dimensions(&$image_settings, $width_key, $height_key, $module, &$setting) {
				...
				if (! empty ( $all_modules [$module . "_module"] ) && is_array ( $all_modules [$module . "_module"] )) {
					foreach ( $all_modules [$module . "_module"] as $my_module ) {
						if (empty ( $my_module [$width_key] ) || empty ( $my_module [$height_key] ))
							continue;
		...
		foreach ( $found_items as $item ) {
			if (stripos ( $item ['full_path'], $image_base_realpath ) !== 0)
				continue;
			
			$rel_path = substr ( $item ['full_path'], strlen ( $image_base_realpath ) );
			
			if (stripos ( $rel_path, 'cache' ) === 0)
				continue;
				...
				foreach ( $image_settings as $image_setting ) {
					...
					try {
						...
						if ($item ['size'] > NITRO_CDN_RESIZE_MIN_FILESIZE) {
							...
							if (in_array ( $image_info ['mime'], array (
									'image/gif',
									'image/png',
									'image/jpeg' 
							) )) {
	...
	public function generateUpToDateMimeArray($url) { // FUNCTION FROM Josh Sean @ http://www.php.net/manual/en/function.mime-content-type.php
		if (! empty ( $GLOBALS ['nitro.mimes'] ))
			return $GLOBALS ['nitro.mimes'];
	...
	public function getServerInfo($permission) {
		...
		/* PHP VERSION */
		if (! $permission)
			$result ['php_version'] = $text_no_permission;
		else {
		...
		/* WEB SERVER */
		if (! $permission)
			$result ['web_server'] = $text_no_permission;
		else {
			if (ini_get ( 'allow_url_fopen' ) == 1 || strtolower ( ini_get ( 'allow_url_fopen' ) ) == 'off') {
				...
				if (stripos ( $status, 'HTTP/1.1 2' ) !== FALSE)
					$type = 'success';
				else if (stripos ( $status, 'HTTP/1.1 3' ) !== FALSE)
					$type = 'warning';
				else
					$type = 'error';
				...
				$status = '<span class="info-warning"><strong>Unknown (allow_url_fopen is Off)</strong></span>';
			$result ['web_server'] = 'OS: ' . PHP_OS . ' | SAPI: ' . PHP_SAPI . ' | Status: ' . $status;
		}
		
		/* FTP FUNCTIONS */
		if (! $permission)
			$result ['ftp_functions'] = $text_no_permission;
		else {
			$ftp = array ();
		...
		/* OpenSSL */
		if (! $permission)
			$result ['openssl'] = $text_no_permission;
		else {
		...
		/* CURL */
		if (! $permission)
			$result ['curl'] = $text_no_permission;
		else {
		...
		/* MemCache */
		if (! $permission)
			$result ['memcache'] = $text_no_permission;
		else {
		...
		/* exec() */
		if (! $permission)
			$result ['exec'] = $text_no_permission;
		else {
		...
		/* zlib */
		if (! $permission)
			$result ['zlib'] = $text_no_permission;
		else {
		...
		/* safe mode */
		if (! $permission)
			$result ['safe_mode'] = $text_no_permission;
		else {
		...
		if (function_exists ( 'apache_get_modules' )) {
			...
			$shell_exec_enabled = function_exists ( 'shell_exec' ) && ! in_array ( 'shell_exec', array_map ( 'trim', explode ( ', ', ini_get ( 'disable_functions' ) ) ) ) && ! (strtolower ( ini_get ( 'safe_mode' ) ) != 'off' && ini_get ( 'safe_mode' ) != 0);
		...
		/* mod_deflate */
		if (! $permission)
			$result ['mod_deflate'] = $text_no_permission;
		else {
			if ($modules === false)
				$mod_result = '<span class="info-warning"><strong>UNKNOWN</strong></span>';
			else if (stripos ( $modules, 'mod_deflate' ) !== false)
				$mod_result = '<span class="info-success"><strong>YES</strong></span>';
			else
				$mod_result = '<span class="info-error"><strong>NO</strong></span>';
			$result ['mod_deflate'] = $mod_result;
		}
		
		/* mod_env */
		if (! $permission)
			$result ['mod_env'] = $text_no_permission;
		else {
			if ($modules === false)
				$mod_result = '<span class="info-warning"><strong>UNKNOWN</strong></span>';
			else if (stripos ( $modules, 'mod_env' ) !== false)
				$mod_result = '<span class="info-success"><strong>YES</strong></span>';
			else
				$mod_result = '<span class="info-error"><strong>NO</strong></span>';
		...
		/* mod_expires */
		if (! $permission)
			$result ['mod_expires'] = $text_no_permission;
		else {
			if ($modules === false)
				$mod_result = '<span class="info-warning"><strong>UNKNOWN</strong></span>';
			else if (stripos ( $modules, 'mod_expires' ) !== false)
				$mod_result = '<span class="info-success"><strong>YES</strong></span>';
			else
				$mod_result = '<span class="info-error"><strong>NO</strong></span>';
		...
		/* mod_headers */
		if (! $permission)
			$result ['mod_headers'] = $text_no_permission;
		else {
			if ($modules === false)
				$mod_result = '<span class="info-warning"><strong>UNKNOWN</strong></span>';
			else if (stripos ( $modules, 'mod_headers' ) !== false)
				$mod_result = '<span class="info-success"><strong>YES</strong></span>';
			else
				$mod_result = '<span class="info-error"><strong>NO</strong></span>';
		...
		/* mod_mime */
		if (! $permission)
			$result ['mod_mime'] = $text_no_permission;
		else {
			if ($modules === false)
				$mod_result = '<span class="info-warning"><strong>UNKNOWN</strong></span>';
			else if (stripos ( $modules, 'mod_mime' ) !== false)
				$mod_result = '<span class="info-success"><strong>YES</strong></span>';
			else
				$mod_result = '<span class="info-error"><strong>NO</strong></span>';
		...
		/* mod_rewrite */
		if (! $permission)
			$result ['mod_rewrite'] = $text_no_permission;
		else {
			if ($modules === false)
				$mod_result = '<span class="info-warning"><strong>UNKNOWN</strong></span>';
			else if (stripos ( $modules, 'mod_rewrite' ) !== false)
				$mod_result = '<span class="info-success"><strong>YES</strong></span>';
			else
				$mod_result = '<span class="info-error"><strong>NO</strong></span>';
		...
		/* mod_setenvif */
		if (! $permission)
			$result ['mod_setenvif'] = $text_no_permission;
		else {
			if ($modules === false)
				$mod_result = '<span class="info-warning"><strong>UNKNOWN</strong></span>';
			else if (stripos ( $modules, 'mod_setenvif' ) !== false)
				$mod_result = '<span class="info-success"><strong>YES</strong></span>';
			else
				$mod_result = '<span class="info-error"><strong>NO</strong></span>';
		...
		if (! $permission)
			$result ['path_system_nitro_cache'] = $text_no_permission;
		else {
			if (file_exists ( DIR_SYSTEM . 'nitro/cache' ) && is_writable ( DIR_SYSTEM . 'nitro/cache' ))
				$result ['path_system_nitro_cache'] = '<span class="info-success"><strong>Writable</strong></span>';
			else if (! file_exists ( DIR_SYSTEM . 'nitro/cache' ))
				$result ['path_system_nitro_cache'] = '<span><strong>Does not exist.</strong></span>';
			else
				$result ['path_system_nitro_cache'] = '<span class="info-error"><strong>Not writable.</strong></span>';
		}
		
		/* path_assets */
		if (! $permission)
			$result ['path_assets'] = $text_no_permission;
		else {
			if (file_exists ( DIR_SYSTEM . '../assets' ) && is_writable ( DIR_SYSTEM . 'nitro/cache' ))
				$result ['path_assets'] = '<span class="info-success"><strong>Writable</strong></span>';
			else if (! file_exists ( DIR_SYSTEM . '../assets' ))
				$result ['path_assets'] = '<span><strong>Does not exist.</strong></span>';
			else
				$result ['path_assets'] = '<span class="info-error"><strong>Not writable.</strong></span>';
		}
		
		/* path_system_nitro_data */
		if (! $permission)
			$result ['path_system_nitro_data'] = $text_no_permission;
		else {
			if (file_exists ( DIR_SYSTEM . 'nitro/data' ) && is_writable ( DIR_SYSTEM . 'nitro/data' ))
				$result ['path_system_nitro_data'] = '<span class="info-success"><strong>Writable</strong></span>';
			else if (! file_exists ( DIR_SYSTEM . 'nitro/data' ))
				$result ['path_system_nitro_data'] = '<span><strong>Does not exist.</strong></span>';
			else
				$result ['path_system_nitro_data'] = '<span class="info-error"><strong>Not writable.</strong></span>';
		}
		
		/* path_system_nitro_data_googlepagespeed-desktop */
		if (! $permission)
			$result ['path_system_nitro_data_googlepagespeed-desktop'] = $text_no_permission;
		else {
			if (file_exists ( DIR_SYSTEM . 'nitro/data/googlepagespeed-desktop.tpl' ) && is_writable ( DIR_SYSTEM . 'nitro/data/googlepagespeed-desktop.tpl' ))
				$result ['path_system_nitro_data_googlepagespeed-desktop'] = '<span class="info-success"><strong>Writable</strong></span>';
			else if (! file_exists ( DIR_SYSTEM . 'nitro/data/googlepagespeed-desktop.tpl' ))
				$result ['path_system_nitro_data_googlepagespeed-desktop'] = '<span><strong>Does not exist.</strong></span>';
			else
				$result ['path_system_nitro_data_googlepagespeed-desktop'] = '<span class="info-error"><strong>Not writable.</strong></span>';
		}
		
		/* path_system_nitro_data_googlepagespeed-mobile */
		if (! $permission)
			$result ['path_system_nitro_data_googlepagespeed-mobile'] = $text_no_permission;
		else {
			if (file_exists ( DIR_SYSTEM . 'nitro/data/googlepagespeed-mobile.tpl' ) && is_writable ( DIR_SYSTEM . 'nitro/data/googlepagespeed-mobile.tpl' ))
				$result ['path_system_nitro_data_googlepagespeed-mobile'] = '<span class="info-success"><strong>Writable</strong></span>';
			else if (! file_exists ( DIR_SYSTEM . 'nitro/data/googlepagespeed-mobile.tpl' ))
				$result ['path_system_nitro_data_googlepagespeed-mobile'] = '<span><strong>Does not exist.</strong></span>';
			else
				$result ['path_system_nitro_data_googlepagespeed-mobile'] = '<span class="info-error"><strong>Not writable.</strong></span>';
		}
		
		/* path_system_nitro_data_persistence */
		if (! $permission)
			$result ['path_system_nitro_data_persistence'] = $text_no_permission;
		else {
			if (file_exists ( DIR_SYSTEM . 'nitro/data/persistence.tpl' ) && is_writable ( DIR_SYSTEM . 'nitro/data/persistence.tpl' ))
				$result ['path_system_nitro_data_persistence'] = '<span class="info-success"><strong>Writable</strong></span>';
			else if (! file_exists ( DIR_SYSTEM . 'nitro/data/persistence.tpl' ))
				$result ['path_system_nitro_data_persistence'] = '<span><strong>Does not exist.</strong></span>';
			else
				$result ['path_system_nitro_data_persistence'] = '<span class="info-error"><strong>Not writable.</strong></span>';
		}
		
		/* path_system_nitro_data_amazon_persistence */
		if (! $permission)
			$result ['path_system_nitro_data_amazon_persistence'] = $text_no_permission;
		else {
			if (file_exists ( DIR_SYSTEM . 'nitro/data/amazon_persistence.tpl' ) && is_writable ( DIR_SYSTEM . 'nitro/data/amazon_persistence.tpl' ))
				$result ['path_system_nitro_data_amazon_persistence'] = '<span class="info-success"><strong>Writable</strong></span>';
			else if (! file_exists ( DIR_SYSTEM . 'nitro/data/amazon_persistence.tpl' ))
				$result ['path_system_nitro_data_amazon_persistence'] = '<span><strong>Does not exist.</strong></span>';
			else
				$result ['path_system_nitro_data_amazon_persistence'] = '<span class="info-error"><strong>Not writable.</strong></span>';
		}
		
		/* path_system_nitro_data_ftp_persistence */
		if (! $permission)
			$result ['path_system_nitro_data_ftp_persistence'] = $text_no_permission;
		else {
			if (file_exists ( DIR_SYSTEM . 'nitro/data/ftp_persistence.tpl' ) && is_writable ( DIR_SYSTEM . 'nitro/data/ftp_persistence.tpl' ))
				$result ['path_system_nitro_data_ftp_persistence'] = '<span class="info-success"><strong>Writable</strong></span>';
			else if (! file_exists ( DIR_SYSTEM . 'nitro/data/ftp_persistence.tpl' ))
				$result ['path_system_nitro_data_ftp_persistence'] = '<span><strong>Does not exist.</strong></span>';
			else
				$result ['path_system_nitro_data_ftp_persistence'] = '<span class="info-error"><strong>Not writable.</strong></span>';
		}
	...
	public function clearProductCache($product_id) {
		...
		$this->db->query ( "DELETE FROM " . DB_PREFIX . "nitro_product_cache WHERE product_id='" . ( int ) $product_id . "'" );
		...
		if (file_exists ( $folder )) {
			if (is_writeable ( $folder )) {
				if (is_dir ( $folder )) {
					...
					if (! rmdir ( $folder ))
						throw new Exception ( 'Delete not successful. The path ' . $folder . ' could not get deleted.' );
				} else {
					if (! unlink ( $folder ))
						throw new Exception ( 'Delete not successful. The path ' . $folder . ' could not get deleted.' );
				}
			} else
				throw new Exception ( 'Delete not successful. The path ' . $folder . ' is not writable.' );
		}
	}
